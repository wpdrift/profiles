<?php
/**
 * Profiles Core Caching Functions.
 *
 ...
 * @return int
 */
function profiles_core_clear_cache() {
	global $cache_path;
	...
	if ( function_exists( 'prune_super_cache' ) ) {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'profiles_core_clear_cache' );
 ...
 */
function profiles_core_clear_user_object_cache( $user_id ) {
	wp_cache_delete( 'profiles_user_' . $user_id, 'profiles' );
}
 ...
 * @since 1.6.0
 */
function profiles_core_clear_member_count_caches() {
	wp_cache_delete( 'profiles_total_member_count', 'profiles' );
	delete_transient( 'profiles_active_member_count' );
}
add_action( 'profiles_core_activated_user',         'profiles_core_clear_member_count_caches' );
add_action( 'profiles_core_process_spammer_status', 'profiles_core_clear_member_count_caches' );
add_action( 'profiles_core_deleted_account',        'profiles_core_clear_member_count_caches' );
add_action( 'profiles_first_activity_for_member',   'profiles_core_clear_member_count_caches' );
add_action( 'deleted_user',                   'profiles_core_clear_member_count_caches' );
 ...
 */
function profiles_core_clear_directory_pages_cache_page_edit( $post_id ) {
	...
	wp_cache_delete( 'directory_pages', 'profiles_pages' );
}
add_action( 'save_post', 'profiles_core_clear_directory_pages_cache_page_edit' );

/**
 * Clear the directory_pages cache when the profiles-pages option is updated.
 *
 * @since 2.0.0
 *
 * @param string $option Option name.
 */
function profiles_core_clear_directory_pages_cache_settings_edit( $option ) {
	if ( 'profiles-pages' === $option ) {
		wp_cache_delete( 'directory_pages', 'profiles_pages' );
	}
}
add_action( 'update_option', 'profiles_core_clear_directory_pages_cache_settings_edit' );
 ...
 * @param string $option Option name.
 */
function profiles_core_clear_root_options_cache( $option ) {
	foreach ( array( 'add_option', 'add_site_option', 'update_option', 'update_site_option' ) as $action ) {
		remove_action( $action, 'profiles_core_clear_root_options_cache' );
	}

	// Surrounding code prevents infinite loops on WP < 4.4.
	$keys = array_keys( profiles_get_default_options() );

	foreach ( array( 'add_option', 'add_site_option', 'update_option', 'update_site_option' ) as $action ) {
		add_action( $action, 'profiles_core_clear_root_options_cache' );
	}
	...
	if ( in_array( $option, $keys ) ) {
		wp_cache_delete( 'root_blog_options', 'profiles' );
	}
}
add_action( 'update_option', 'profiles_core_clear_root_options_cache' );
add_action( 'update_site_option', 'profiles_core_clear_root_options_cache' );
add_action( 'add_option', 'profiles_core_clear_root_options_cache' );
add_action( 'add_site_option', 'profiles_core_clear_root_options_cache' );
 ...
 * @return array
 */
function profiles_get_non_cached_ids( $item_ids, $cache_group ) {
	$uncached = array();
 ...
 */
function profiles_update_meta_cache( $args = array() ) {
	global $wpdb;
	...
	$defaults = array(
		...
		'cache_key_prefix' => ''       // Prefix to use when creating cache key names. Eg 'profiles_groups_groupmeta'.
	);
	...
	$uncached_ids = profiles_get_non_cached_ids( $object_ids, $cache_group );
