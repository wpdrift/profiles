 ...
 *
 * @package Profiles
 * @suprofilesackage XProfileFilters
 * @since 1.0.0
 */
...
add_filter( 'profiles_get_the_profile_field_value',           'wptexturize'        );
add_filter( 'profiles_get_the_profile_field_value',           'convert_chars'      );
add_filter( 'profiles_get_the_profile_field_value',           'wpautop'            );
add_filter( 'profiles_get_the_profile_field_value',           'force_balance_tags' );
...
add_filter( 'profiles_get_the_profile_group_name',            'stripslashes' );
add_filter( 'profiles_get_the_profile_group_description',     'stripslashes' );
add_filter( 'profiles_get_the_profile_field_value',           'stripslashes' );
add_filter( 'profiles_get_the_profile_field_edit_value',      'stripslashes' );
...
// Save field options.
add_filter( 'xprofile_field_options_before_save', 'profiles_xprofile_sanitize_field_options' );
add_filter( 'xprofile_field_default_before_save', 'profiles_xprofile_sanitize_field_default' );
 ...
 * @return mixed
 */
function profiles_xprofile_sanitize_field_options( $field_options = '' ) {
 ...
 * @return mixed
 */
function profiles_xprofile_sanitize_field_default( $field_default = '' ) {
...
function profiles_xprofile_filter_kses( $content, $data_obj = null ) {
	...
	if ( $data_obj instanceof Profiles_XProfile_ProfileData && profiles_xprofile_is_richtext_enabled_for_field( $data_obj->field_id ) ) {
		$richtext_tags = array(
	 ...
	 * @param Profiles_XProfile_ProfileData $data_obj             The Profiles_XProfile_ProfileData object.
	 */
...
/**
 * Apply display_filter() filters as defined by Profiles_XProfile_Field_Type classes, when inside a profiles_has_profile() loop.
 *
 * @since 2.1.0
...
/**
 * Apply pre_validate_filter() filters as defined by the Profiles_XProfile_Field_Type classes before validating.
 *
 * @since 2.1.0
 *
		...
		foreach ( (array) $values as $value ) {
			...
			// Is not clickable.
				...
				// Less than 5 spaces.
					...
					$search_url   = add_query_arg( array( $query_arg => urlencode( $value ) ), profiles_get_members_directory_permalink() );
...
function profiles_xprofile_filter_comments( $comments, $post_id = 0 ) {
	...
	if ( $fullnames = profiles_core_get_user_displaynames( $user_ids ) ) {
 ...
 * @since 1.7.0
 *
 * @param Profiles_User_Query $user_query   User query to filter.
 * @param string        $user_ids_sql SQL statement to use.
 */
function profiles_xprofile_filter_user_query_populate_extras( Profiles_User_Query $user_query, $user_ids_sql = '' ) {

	if ( ! profiles_is_active( 'xprofile' ) ) {
		return;
	}

	$user_id_names = profiles_core_get_user_displaynames( $user_query->user_ids );
...
add_filter( 'profiles_user_query_populate_extras', 'profiles_xprofile_filter_user_query_populate_extras', 2, 2 );

/**
 * Parse 'xprofile_query' argument passed to Profiles_User_Query.
 *
 * @since 2.2.0
 *
 * @param Profiles_User_Query $q User query object.
 */
function profiles_xprofile_add_xprofile_query_to_user_query( Profiles_User_Query $q ) {
	...
	$xprofile_query = new Profiles_XProfile_Query( $q->query_vars['xprofile_query'] );
...
add_action( 'profiles_pre_user_query', 'profiles_xprofile_add_xprofile_query_to_user_query' );
 ...
 * @return string
 */
function profiles_xprofile_filter_meta_query( $q ) {
	global $wpdb;
