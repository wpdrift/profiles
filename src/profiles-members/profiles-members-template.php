...
if ( ! profiles()->do_autoload ) {
	require dirname( __FILE__ ) . '/classes/class-profiles-core-members-template.php';
}
 ...
 *
 */
function profiles_profile_slug() {
	echo profiles_get_profile_slug();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_profile_slug() {
		 ...
		 */
		return apply_filters( 'profiles_get_profile_slug', profiles()->profile->slug );
	}
 ...
 *
 */
function profiles_members_slug() {
	echo profiles_get_members_slug();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_members_slug() {
		 ...
		 */
		return apply_filters( 'profiles_get_members_slug', profiles()->members->slug );
	}
 ...
 *
 */
function profiles_members_root_slug() {
	echo profiles_get_members_root_slug();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_members_root_slug() {
		 ...
		 */
		return apply_filters( 'profiles_get_members_root_slug', profiles()->members->root_slug );
	}
 ...
 * @since 2.5.0
 */
function profiles_members_member_type_base() {
	echo esc_url( profiles_get_members_member_type_base() );
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_members_member_type_base() {
		/**
		 ...
		 * @param string $base
		 */
		return apply_filters( 'profiles_members_member_type_base', _x( 'type', 'member type URL base', 'profiles' ) );
	}
 ...
 *
 */
function profiles_members_directory_permalink() {
	echo esc_url( profiles_get_members_directory_permalink() );
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_members_directory_permalink() {
		 ...
		 */
		return apply_filters( 'profiles_get_members_directory_permalink', trailingslashit( profiles_get_root_domain() . '/' . profiles_get_members_root_slug() ) );
	}
 ...
 */
function profiles_member_type_directory_permalink( $member_type = '' ) {
	echo esc_url( profiles_get_member_type_directory_permalink( $member_type ) );
}
	/**
	 ...
	 */
	function profiles_get_member_type_directory_permalink( $member_type = '' ) {

		if ( $member_type ) {
			$_member_type = $member_type;
		} else {
			// Fall back on the current member type.
			$_member_type = profiles_get_current_member_type();
		}

		$type = profiles_get_member_type_object( $_member_type );
		 ...
		 */
		return apply_filters( 'profiles_get_member_type_directory_permalink', trailingslashit( profiles_get_members_directory_permalink() . profiles_get_members_member_type_base() . '/' . $type->directory_slug ), $type, $member_type );
	}
 ...
 *
 */
function profiles_signup_slug() {
	echo profiles_get_signup_slug();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_signup_slug() {
		$profiles = profiles();

		if ( !empty( $profiles->pages->register->slug ) ) {
			$slug = $profiles->pages->register->slug;
		} elseif ( defined( 'Profiles_REGISTER_SLUG' ) ) {
			$slug = Profiles_REGISTER_SLUG;
		} else {
			$slug = 'register';
		}
		 ...
		 */
		return apply_filters( 'profiles_get_signup_slug', $slug );
	}
 ...
 *
 */
function profiles_activate_slug() {
	echo profiles_get_activate_slug();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_activate_slug() {
		$profiles = profiles();

		if ( !empty( $profiles->pages->activate->slug ) ) {
			$slug = $profiles->pages->activate->slug;
		} elseif ( defined( 'Profiles_ACTIVATION_SLUG' ) ) {
			$slug = Profiles_ACTIVATION_SLUG;
		} else {
			$slug = 'activate';
		}
		 ...
		 */
		return apply_filters( 'profiles_get_activate_slug', $slug );
	}

/**
 * Initialize the members loop.
 *
 * Based on the $args passed, profiles_has_members() populates the $members_template
 * global, enabling the use of Profiles templates and template functions to
 * display a list of members.
 *
 * @since 1.2.0
 *
 * @global object $members_template {@link Profiles_Members_Template}
 *
 * @param array|string $args {
 *     Arguments for limiting the contents of the members loop. Most arguments
 *     are in the same format as {@link Profiles_User_Query}. However, because
 *     the format of the arguments accepted here differs in a number of ways,
 *     and because profiles_has_members() determines some default arguments in a
 ...
 */
function profiles_has_members( $args = '' ) {
	...
	// User filtering.
	if ( profiles_is_user_friends() && ! profiles_is_user_friend_requests() ) {
		$user_id = profiles_displayed_user_id();
	}

	$member_type = profiles_get_current_member_type();
	...
	$search_query_arg = profiles_core_get_component_search_query_arg( 'members' );
	...
	$members_template = new Profiles_Core_Members_Template(
		$r['type'],
		$r['page'],
		$r['per_page'],
		$r['max'],
	 ...
	 * @param array $r                Array of arguments passed into the Profiles_Core_Members_Template class.
	 */
	return apply_filters( 'profiles_has_members', $members_template->has_members(), $members_template, $r );
}
 ...
 * @since 1.2.0
 */
function profiles_members_pagination_count() {
	echo profiles_get_members_pagination_count();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_members_pagination_count() {
		global $members_template;
		...
		$from_num  = profiles_core_number_format( $start_num );
		$to_num    = profiles_core_number_format( ( $start_num + ( $members_template->pag_num - 1 ) > $members_template->total_member_count ) ? $members_template->total_member_count : $start_num + ( $members_template->pag_num - 1 ) );
		$total     = profiles_core_number_format( $members_template->total_member_count );
		 ...
		 */
		return apply_filters( 'profiles_members_pagination_count', $pag );
	}
 ...
 * @since 1.2.0
 */
function profiles_members_pagination_links() {
	echo profiles_get_members_pagination_links();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_members_pagination_links() {
		global $members_template;
		 ...
		 */
		return apply_filters( 'profiles_get_members_pagination_links', $members_template->pag_links );
	}
 ...
 *
 */
function profiles_member_user_id() {
	echo profiles_get_member_user_id();
}
	/**
	 ...
	 */
	function profiles_get_member_user_id() {
		global $members_template;
		 ...
		 */
		return apply_filters( 'profiles_get_member_user_id', $member_id );
	}
 ...
 */
function profiles_member_class( $classes = array() ) {
	echo profiles_get_member_class( $classes );
}
	/**
	 ...
	 */
	function profiles_get_member_class( $classes = array() ) {
		global $members_template;
		...
		if ( $members_template->member_count > 1 ) {
			...
			$classes[] = 'profiles-single-member';
		}
		...
		if ( ! empty( $members_template->member->last_activity ) ) {
			...
			$current_time  = strtotime( profiles_core_current_time() );
		...
		if ( profiles_loggedin_user_id() === (int) $members_template->member->id ) {
			$classes[] = 'is-current-user';
		}

		// Add current user member types.
		if ( $member_types = profiles_get_member_type( $members_template->member->id, false ) ) {
		 ...
		 */
		$classes = apply_filters( 'profiles_get_member_class', $classes );
 ...
 * @since 1.2.5
 */
function profiles_member_user_nicename() {
	echo profiles_get_member_user_nicename();
}
	/**
	 ...
	 */
	function profiles_get_member_user_nicename() {
		global $members_template;
		 ...
		 */
		return apply_filters( 'profiles_get_member_user_nicename', $members_template->member->user_nicename );
	}
 ...
 * @since 1.2.5
 */
function profiles_member_user_login() {
	echo profiles_get_member_user_login();
}
	/**
	 ...
	 */
	function profiles_get_member_user_login() {
		global $members_template;
		 ...
		 */
		return apply_filters( 'profiles_get_member_user_login', $members_template->member->user_login );
	}
 ...
 * @since 1.2.5
 */
function profiles_member_user_email() {
	echo profiles_get_member_user_email();
}
	/**
	 ...
	 */
	function profiles_get_member_user_email() {
		global $members_template;
		 ...
		 */
		return apply_filters( 'profiles_get_member_user_email', $members_template->member->user_email );
	}
 ...
 * @return bool
 */
function profiles_member_is_loggedin_user() {
	global $members_template;
	 ...
	 */
	return apply_filters( 'profiles_member_is_loggedin_user', profiles_loggedin_user_id() == $members_template->member->id ? true : false );
}
 ...
 * @since 1.2.0
 *
 * @see profiles_get_member_avatar() for description of arguments.
 *
 * @param array|string $args See {@link profiles_get_member_avatar()}.
 */
function profiles_member_avatar( $args = '' ) {
	 ...
	 * @param array|string $args  See {@link profiles_get_member_avatar()}.
	 */
	echo apply_filters( 'profiles_member_avatar', profiles_get_member_avatar( $args ), $args );
}
	/**
	 ...
	 * @since 1.2.0
	 *
	 * @see profiles_core_fetch_avatar() For a description of arguments and
	 *      return values.
	 *
	 * @param array|string $args  {
	 *     Arguments are listed here with an explanation of their defaults.
	 *     For more information about the arguments, see
	 *     {@link profiles_core_fetch_avatar()}.
	 ...
	 */
	function profiles_get_member_avatar( $args = '' ) {
		global $members_template;
		 ...
		 *
		 * @param string $value Formatted HTML <img> element, or raw avatar URL based on $html arg.
		 * @param array  $r     Array of parsed arguments. See {@link profiles_get_member_avatar()}.
		 */
		return apply_filters( 'profiles_get_member_avatar', profiles_core_fetch_avatar( array( 'item_id' => $members_template->member->id, 'type' => $type, 'alt' => $alt, 'css_id' => $id, 'class' => $class, 'width' => $width, 'height' => $height, 'email' => $members_template->member->user_email ) ), $r );
	}
 ...
 * @since 1.2.0
 */
function profiles_member_permalink() {
	echo profiles_get_member_permalink();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_member_permalink() {
		global $members_template;
		 ...
		 */
		return apply_filters( 'profiles_get_member_permalink', profiles_core_get_user_domain( $members_template->member->id, $members_template->member->user_nicename, $members_template->member->user_login ) );
	}

	/**
	 * Alias of {@link profiles_member_permalink()}.
	 *
	 * @since 1.2.0
	 */
	function profiles_member_link() { echo profiles_get_member_permalink(); }

	/**
	 * Alias of {@link profiles_get_member_permalink()}.
	 *
	 * @since 1.2.0
	 */
	function profiles_get_member_link() { return profiles_get_member_permalink(); }
...
function profiles_member_name() {
	 ...
	 */
	echo apply_filters( 'profiles_member_name', profiles_get_member_name() );
}
	/**
	 ...
	 */
	function profiles_get_member_name() {
		global $members_template;
		 ...
		 */
		return apply_filters( 'profiles_get_member_name', $members_template->member->fullname );
	}
	add_filter( 'profiles_get_member_name', 'wp_filter_kses' );
	add_filter( 'profiles_get_member_name', 'stripslashes'   );
	add_filter( 'profiles_get_member_name', 'strip_tags'     );
	add_filter( 'profiles_get_member_name', 'esc_html'       );
 ...
 * @since 1.2.0
 *
 * @param array $args See {@link profiles_get_member_last_active()}.
 */
function profiles_member_last_active( $args = array() ) {
	echo profiles_get_member_last_active( $args );
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_member_last_active( $args = array() ) {
		global $members_template;
		...
		if ( isset( $members_template->member->last_activity ) ) {
			...
			$last_activity = ! empty( $r['active_format'] )
				? profiles_core_get_last_activity( $members_template->member->last_activity, $r['active_format'] )
				: profiles_core_time_since( $members_template->member->last_activity );
		 ...
		 */
		return apply_filters( 'profiles_member_last_active', $last_activity, $r );
	}
 ...
 * @since 1.2.0
 *
 * @see profiles_get_member_profile_data() for a description of params.
 *
 * @param array|string $args See {@link profiles_get_member_profile_data()}.
 */
function profiles_member_profile_data( $args = '' ) {
	echo profiles_get_member_profile_data( $args );
}
	/**
	 * Get a piece of user profile data.
	 *
	 * When used in a profiles_has_members() loop, this function will attempt
	 ...
	 */
	function profiles_get_member_profile_data( $args = '' ) {
		global $members_template;
		...
		} elseif ( profiles_displayed_user_id() ) {
			$default_user_id = profiles_displayed_user_id();
		}
		...
		if ( empty( $profile_data ) && method_exists( 'Profiles_XProfile_ProfileData', 'get_all_for_user' ) ) {
			$profile_data = Profiles_XProfile_ProfileData::get_all_for_user( $r['user_id'] );
		}
		 ...
		 */
		return apply_filters( 'profiles_get_member_profile_data', $data, $r );
	}
 ...
 * @since 1.2.0
 */
function profiles_member_registered() {
	echo profiles_get_member_registered();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_member_registered() {
		global $members_template;

		$registered = esc_attr( profiles_core_get_last_activity( $members_template->member->user_registered, _x( 'registered %s', 'Records the timestamp that the user registered into the activity stream', 'profiles' ) ) );
		 ...
		 */
		return apply_filters( 'profiles_member_registered', $registered );
	}
 ...
 * @since 1.2.0
 */
function profiles_member_random_profile_data() {
	global $members_template;
 ...
 * @since 1.2.0
 */
function profiles_member_hidden_fields() {
	$query_arg = profiles_core_get_component_search_query_arg( 'members' );
 ...
 * @since 1.0.0
 */
function profiles_directory_members_search_form() {
	...
	if ( ! empty( $_REQUEST[ $query_arg ] ) ) {
		...
		$search_value = profiles_get_search_default_text( 'members' );
	}
	 ...
	 */
	echo apply_filters( 'profiles_directory_members_search_form', $search_form_html );
}
 ...
 * @since 1.2.0
 */
function profiles_total_site_member_count() {
	echo profiles_get_total_site_member_count();
}
	/**
	 ...
	 * @return int
	 */
	function profiles_get_total_site_member_count() {
		 ...
		 */
		return apply_filters( 'profiles_get_total_site_member_count', profiles_core_number_format( profiles_core_get_total_member_count() ) );
	}
 ...
 */
function profiles_get_loggedin_user_nav() {
	$profiles = profiles();
	...
	foreach ( (array) $profiles->members->nav->get_primary() as $nav_item ) {
		...
		if ( ! profiles_is_directory() && ! empty( $profiles->active_components[ profiles_current_component() ] ) && $profiles->active_components[ profiles_current_component() ] == $nav_item->css_id ) {
		...
		// if it exists.
		if ( !profiles_is_my_profile() && profiles_displayed_user_id() ) {
			$selected = '';
		}

		// Echo out the final list item.
		echo apply_filters_ref_array( 'profiles_get_loggedin_user_nav_' . $nav_item->css_id, array( '<li id="li-nav-' . $nav_item->css_id . '" ' . $selected . '><a id="my-' . $nav_item->css_id . '" href="' . $nav_item->link . '">' . $nav_item->name . '</a></li>', &$nav_item ) );
	}

	// Always add a log out list item to the end of the navigation.
	$logout_link = '<li><a id="wp-logout" href="' .  wp_logout_url( profiles_get_root_domain() ) . '">' . __( 'Log Out', 'profiles' ) . '</a></li>';

	echo apply_filters( 'profiles_logout_nav_link', $logout_link );
}
 ...
 * @since 2.6.0
 */
function profiles_displayed_user_front_template_part() {
	$located = profiles_displayed_user_get_front_template();
	...
	if ( false !== $located ) {
		...
		/**
		 * Let plugins adding an action to profiles_get_template_part get it from here
		 *
 ...
 */
function profiles_displayed_user_get_front_template( $displayed_user = null ) {
	if ( ! is_object( $displayed_user ) || empty( $displayed_user->id ) ) {
		$displayed_user = profiles_get_displayed_user();
	}
	 ...
	 * type using the hook 'profiles_register_member_types'
	 */
	if ( profiles_get_member_types() ) {
		$displayed_user_member_type = profiles_get_member_type( $displayed_user->id );
	 ...
	 */
	return profiles_locate_template( apply_filters( 'profiles_displayed_user_get_front_template', $template_names ), false, true );
}
 ...
 * @since 2.6.0
 */
function profiles_displayed_user_has_front_template() {
	$displayed_user = profiles_get_displayed_user();
 ...
 * @since 1.1.0
 */
function profiles_get_displayed_user_nav() {
	$profiles = profiles();

	foreach ( $profiles->members->nav->get_primary() as $user_nav_item ) {
		if ( empty( $user_nav_item->show_for_displayed_user ) && ! profiles_is_my_profile() ) {
			continue;
		}

		$selected = '';
		if ( profiles_is_current_component( $user_nav_item->slug ) ) {
			$selected = ' class="current selected"';
		}

		if ( profiles_loggedin_user_domain() ) {
			$link = str_replace( profiles_loggedin_user_domain(), profiles_displayed_user_domain(), $user_nav_item->link );
		} else {
			$link = trailingslashit( profiles_displayed_user_domain() . $user_nav_item->link );
		}
		 ...
		 *                              Passed by reference.
		 */
		echo apply_filters_ref_array( 'profiles_get_displayed_user_nav_' . $user_nav_item->css_id, array( '<li id="' . $user_nav_item->css_id . '-personal-li" ' . $selected . '><a id="user-' . $user_nav_item->css_id . '" href="' . $link . '">' . $user_nav_item->name . '</a></li>', &$user_nav_item ) );
	}
}
 ...
 *              False otherwise
 */
function profiles_displayed_user_use_cover_image_header() {
	return (bool) profiles_is_active( 'xprofile', 'cover_image' ) && ! profiles_disable_cover_image_uploads() && profiles_attachments_is_wp_version_supported();
}
 ...
 * @since 1.1.0
 *
 * @see profiles_get_loggedin_user_avatar() for a description of params.
 *
 * @param array|string $args Array of arguments for logged in user avatar.
 */
function profiles_loggedin_user_avatar( $args = '' ) {
	echo profiles_get_loggedin_user_avatar( $args );
}
	/**
	 ...
	 * @return string User avatar string.
	 */
	function profiles_get_loggedin_user_avatar( $args = '' ) {

		$r = wp_parse_args( $args, array(
			'item_id' => profiles_loggedin_user_id(),
			'type'    => 'thumb',
			'width'   => false,
			'height'  => false,
			'html'    => true,
			'alt'     => sprintf( __( 'Profile picture of %s', 'profiles' ), profiles_get_loggedin_user_fullname() )
		) );
		 ...
		 */
		return apply_filters( 'profiles_get_loggedin_user_avatar', profiles_core_fetch_avatar( $r ), $r, $args );
	}
 ...
 * @since 1.1.0
 *
 * @see profiles_get_displayed_user_avatar() for a description of params.
 *
 * @param array|string $args Array of arguments for displayed user avatar.
 */
function profiles_displayed_user_avatar( $args = '' ) {
	echo profiles_get_displayed_user_avatar( $args );
}
	/**
	 ...
	 * @return string User avatar string.
	 */
	function profiles_get_displayed_user_avatar( $args = '' ) {
		...
		$r = wp_parse_args( $args, array(
			...
			'height'  => false,
			'html'    => true,
			'alt'     => sprintf( __( 'Profile picture of %s', 'profiles' ), profiles_get_displayed_user_fullname() )
		) );
		 ...
		 * @param array  $args  Array of initial arguments.
		 */
		return apply_filters( 'profiles_get_displayed_user_avatar', profiles_core_fetch_avatar( $r ), $r, $args );
	}
 ...
 * @since 1.5.0
 */
function profiles_displayed_user_email() {
	echo profiles_get_displayed_user_email();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_displayed_user_email() {
		$profiles = profiles();
		 ...
		 */
		return apply_filters( 'profiles_get_displayed_user_email', esc_attr( $retval ) );
	}
 ...
 * @since 1.0.0
 *
 * @see profiles_get_last_activity() for a description of parameters.
 *
 * @param int $user_id See {@link profiles_get_last_activity()}.
 */
function profiles_last_activity( $user_id = 0 ) {
	 ...
	 */
	echo apply_filters( 'profiles_last_activity', profiles_get_last_activity( $user_id ) );
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_last_activity( $user_id = 0 ) {

		if ( empty( $user_id ) )
			$user_id = profiles_displayed_user_id();

		$last_activity = profiles_core_get_last_activity( profiles_get_user_last_activity( $user_id ), __('active %s', 'profiles') );
		 ...
		 */
		return apply_filters( 'profiles_get_last_activity', $last_activity, $user_id );
	}
 ...
 * @since 1.2.0
 */
function profiles_user_firstname() {
	echo profiles_get_user_firstname();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_user_firstname( $name = false ) {

		// Try to get displayed user.
		if ( empty( $name ) )
			$name = profiles_get_displayed_user_fullname();

		// Fall back on logged in user.
		if ( empty( $name ) )
			$name = profiles_get_loggedin_user_fullname();
		 ...
		 */
		return apply_filters( 'profiles_get_user_firstname', $fullname[0], $fullname );
	}
 ...
 * @since 1.2.4
 */
function profiles_loggedin_user_link() {
	echo profiles_get_loggedin_user_link();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_loggedin_user_link() {
		 ...
		 */
		return apply_filters( 'profiles_get_loggedin_user_link', profiles_loggedin_user_domain() );
	}
 ...
 * @since 1.2.4
 */
function profiles_displayed_user_link() {
	echo profiles_get_displayed_user_link();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_displayed_user_link() {
		 ...
		 */
		return apply_filters( 'profiles_get_displayed_user_link', profiles_displayed_user_domain() );
	}

	/**
	 * Alias of {@link profiles_displayed_user_domain()}.
	 *
	 * @deprecated
	 */
	function profiles_user_link() { profiles_displayed_user_domain(); }

/**
 * Alias of {@link profiles_displayed_user_id()}.
 *
 * @since 1.0.0
 */
function profiles_current_user_id() { return profiles_displayed_user_id(); }
 ...
 * @return string
 */
function profiles_displayed_user_domain() {
	$profiles = profiles();
	 ...
	 */
	return apply_filters( 'profiles_displayed_user_domain', isset( $profiles->displayed_user->domain ) ? $profiles->displayed_user->domain : '' );
}
 ...
 * @return string
 */
function profiles_loggedin_user_domain() {
	$profiles = profiles();
	 ...
	 */
	return apply_filters( 'profiles_loggedin_user_domain', isset( $profiles->loggedin_user->domain ) ? $profiles->loggedin_user->domain : '' );
}
 ...
 * @since 1.0.0
 */
function profiles_displayed_user_fullname() {
	echo profiles_get_displayed_user_fullname();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_displayed_user_fullname() {
		$profiles = profiles();
		 ...
		 */
		return apply_filters( 'profiles_displayed_user_fullname', isset( $profiles->displayed_user->fullname ) ? $profiles->displayed_user->fullname : '' );
	}

	/**
	 * Alias of {@link profiles_get_displayed_user_fullname()}.
	 *
	 * @since 1.0.0
	 */
	function profiles_user_fullname() { echo profiles_get_displayed_user_fullname(); }
 ...
 * @since 1.0.0
 */
function profiles_loggedin_user_fullname() {
	echo profiles_get_loggedin_user_fullname();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_loggedin_user_fullname() {
		$profiles = profiles();
		 ...
		 */
		return apply_filters( 'profiles_get_loggedin_user_fullname', isset( $profiles->loggedin_user->fullname ) ? $profiles->loggedin_user->fullname : '' );
	}
 ...
 * @since 1.2.0
 */
function profiles_displayed_user_username() {
	echo profiles_get_displayed_user_username();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_displayed_user_username() {
		$profiles = profiles();

		if ( profiles_displayed_user_id() ) {
			$username = profiles_core_get_username( profiles_displayed_user_id(), $profiles->displayed_user->userdata->user_nicename, $profiles->displayed_user->userdata->user_login );
		} else {
			$username = '';
		}
		 ...
		 */
		return apply_filters( 'profiles_get_displayed_user_username', $username );
	}
 ...
 * @since 1.2.0
 */
function profiles_loggedin_user_username() {
	echo profiles_get_loggedin_user_username();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_loggedin_user_username() {
		$profiles = profiles();

		if ( profiles_loggedin_user_id() ) {
			$username = profiles_core_get_username( profiles_loggedin_user_id(), $profiles->loggedin_user->userdata->user_nicename, $profiles->loggedin_user->userdata->user_login );
		} else {
			$username = '';
		}
		 ...
		 */
		return apply_filters( 'profiles_get_loggedin_user_username', $username );
	}
/**
 ...
 * @since 2.3.0
 */
function profiles_current_member_type_message() {
	echo profiles_get_current_member_type_message();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_current_member_type_message() {
		$type_object = profiles_get_member_type_object( profiles_get_current_member_type() );
		 ...
		 */
		return apply_filters( 'profiles_get_current_member_type_message', $message );
	}
 ...
 */
function profiles_has_custom_activation_page() {
	static $has_page = false;

	if ( empty( $has_page ) )
		$has_page = profiles_get_activate_slug() && profiles_locate_template( array( 'registration/activate.php', 'members/activate.php', 'activate.php' ), false );
 ...
 * @since 1.0.0
 */
function profiles_activation_page() {
	echo esc_url( profiles_get_activation_page() );
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_activation_page() {
		if ( profiles_has_custom_activation_page() ) {
			$page = trailingslashit( profiles_get_root_domain() . '/' . profiles_get_activate_slug() );
		} else {
			$page = trailingslashit( profiles_get_root_domain() ) . 'wp-activate.php';
		}
		 ...
		 */
		return apply_filters( 'profiles_get_activation_page', $page );
	}
 ...
 * @since 1.1.0
 */
function profiles_signup_username_value() {
	echo profiles_get_signup_username_value();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_signup_username_value() {
		$value = '';
		 ...
		 */
		return apply_filters( 'profiles_get_signup_username_value', $value );
	}
 ...
 * @since 1.1.0
 */
function profiles_signup_email_value() {
	echo profiles_get_signup_email_value();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_signup_email_value() {
		$value = '';
		 ...
		 */
		return apply_filters( 'profiles_get_signup_email_value', $value );
	}
 ...
 * @since 1.1.0
 */
function profiles_signup_with_blog_value() {
	echo profiles_get_signup_with_blog_value();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_signup_with_blog_value() {
		$value = '';
		 ...
		 */
		return apply_filters( 'profiles_get_signup_with_blog_value', $value );
	}
 ...
 * @since 1.1.0
 */
function profiles_signup_blog_url_value() {
	echo profiles_get_signup_blog_url_value();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_signup_blog_url_value() {
		$value = '';
		 ...
		 */
		return apply_filters( 'profiles_get_signup_blog_url_value', $value );
	}
 ...
 * @since 2.1.0
 */
function profiles_signup_subdomain_base() {
	echo profiles_signup_get_subdomain_base();
}
	/**
	 * Return the base URL for subdomain installations of WordPress Multisite.
	 *
	 * Replaces profiles_blogs_get_subdomain_base()
	 *
	 * @since 2.1.0
	 *
	 * @return string The base URL - eg, 'example.com' for site_url() example.com or www.example.com.
	 */
	function profiles_signup_get_subdomain_base() {
		global $current_site;

		// In case plugins are still using this filter.
		$subdomain_base = apply_filters( 'profiles_blogs_subdomain_base', preg_replace( '|^www\.|', '', $current_site->domain ) . $current_site->path );
		 ...
		 */
		return apply_filters( 'profiles_signup_subdomain_base', $subdomain_base );
	}
 ...
 * @since 1.1.0
 */
function profiles_signup_blog_title_value() {
	echo profiles_get_signup_blog_title_value();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_signup_blog_title_value() {
		$value = '';
		 ...
		 */
		return apply_filters( 'profiles_get_signup_blog_title_value', $value );
	}
 ...
 * @since 1.1.0
 */
function profiles_signup_blog_privacy_value() {
	echo profiles_get_signup_blog_privacy_value();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_signup_blog_privacy_value() {
		$value = '';
		 ...
		 */
		return apply_filters( 'profiles_get_signup_blog_privacy_value', $value );
	}
 ...
 * @since 1.1.0
 */
function profiles_signup_avatar_dir_value() {
	echo profiles_get_signup_avatar_dir_value();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_signup_avatar_dir_value() {
		$profiles = profiles();
		 ...
		 */
		return apply_filters( 'profiles_get_signup_avatar_dir_value', $signup_avatar_dir );
	}
 ...
 * @since 1.1.0
 */
function profiles_current_signup_step() {
	echo profiles_get_current_signup_step();
}
	/**
	 ...
	 * @return string
	 */
	function profiles_get_current_signup_step() {
 ...
 * @since 1.1.0
 *
 * @see profiles_get_signup_avatar() for description of arguments.
 *
 * @param array|string $args See {@link profiles_get_signup_avatar(}.
 */
function profiles_signup_avatar( $args = '' ) {
	echo profiles_get_signup_avatar( $args );
}
	/**
	 ...
	 * @since 1.1.0
	 *
	 * @see profiles_core_fetch_avatar() for description of arguments.
	 *
	 * @param array|string $args {
	 *     Array of optional arguments.
	 *     @type int    $size  Height/weight in pixels. Default: value of
	 *                         profiles_core_avatar_full_width().
	 *     @type string $class CSS class. Default: 'avatar'.
	 *     @type string $alt   HTML 'alt' attribute. Default: 'Your Avatar'.
	 * }
	 * @return string
	 */
	function profiles_get_signup_avatar( $args = '' ) {
		$profiles = profiles();

		$defaults = array(
			'size' => profiles_core_avatar_full_width(),
			'class' => 'avatar',
		...
		// Avatar DIR is found.
		if ( $signup_avatar_dir = profiles_get_signup_avatar_dir_value() ) {
			$gravatar_img = profiles_core_fetch_avatar( array(
			...
			elseif ( 'mystery' == $profiles->grav_default->user )
				$default_grav = $profiles->plugin_url . 'profiles-core/images/mystery-man.jpg';
			else
			 ...
			 */
			$gravatar_url    = apply_filters( 'profiles_gravatar_url', '//www.gravatar.com/avatar/' );
			$md5_lcase_email = md5( strtolower( profiles_get_signup_email_value() ) );
		 ...
		 */
		return apply_filters( 'profiles_get_signup_avatar', $gravatar_img, $args );
	}

/**
 * Output a link to a members component suprofilesage.
 *
 * @since 1.5.0
 *
 * @see profiles_get_members_component_link() for description of parameters.
 *
 * @param string      $component See {@profiles_get_members_component_link()}.
 * @param string      $action See {@profiles_get_members_component_link()}.
 * @param string      $query_args See {@profiles_get_members_component_link()}.
 * @param string|bool $nonce See {@profiles_get_members_component_link()}.
 */
function profiles_members_component_link( $component, $action = '', $query_args = '', $nonce = false ) {
	echo esc_url( profiles_get_members_component_link( $component, $action, $query_args, $nonce ) );
}
	/**
	 * Generate a link to a members component suprofilesage.
	 *
	 * @since 1.5.0
	 *
	 ...
	 * @return string
	 */
	function profiles_get_members_component_link( $component, $action = '', $query_args = '', $nonce = false ) {
		...
		if ( !empty( $action ) )
			$url = profiles_displayed_user_domain() . $profiles->{$component}->slug . '/' . $action;
		else
			$url = profiles_displayed_user_domain() . $profiles->{$component}->slug;
