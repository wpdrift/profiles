...
if ( !class_exists( 'Profiles_Component' ) ) :
...
class Profiles_Component {
	 ...
	 *                     __( 'Groups', 'profiles' ).
	 * @param string $path The file path for the component's files. Used by {@link Profiles_Component::includes()}.
	 * @param array  $params {
	 ...
	 *                                            would look for a file called "profiles-{$this->id}-awesome.php" and you could use
	 *                                            profiles_is_active( $this->id, 'awesome' ) to determine if the feature is active.
	...
	public function setup_globals( $args = array() ) {
		 ...
		 */
		$this->slug                  = apply_filters( 'profiles_' . $this->id . '_slug',                  $r['slug']                  );
		 ...
		 */
		$this->root_slug             = apply_filters( 'profiles_' . $this->id . '_root_slug',             $r['root_slug']             );
		 ...
		 */
		$this->has_directory         = apply_filters( 'profiles_' . $this->id . '_has_directory',         $r['has_directory']         );
		 ...
		 */
		$this->directory_title       = apply_filters( 'profiles_' . $this->id . '_directory_title',       $r['directory_title']         );
		 ...
		 */
		$this->search_string         = apply_filters( 'profiles_' . $this->id . '_search_string',         $r['search_string']         );
		 ...
		 */
		$this->notification_callback = apply_filters( 'profiles_' . $this->id . '_notification_callback', $r['notification_callback'] );
		...
		/**
		 * Fires at the end of the setup_globals method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_setup_globals' );
	}

	/**
	 * Include required files.
	 *
	 * Please note that, by default, this method is fired on the profiles_include
	 ...
	 * loaded at profiles_include with priority 10 (as recommended), will not be
	 ...
	 * called 'my_component'):
	 *   - ./actions
	 *   - ./profiles-my_component/actions
	 *   - ./profiles-my_component/profiles-my_component-actions.php
	 *
	 * @since 1.5.0
	 *
	 *
	...
	public function includes( $includes = array() ) {
		...
		if ( ! empty( $includes ) ) {
			...
			foreach ( (array) $includes as $file ) {
				...
				$paths = array(
					...
					'profiles-' . $this->id . '/profiles-' . $this->id . '-' . $file  . '.php',
					'profiles-' . $this->id . '-' . $file . '.php',
					'profiles-' . $this->id . '/' . $file . '.php',

					// Passed with extension.
					$file,
					'profiles-' . $this->id . '-' . $file,
					'profiles-' . $this->id . '/' . $file,
				);
		...
		/**
		 * Fires at the end of the includes method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_includes' );
	}
	...
	public function setup_actions() {
		...
		// Setup globals.
		add_action( 'profiles_setup_globals',          array( $this, 'setup_globals'          ), 10 );

		// Set up canonical stack.
		add_action( 'profiles_setup_canonical_stack',  array( $this, 'setup_canonical_stack'  ), 10 );
		...
		// to profiles_include with the default priority of 10. This is for backwards
		// compatibility; henceforth, plugins should register themselves by
		// extending this base class.
		add_action( 'profiles_include',                array( $this, 'includes'               ), 8 );

		// Setup navigation.
		add_action( 'profiles_setup_nav',              array( $this, 'setup_nav'              ), 10 );

		// Setup WP Toolbar menus.
		add_action( 'profiles_setup_admin_bar',        array( $this, 'setup_admin_bar'        ), $this->adminbar_myaccount_order );

		// Setup component title.
		add_action( 'profiles_setup_title',            array( $this, 'setup_title'            ), 10 );

		// Setup cache groups.
		add_action( 'profiles_setup_cache_groups',     array( $this, 'setup_cache_groups'     ), 10 );

		// Register post types.
		add_action( 'profiles_register_post_types',    array( $this, 'register_post_types'    ), 10 );

		// Register taxonomies.
		add_action( 'profiles_register_taxonomies',    array( $this, 'register_taxonomies'    ), 10 );

		// Add the rewrite tags.
		add_action( 'profiles_add_rewrite_tags',       array( $this, 'add_rewrite_tags'       ), 10 );

		// Add the rewrite rules.
		add_action( 'profiles_add_rewrite_rules',      array( $this, 'add_rewrite_rules'      ), 10 );

		// Add the permalink structure.
		add_action( 'profiles_add_permastructs',       array( $this, 'add_permastructs'       ), 10 );

		// Allow components to parse the main query.
		add_action( 'profiles_parse_query',            array( $this, 'parse_query'            ), 10 );

		// Generate rewrite rules.
		add_action( 'profiles_generate_rewrite_rules', array( $this, 'generate_rewrite_rules' ), 10 );

		/**
		 * Fires at the end of the setup_actions method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_setup_actions' );
	}
	 ...
	 * @since 1.5.0
	 *
	 * @see profiles_core_new_nav_item() For a description of the $main_nav
	 *      parameter formatting.
	 * @see profiles_core_new_subnav_item() For a description of how each item
	 *      in the $sub_nav parameter array should be formatted.
	 *
	 * @param array $main_nav Optional. Passed directly to profiles_core_new_nav_item().
	 *                        See that function for a description.
	 * @param array $sub_nav  Optional. Multidimensional array, each item in
	 *                        which is passed to profiles_core_new_subnav_item(). See that
	...
	public function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		if ( !empty( $main_nav ) ) {
			profiles_core_new_nav_item( $main_nav, 'members' );

			// Sub nav items are not required.
			if ( !empty( $sub_nav ) ) {
				foreach( (array) $sub_nav as $nav ) {
					profiles_core_new_subnav_item( $nav, 'members' );
				}
			}
		}

		/**
		 * Fires at the end of the setup_nav method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_setup_nav' );
	}
	...
	public function setup_admin_bar( $wp_admin_nav = array() ) {
		...
		// Do not proceed if Profiles_USE_WP_ADMIN_BAR constant is not set or is false.
		 ...
		 */
		$wp_admin_nav = apply_filters( 'profiles_' . $this->id . '_admin_nav', $wp_admin_nav );
		...
		if ( !empty( $wp_admin_nav ) ) {
			...
			$wp_admin_nav = profiles_sort_by_key( $wp_admin_nav, 'position', 'num' );
		...
		/**
		 * Fires at the end of the setup_admin_bar method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_setup_admin_bar' );
	}
	...
	public function setup_title() {
		...
		/**
		 * Fires in the setup_title method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action(  'profiles_' . $this->id . '_setup_title' );
	}
	...
	public function setup_cache_groups() {
		...
		/**
		 * Fires in the setup_cache_groups method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 2.2.0
		 */
		do_action( 'profiles_' . $this->id . '_setup_cache_groups' );
	}
	...
	public function register_global_tables( $tables = array() ) {
		 ...
		 * *all* tables, use the 'profiles_core_get_table_prefix' filter instead.
		 *
		 * @since 1.6.0
		 */
		$tables = apply_filters( 'profiles_' . $this->id . '_global_tables', $tables );
		...
		/**
		 * Fires at the end of the register_global_tables method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 2.0.0
		 */
		do_action( 'profiles_' . $this->id . '_register_global_tables' );
	}
	...
	public function register_meta_tables( $tables = array() ) {
		 ...
		 * @since 2.0.0
		 */
		$tables = apply_filters( 'profiles_' . $this->id . '_meta_tables', $tables );
		...
		/**
		 * Fires at the end of the register_meta_tables method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 2.0.0
		 */
		do_action( 'profiles_' . $this->id . '_register_meta_tables' );
	}
	...
	public function register_post_types() {
		...
		/**
		 * Fires in the register_post_types method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_register_post_types' );
	}
	...
	public function register_taxonomies() {
		...
		/**
		 * Fires in the register_taxonomies method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_register_taxonomies' );
	}
	...
	public function add_rewrite_tags() {
		...
		/**
		 * Fires in the add_rewrite_tags method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_add_rewrite_tags' );
	}
	...
	public function add_rewrite_rules() {
		...
		/**
		 * Fires in the add_rewrite_rules method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.9.0
		 */
		do_action( 'profiles_' . $this->id . '_add_rewrite_rules' );
	}
	...
	public function add_permastructs() {
		...
		/**
		 * Fires in the add_permastructs method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.9.0
		 */
		do_action( 'profiles_' . $this->id . '_add_permastructs' );
	}
	...
	public function parse_query( $query ) {
		...
		/**
		 * Fires in the parse_query method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.9.0
		 ...
		 */
		do_action_ref_array( 'profiles_' . $this->id . '_parse_query', array( &$query ) );
	}
	...
	public function generate_rewrite_rules() {
		...
		/**
		 * Fires in the generate_rewrite_rules method inside Profiles_Component.
		 *
		 * This is a dynamic hook that is based on the component string ID.
		 *
		 * @since 1.5.0
		 */
		do_action( 'profiles_' . $this->id . '_generate_rewrite_rules' );
	}
}
endif; // Profiles_Component.
