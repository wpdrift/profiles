 ...
 * @since 2.5.0
 */
class Profiles_Email_Recipient {
	...
	public function __construct( $email_or_user, $name = '' ) {
		...
		if ( $this->user_object ) {
			// This is escaped with esc_html in profiles_core_get_user_displayname()
			$wp_name = wp_specialchars_decode( profiles_core_get_user_displayname( $this->user_object->ID ), ENT_QUOTES );
		...
		/**
		 * Fires inside __construct() method for Profiles_Email_Recipient class.
		 *
		 * @since 2.5.0
		 *
		 * @param string|array|int|WP_User $email_or_user Either a email address, user ID, WP_User object,
		 ...
		 * @param Profiles_Email_Recipient $this Current instance of the email type class.
		 */
		do_action( 'profiles_email_recipient', $email_or_user, $name, $this );
	}
	...
	public function get_address() {
		 ...
		 * @param Profiles_Email $recipient $this Current instance of the email recipient class.
		 */
		return apply_filters( 'profiles_email_recipient_get_address', $this->address, $this );
	}
	...
	public function get_name() {
		 ...
		 * @param Profiles_Email $recipient $this Current instance of the email recipient class.
		 */
		return apply_filters( 'profiles_email_recipient_get_name', $this->name, $this );
	}
	...
	public function get_user( $transform = 'raw' ) {
		 ...
		 * @param Profiles_Email $recipient $this Current instance of the email recipient class.
		 */
		return apply_filters( 'profiles_email_recipient_get_user', $this->user_object, $transform, $this );
	}
}
