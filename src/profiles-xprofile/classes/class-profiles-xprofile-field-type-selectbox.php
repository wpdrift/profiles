 ...
 * @since 2.0.0
 */
class Profiles_XProfile_Field_Type_Selectbox extends Profiles_XProfile_Field_Type {
	...
	public function __construct() {
		...
		$this->name     = _x( 'Drop Down Select Box', 'xprofile field type', 'profiles' );
		...
		/**
		 * Fires inside __construct() method for Profiles_XProfile_Field_Type_Selectbox class.
		 *
		 * @since 2.0.0
		 *
		 * @param Profiles_XProfile_Field_Type_Selectbox $this Current instance of
		 *                                               the field type select box.
		 */
		do_action( 'profiles_xprofile_field_type_selectbox', $this );
	}
	...
	public function edit_field_html( array $raw_properties = array() ) {
		...
		<select <?php echo $this->get_edit_field_html_elements( $raw_properties ); ?>>
			<?php profiles_the_profile_field_options( array( 'user_id' => $user_id ) ); ?>
		</select>
	...
	public function edit_field_options_html( array $args = array() ) {
		...
		$html    = '<option value="">' . /* translators: no option picked in select box */ esc_html__( '----', 'profiles' ) . '</option>';
		...
		for ( $k = 0, $count = count( $options ); $k < $count; ++$k ) {
			 ...
			 */
			$html .= apply_filters( 'profiles_get_the_profile_field_options_select', '<option' . $selected . ' value="' . esc_attr( stripslashes( $options[$k]->name ) ) . '">' . esc_html( stripslashes( $options[$k]->name ) ) . '</option>', $options[$k], $this->field_obj->id, $selected, $k );
		}
