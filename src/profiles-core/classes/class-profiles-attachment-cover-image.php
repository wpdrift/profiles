 ...
 * @since 2.4.0
 */
class Profiles_Attachment_Cover_Image extends Profiles_Attachment {
	/**
	 * The constuctor.
	 *
	 * @since 2.4.0
	...
	public function __construct() {
		...
		$allowed_types        = profiles_attachments_get_allowed_types( 'cover_image' );
		$max_upload_file_size = profiles_attachments_get_max_upload_file_size( 'cover_image' );

		parent::__construct( array(
			'action'                => 'profiles_cover_image_upload',
			'file_input'            => 'file',
			'original_max_filesize' => $max_upload_file_size,
			'base_dir'              => profiles_attachments_uploads_dir_get( 'dir' ),
			'required_wp_files'     => array( 'file', 'image' ),

			// Specific errors for cover images.
			'upload_error_strings'  => array(
				11  => sprintf( __( 'That image is too big. Please upload one smaller than %s', 'profiles' ), size_format( $max_upload_file_size ) ),
				12  => sprintf( _n( 'Please upload only this file type: %s.', 'Please upload only these file types: %s.', count( $allowed_types ), 'profiles' ), self::get_cover_image_types( $allowed_types ) ),
			),
		) );
	}
	...
	public static function get_cover_image_types( $allowed_types = array() ) {
		...
		$comma = _x( ',', 'cover image types separator', 'profiles' );
	...
	public function validate_upload( $file = array() ) {
		...
		} elseif ( ! profiles_attachments_check_filetype( $file['tmp_name'], $file['name'], profiles_attachments_get_allowed_mimes( 'cover_image' ) ) ) {
			$file['error'] = 12;
		}
	...
	public function upload_dir_filter( $upload_dir = array() ) {
		...
		$object_id = profiles_displayed_user_id();

		if ( empty( $object_id ) ) {
			$object_id = profiles_loggedin_user_id();
		}

		$object_directory = 'members';

		// We're in a group, edit default values.
		if ( profiles_is_group() || profiles_is_group_create() ) {
			$object_id        = profiles_get_current_group_id();
		 ...
		 * @param array $upload_dir The original Uploads dir.
		 */
		return apply_filters( 'profiles_attachments_cover_image_upload_dir', array(
	...
	public function generate_filename( $file = '' ) {
		...
		$name = wp_unique_filename( $info['dirname'], uniqid() . "-profiles-cover-image.$ext" );
	...
	public function script_data() {
		...
		if ( profiles_is_user() ) {
			...
			$script_data['profiles_params'] = array(
				'object'          => 'user',
				'item_id'         => $item_id,
				'has_cover_image' => profiles_attachments_get_user_has_cover_image( $item_id ),
				'nonces'  => array(
					'remove' => wp_create_nonce( 'profiles_delete_cover_image' ),
				),
			);

			// Set feedback messages.
			$script_data['feedback_messages'] = array(
				1 => __( 'Your new cover image was uploaded successfully.', 'profiles' ),
				2 => __( 'There was a problem deleting your cover image. Please try again.', 'profiles' ),
				3 => __( 'Your cover image was deleted successfully!', 'profiles' ),
			);
		} elseif ( profiles_is_group() ) {
			$item_id = profiles_get_current_group_id();

			$script_data['profiles_params'] = array(
				'object'          => 'group',
				'item_id'         => profiles_get_current_group_id(),
				'has_cover_image' => profiles_attachments_get_group_has_cover_image( $item_id ),
				'nonces'  => array(
					'remove' => wp_create_nonce( 'profiles_delete_cover_image' ),
				),
			);
			...
			// Set feedback messages.
			$script_data['feedback_messages'] = array(
				1 => __( 'The group cover image was uploaded successfully.', 'profiles' ),
				2 => __( 'There was a problem deleting the group cover image. Please try again.', 'profiles' ),
				3 => __( 'The group cover image was deleted successfully!', 'profiles' ),
			);
		} else {
			 ...
			 */
			$script_data['profiles_params'] = apply_filters( 'profiles_attachment_cover_image_params', array() );
		}

		// Include our specific js & css.
		$script_data['extra_js']  = array( 'profiles-cover-image' );
		 ...
		 */
		return apply_filters( 'profiles_attachments_cover_image_script_data', $script_data );
	}
}
