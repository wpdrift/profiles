 ...
 *
 * Plupload 2.1.1 was introduced in WordPress 3.9. Our profiles-plupload.js
 ...
 */
function profiles_attachments_is_wp_version_supported() {
	return (bool) version_compare( profiles_get_major_wp_version(), '3.9', '>=' );
}
 ...
 */
function profiles_attachments_uploads_dir_get( $data = '' ) {
	...
	if ( 'dir' === $data ) {
		...
		$upload_data = profiles_upload_dir();
	 ...
	 */
	return apply_filters( 'profiles_attachments_uploads_dir_get', $retval, $data );
}
 ...
 */
function profiles_attachments_get_max_upload_file_size( $type = '' ) {
	$fileupload_maxk = profiles_core_get_root_option( 'fileupload_maxk' );
	 ...
	 */
	return apply_filters( 'profiles_attachments_get_max_upload_file_size', $fileupload_maxk, $type );
}
 ...
 */
function profiles_attachments_get_allowed_types( $type = 'avatar' ) {
	 ...
	 */
	return apply_filters( 'profiles_attachments_get_allowed_types', $exts, $type );
}
 ...
 */
function profiles_attachments_get_allowed_mimes( $type = '', $allowed_types = array() ) {
	if ( empty( $allowed_types ) ) {
		$allowed_types = profiles_attachments_get_allowed_types( $type );
	}
 ...
 */
function profiles_attachments_check_filetype( $file, $filename, $allowed_mimes ) {
 ...
 */
function profiles_attachments_create_item_type( $type = 'avatar', $args = array() ) {
	...
	$allowed_mimes = profiles_attachments_get_allowed_mimes( $type );
	...
	if ( 'avatar' === $type ) {
		...
		if ( empty( $r['crop_w'] ) ) {
			$r['crop_w'] = profiles_core_avatar_full_width();
		}

		// Set crop height for the avatar if not given.
		if ( empty( $r['crop_h'] ) ) {
			$r['crop_h'] = profiles_core_avatar_full_height();
		}
		...
		$attachment_data = profiles_attachments_uploads_dir_get();
	...
	if ( 'avatar' === $type ) {
		$created = profiles_core_avatar_handle_crop( array(
			'object'        => $r['object'],
	...
	// It's a cover image we need to fit it to feature's dimensions.
		...
		$cover_image = profiles_attachments_cover_image_generate_file( array(
 ...
 */
function profiles_attachments_get_attachment( $data = 'url', $args = array() ) {
	// Default value.
	 ...
	 * }
	 */
	$pre_filter = apply_filters( 'profiles_attachments_pre_get_attachment', null, $r );
	if ( $pre_filter !== null ) {
		return $pre_filter;
	}

	// Get Profiles Attachments Uploads Dir datas.
	$profiles_attachments_uploads_dir = profiles_attachments_uploads_dir_get();
 ...
 * @since 2.4.0
 *
 * @see profiles_attachments_get_attachment() For more information on accepted arguments.
 *
 ...
 */
function profiles_attachments_delete_file( $args = array() ) {
	$attachment_path = profiles_attachments_get_attachment( 'path', $args );
	 ...
	 */
	if ( ! apply_filters( 'profiles_attachments_pre_delete_file', true, $args ) ) {
		return true;
	}
 ...
 */
function profiles_attachments_get_plupload_default_settings() {
	...
	$defaults = array(
		...
		'multipart_params'    => array(
			'action'          => 'profiles_upload_attachment',
			'_wpnonce'        => wp_create_nonce( 'profiles-uploader' ),
		),
	 ...
	 */
	return apply_filters( 'profiles_attachments_get_plupload_default_settings', $settings );
}
 ...
 */
function profiles_attachments_get_plupload_l10n() {
	// Localization strings.
	return apply_filters( 'profiles_attachments_get_plupload_l10n', array(
 ...
 * @since 2.3.0
 *
 * @see Profiles_Attachment::script_data() && Profiles_Attachment_Avatar::script_data() for examples showing how
 * to set specific script data.
 *
 * @param string $class Name of the class extending Profiles_Attachment (eg: Profiles_Attachment_Avatar).
 * @return null|WP_Error
 */
function profiles_attachments_enqueue_scripts( $class = '' ) {
	// Enqueue me just once per page, please.
	if ( did_action( 'profiles_attachments_enqueue_scripts' ) ) {
		return;
	}
	...
	$args = profiles_parse_args( $script_data, array(
		'action'            => '',
		'file_data_name'    => '',
		'max_file_size'     => 0,
		'browse_button'     => 'profiles-browse-button',
		'container'         => 'profiles-upload-ui',
	...
	$strings = profiles_attachments_get_plupload_l10n();

	// Get the Profiles uploader settings.
	$settings = profiles_attachments_get_plupload_default_settings();
	...
	if ( ! empty( $args['profiles_params'] ) ) {
		$defaults['multipart_params']['profiles_params'] = $args['profiles_params'];
	}
	...
	if ( 'profiles_avatar_upload' === $defaults['multipart_params']['action'] ) {

		// Include the cropping informations for avatars.
		$settings['crop'] = array(
			'full_h'  => profiles_core_avatar_full_height(),
			'full_w'  => profiles_core_avatar_full_width(),
		);
		...
		$has_avatar = $defaults['multipart_params']['profiles_params']['has_avatar'];

		// What is the object the avatar belongs to.
		$object = $defaults['multipart_params']['profiles_params']['object'];
		...
		if ( profiles_avatar_use_webcam() && 'user' === $object ) {
		 ...
		 */
		$settings['nav'] = profiles_sort_by_key( apply_filters( 'profiles_attachments_avatar_nav', $avatar_nav, $object ), 'order', 'num' );

	// Specific to Profiles cover images.
	} elseif ( 'profiles_cover_image_upload' === $defaults['multipart_params']['action'] ) {
		...
		if ( 'group' === $object ) {
			...
			$cover_component = apply_filters( 'profiles_attachments_cover_image_ui_component', $cover_component );
		}
		// Get cover image advised dimensions.
		$cover_dimensions = profiles_attachments_get_cover_image_dimensions( $cover_component );

		// Set warning messages.
		$strings['cover_image_warnings'] = apply_filters( 'profiles_attachments_cover_image_ui_warnings', array(
			'dimensions'  => sprintf(
	...
	wp_enqueue_script ( 'profiles-plupload' );
	wp_localize_script( 'profiles-plupload', 'Profiles_Uploader', array( 'strings' => $strings, 'settings' => $settings ) );
	...
	/**
	 * Fires at the conclusion of profiles_attachments_enqueue_scripts()
	 * to avoid the scripts to be loaded more than once.
	 *
	 * @since 2.3.0
	 */
	do_action( 'profiles_attachments_enqueue_scripts' );
}
 ...
 * @return bool
 */
function profiles_attachments_current_user_can( $capability, $args = array() ) {
	$can = false;
	...
	if ( 'edit_avatar' === $capability || 'edit_cover_image' === $capability ) {
		...
		if ( isset( $args['item_id'] ) && isset( $args['object'] ) ) {
			// Group profile photo.
			if ( profiles_is_active( 'groups' ) && 'group' === $args['object'] ) {
				if ( profiles_is_group_create() ) {
					$can = (bool) groups_is_user_creator( profiles_loggedin_user_id(), $args['item_id'] ) || profiles_current_user_can( 'profiles_moderate' );
				} else {
					$can = (bool) groups_is_user_admin( profiles_loggedin_user_id(), $args['item_id'] ) || profiles_current_user_can( 'profiles_moderate' );
				}
			// User profile photo.
			} elseif ( profiles_is_active( 'xprofile' ) && 'user' === $args['object'] ) {
				$can = profiles_loggedin_user_id() === (int) $args['item_id'] || profiles_current_user_can( 'profiles_moderate' );
			}
		/**
		 * No avatar arguments, fallback to profiles_user_can_create_groups()
		 * or profiles_is_item_admin()
		 */
		} else {
			if ( profiles_is_group_create() ) {
				$can = profiles_user_can_create_groups();
			} else {
				$can = profiles_is_item_admin();
			}
		}
	}

	return apply_filters( 'profiles_attachments_current_user_can', $can, $capability, $args );
}
 ...
 */
function profiles_attachments_json_response( $success, $is_html4 = false, $data = null ) {
 ...
 * @return bool
 */
function profiles_attachments_get_template_part( $slug ) {
	$attachment_template_part = 'assets/_attachments/' . $slug;

	// Load the attachment template in WP Administration screens.
	if ( is_admin() && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) ) {
		$attachment_admin_template_part = profiles()->themes_dir . '/profiles-legacy/profiles/' . $attachment_template_part . '.php';
	...
	// Load the attachment template in WP_USE_THEMES env.
		...
		profiles_get_template_part( $attachment_template_part );
	}
}
 ...
 */
function profiles_attachments_get_cover_image_settings( $component = 'xprofile' ) {
	// Default parameters.
	$args = array();

	// First look in BP Theme Compat.
	$cover_image = profiles_get_theme_compat_feature( 'cover_image' );
	 ...
	 * add_filter( 'profiles_before_xprofile_cover_image_settings_parse_args', 'your_filter', 10, 1 );
	 *
	 * @since 2.4.0
	 *
	 * @param array $settings The cover image settings
	 */
	$settings = profiles_parse_args( $args, array(
		'components'    => array(),
		'width'         => 1300,
		'height'        => 225,
		'callback'      => '',
 ...
 */
function profiles_attachments_get_cover_image_dimensions( $component = 'xprofile' ) {
	// Let's prevent notices when setting the warning strings.
	$default = array( 'width' => 0, 'height' => 0 );

	$settings = profiles_attachments_get_cover_image_settings( $component );
	 ...
	 */
	return apply_filters( 'profiles_attachments_get_cover_image_dimensions', $wh, $settings, $component );
}
 ...
 */
function profiles_attachments_cover_image_is_edit() {
	$retval = false;

	$current_component = profiles_current_component();
	if ( profiles_is_active( 'xprofile' ) && profiles_is_current_component( 'xprofile' ) ) {
		$current_component = 'xprofile';
	}

	if ( ! profiles_is_active( $current_component, 'cover_image' ) ) {
		return $retval;
	}

	if ( profiles_is_user_change_cover_image() ) {
		$retval = ! profiles_disable_cover_image_uploads();
	}

	if ( ( profiles_is_group_admin_page() && 'group-cover-image' == profiles_get_group_current_admin_tab() )
		|| ( profiles_is_group_create() && profiles_is_group_creation_step( 'group-cover-image' ) ) ) {
		$retval = ! profiles_disable_group_cover_image_uploads();
	}

	return apply_filters( 'profiles_attachments_cover_image_is_edit', $retval, $current_component );
}
 ...
 */
function profiles_attachments_get_user_has_cover_image( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
		$user_id = profiles_displayed_user_id();
	}

	$cover_src = profiles_attachments_get_attachment( 'url', array(
		'item_id'   => $user_id,
	) );

	return (bool) apply_filters( 'profiles_attachments_get_user_has_cover_image', $cover_src, $user_id );
}
 ...
 */
function profiles_attachments_get_group_has_cover_image( $group_id = 0 ) {
	if ( empty( $group_id ) ) {
	...
	return (bool) apply_filters( 'profiles_attachments_get_user_has_cover_image', $cover_src, $group_id );
}
 ...
 * }
 * @param Profiles_Attachment_Cover_Image|null $cover_image_class The class to use to fit the cover image.
 * @return bool|array An array containing cover image data on success, false otherwise.
 */
function profiles_attachments_cover_image_generate_file( $args = array(), $cover_image_class = null ) {
	...
	$dimensions = profiles_attachments_get_cover_image_dimensions( $args['component'] );
	...
	if ( ! is_a( $cover_image_class, 'Profiles_Attachment_Cover_Image' ) ) {
		$cover_image_class = new Profiles_Attachment_Cover_Image();
	}
 ...
 */
function profiles_attachments_cover_image_ajax_upload() {
	...
	if ( ! empty( $_POST['profiles_params'] ) ) {
		$profiles_params = profiles_parse_args( $_POST['profiles_params'], array(
			'object'  => 'user',
			'item_id' => profiles_loggedin_user_id(),
	...
	// Capability check.
	if ( ! profiles_attachments_current_user_can( 'edit_cover_image', $profiles_params ) ) {
		profiles_attachments_json_response( false, $is_html4 );
	}
	...
	// Other object's cover image.
		...
		$object_data = apply_filters( 'profiles_attachments_cover_image_object_dir', array(), $profiles_params['object'] );
	}
	 ...
	 */
	$pre_filter = apply_filters( 'profiles_attachments_pre_cover_image_ajax_upload', array(), $profiles_params, $needs_reset, $object_data );
	if ( isset( $pre_filter['result'] ) ) {
		profiles_attachments_json_response( $pre_filter['result'], $is_html4, $pre_filter );
	}

	$cover_image_attachment = new Profiles_Attachment_Cover_Image();
	 ...
	 * the same Ajax request, as we already instantiated the Profiles_Attachment_Cover_Image
	 * class, let's use it.
	 */
	$cover = profiles_attachments_cover_image_generate_file( array(
		'file'            => $uploaded['file'],
...
add_action( 'wp_ajax_profiles_cover_image_upload', 'profiles_attachments_cover_image_ajax_upload' );
 ...
 *                     error message otherwise.
 */
function profiles_attachments_cover_image_ajax_delete() {
	// Bail if not a POST action.
	if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) ) {
		wp_send_json_error();
	}
	...
	// Check the nonce.
	check_admin_referer( 'profiles_delete_cover_image', 'nonce' );

	// Capability check.
	if ( ! profiles_attachments_current_user_can( 'edit_cover_image', $cover_image_data ) ) {
		wp_send_json_error();
	}
	...
	// Handle delete.
	if ( profiles_attachments_delete_file( array( 'item_id' => $cover_image_data['item_id'], 'object_dir' => $dir, 'type' => 'cover-image' ) ) ) {
		...
		$cover_params = profiles_attachments_get_cover_image_settings( $component );
...
add_action( 'wp_ajax_profiles_cover_image_delete', 'profiles_attachments_cover_image_ajax_delete' );
