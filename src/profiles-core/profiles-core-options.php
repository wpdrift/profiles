 ...
 */
function profiles_get_default_options() {
	...
	$options = array (
		...
		'profiles-deactivated-components'            => array(),
		...
		'profiles-xprofile-base-group-name'          => 'Base',

		// Base fullname field name.
		'profiles-xprofile-fullname-field-name'      => 'Name',

		/* Blogs *************************************************************/

		// Used to decide if blogs need indexing.
		'profiles-blogs-first-install'               => false,

		/* Settings **********************************************************/

		// Disable the WP to BP profile sync.
		'profiles-disable-profile-sync'              => false,

		// Hide the Toolbar for logged out users.
		'hide-loggedout-adminbar'              => false,

		// Avatar uploads.
		'profiles-disable-avatar-uploads'            => false,

		// Cover image uploads.
		'profiles-disable-cover-image-uploads'       => false,

		// Group Profile Photos.
		'profiles-disable-group-avatar-uploads'      => false,

		// Group Cover image uploads.
		'profiles-disable-group-cover-image-uploads' => false,

		// Allow users to delete their own accounts.
		'profiles-disable-account-deletion'          => false,

		// Allow comments on blog and forum activity items.
		'profiles-disable-blogforum-comments'        => true,

		// The ID for the current theme package.
		'_profiles_theme_package_id'                 => 'legacy',

		// Email unsubscribe salt.
		'profiles-emails-unsubscribe-salt'           => base64_encode( wp_generate_password( 64, true, true ) ),

		/* Groups ************************************************************/

		// @todo Move this into the groups component
		// Restrict group creation to super admins.
		'profiles_restrict_group_creation'           => false,

		/* Akismet ***********************************************************/

		// Users from all sites can post.
		'_profiles_enable_akismet'                   => true,

		/* Activity HeartBeat ************************************************/

		// HeartBeat is on to refresh activities.
		'_profiles_enable_heartbeat_refresh'         => true,

		/* BuddyBar **********************************************************/

		// Force the BuddyBar.
		'_profiles_force_buddybar'                   => false,

		/* Legacy theme *********************************************/

		// Whether to register the profiles-default themes directory.
		'_profiles_retain_profiles_default'                => false,

		/* Widgets **************************************************/
		'widget_profiles_core_login_widget'                => false,
		'widget_profiles_core_members_widget'              => false,
		'widget_profiles_core_whos_online_widget'          => false,
		'widget_profiles_core_recently_active_widget'      => false,
		'widget_profiles_groups_widget'                    => false,
		'widget_profiles_messages_sitewide_notices_widget' => false,
	);
	 ...
	 */
	return apply_filters( 'profiles_get_default_options', $options );
}
 ...
 * @since 1.6.0
 */
function profiles_add_options() {

	// Get the default options and values.
	$options = profiles_get_default_options();

	// Add default options.
	foreach ( $options as $key => $value ) {
		profiles_add_option( $key, $value );
	}
	 ...
	 * @since 1.6.0
	 */
	do_action( 'profiles_add_options' );
}

/**
 * Delete default options.
 *
 * Hooked to profiles_uninstall, it is only called once when Profiles is uninstalled.
 ...
 * @since 1.6.0
 */
function profiles_delete_options() {
	 ...
	 * @since 1.6.0
	 */
	do_action( 'profiles_delete_options' );
}
 ...
 * @since 1.6.0
 */
function profiles_setup_option_filters() {
	...
	foreach ( array_keys( $options ) as $key ) {
		add_filter( 'pre_option_' . $key, 'profiles_pre_get_option' );
	}
	 ...
	 * @since 1.6.0
	 */
	do_action( 'profiles_setup_option_filters' );
}
 ...
 */
function profiles_pre_get_option( $value = false ) {
	$profiles = profiles();
 ...
 * (such as profiles-pages) on the appropriate blog, given your current setup.
 *
 * The 'profiles_get_option' filter is primarily for backward-compatibility.
 *
 * @since 1.2.0
 *
 ...
 */
function profiles_get_option( $option_name, $default = '' ) {
	$value = get_blog_option( profiles_get_root_blog_id(), $option_name, $default );
	 ...
	 */
	return apply_filters( 'profiles_get_option', $value );
}
 ...
 */
function profiles_add_option( $option_name, $value ) {
	return add_blog_option( profiles_get_root_blog_id(), $option_name, $value );
}
 ...
 * settings data (such as profiles-pages) on the appropriate blog, given your current
 * setup.
 *
 * @since 1.5.0
 *
 ...
 * @return bool True on success, false on failure.
 */
function profiles_update_option( $option_name, $value ) {
	return update_blog_option( profiles_get_root_blog_id(), $option_name, $value );
}
 ...
 * @return bool True on success, false on failure.
 */
function profiles_delete_option( $option_name ) {
	return delete_blog_option( profiles_get_root_blog_id(), $option_name );
}
 ...
 * @return bool
 */
function profiles_core_activate_site_options( $keys = array() ) {
	...
	if ( !empty( $keys ) && is_array( $keys ) ) {
		...
		foreach ( $keys as $key => $default ) {
			if ( empty( $profiles->site_options[ $key ] ) ) {
				$profiles->site_options[ $key ] = profiles_get_option( $key, $default );

				if ( !profiles_update_option( $key, $profiles->site_options[ $key ] ) ) {
					$errors = true;
				}
			}
		}
 ...
 */
function profiles_core_get_root_options() {
	global $wpdb;

	// Get all the Profiles settings, and a few useful WP ones too.
	$root_blog_options                   = profiles_get_default_options();
	...
	$root_blog_options_meta = wp_cache_get( 'root_blog_options', 'profiles' );

	if ( false === $root_blog_options_meta ) {
		$blog_options_keys      = "'" . join( "', '", (array) $root_blog_option_keys ) . "'";
		$blog_options_table	    = profiles_is_multiblog_mode() ? $wpdb->options : $wpdb->get_blog_prefix( profiles_get_root_blog_id() ) . 'options';
		...
		if ( is_multisite() ) {
			 ...
			 */
			$network_options = apply_filters( 'profiles_core_network_options', array(
				'tags_blog_id'       => '0',
				'sitewide_tags_blog' => '',
				'registration'       => '0',
		...
		if ( empty( $root_blog_options_meta ) || ( count( $root_blog_options_meta ) < count( $root_blog_option_keys ) ) ) {
			...
			foreach ( $root_blog_options as $old_meta_key => $old_meta_default ) {
				...
				if ( empty( $old_meta_value ) ) {
					$old_meta_value = profiles_get_option( $old_meta_key, $old_meta_default );
				}

				// Update the root blog option.
				profiles_update_option( $old_meta_key, $old_meta_value );
		...
		wp_cache_set( 'root_blog_options', $root_blog_options_meta, 'profiles' );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_root_options', $root_blog_options_meta );
}
 ...
 * See {@see profiles_core_get_root_options()}.
 *
 * @since 2.3.0
 *
 ...
 */
function profiles_core_get_root_option( $option ) {
	$profiles = profiles();
 ...
 */
function profiles_disable_profile_sync( $default = false ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_disable_profile_sync', (bool) profiles_get_option( 'profiles-disable-profile-sync', $default ) );
}
 ...
 *              otherwise false.
 */
function profiles_hide_loggedout_adminbar( $default = true ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_hide_loggedout_adminbar', (bool) profiles_get_option( 'hide-loggedout-adminbar', $default ) );
}
 ...
 */
function profiles_disable_avatar_uploads( $default = true ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_disable_avatar_uploads', (bool) profiles_get_option( 'profiles-disable-avatar-uploads', $default ) );
}
 ...
 */
function profiles_disable_cover_image_uploads( $default = false ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_disable_cover_image_uploads', (bool) profiles_get_option( 'profiles-disable-cover-image-uploads', $default ) );
}

/**
 * Are group avatars disabled?
 *
 * For backward compatibility, this option falls back on the value of 'profiles-disable-avatar-uploads' when no value is
 * found in the database.
 *
 * @since 2.3.0
 *
 * @param bool|null $default Optional. Fallback value if not found in the database.
 *                           Defaults to the value of `profiles_disable_avatar_uploads()`.
 * @return bool True if group avatar uploads are disabled, otherwise false.
 */
function profiles_disable_group_avatar_uploads( $default = null ) {
	$disabled = profiles_get_option( 'profiles-disable-group-avatar-uploads', '' );

	if ( '' === $disabled ) {
		if ( is_null( $default ) ) {
			$disabled = profiles_disable_avatar_uploads();
		} else {
			$disabled = $default;
		}
	}
	 ...
	 */
	return (bool) apply_filters( 'profiles_disable_group_avatar_uploads', $disabled, $default );
}
 ...
 */
function profiles_disable_group_cover_image_uploads( $default = false ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_disable_group_cover_image_uploads', (bool) profiles_get_option( 'profiles-disable-group-cover-image-uploads', $default ) );
}
 ...
 *              false.
 */
function profiles_disable_account_deletion( $default = false ) {
	 ...
	 */
	return apply_filters( 'profiles_disable_account_deletion', (bool) profiles_get_option( 'profiles-disable-account-deletion', $default ) );
}
 ...
 */
function profiles_disable_blogforum_comments( $default = false ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_disable_blogforum_comments', (bool) profiles_get_option( 'profiles-disable-blogforum-comments', $default ) );
}
 ...
 */
function profiles_restrict_group_creation( $default = true ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_restrict_group_creation', (bool) profiles_get_option( 'profiles_restrict_group_creation', $default ) );
}
 ...
 */
function profiles_force_buddybar( $default = true ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_force_buddybar', (bool) profiles_get_option( '_profiles_force_buddybar', $default ) );
}
 ...
 */
function profiles_group_forums_root_id( $default = '0' ) {
	echo profiles_get_group_forums_root_id( $default );
}
	/**
	 ...
	 */
	function profiles_get_group_forums_root_id( $default = '0' ) {
		 ...
		 */
		return (int) apply_filters( 'profiles_get_group_forums_root_id', (int) profiles_get_option( '_profiles_group_forums_root_id', $default ) );
	}
 ...
 */
function profiles_is_group_forums_active( $default = true ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_is_group_forums_active', (bool) profiles_get_option( '_profiles_enable_group_forums', $default ) );
}
 ...
 */
function profiles_is_akismet_active( $default = true ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_is_akismet_active', (bool) profiles_get_option( '_profiles_enable_akismet', $default ) );
}
 ...
 */
function profiles_is_activity_heartbeat_active( $default = true ) {
	 ...
	 */
	return (bool) apply_filters( 'profiles_is_activity_heartbeat_active', (bool) profiles_get_option( '_profiles_enable_heartbeat_refresh', $default ) );
}
 ...
 */
function profiles_get_theme_package_id( $default = 'legacy' ) {
	 ...
	 */
	return apply_filters( 'profiles_get_theme_package_id', profiles_get_option( '_profiles_theme_package_id', $default ) );
}
