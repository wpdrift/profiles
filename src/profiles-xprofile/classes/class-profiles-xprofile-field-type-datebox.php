 ...
 * @since 2.0.0
 */
class Profiles_XProfile_Field_Type_Datebox extends Profiles_XProfile_Field_Type {
	...
	public function __construct() {
		...
		/**
		 * Fires inside __construct() method for Profiles_XProfile_Field_Type_Datebox class.
		 *
		 * @since 2.0.0
		 *
		 * @param Profiles_XProfile_Field_Type_Datebox $this Current instance of
		 *                                             the field type datebox.
		 */
		do_action( 'profiles_xprofile_field_type_datebox', $this );
	}
	...
	public function edit_field_html( array $raw_properties = array() ) {
		...
		$day_r = profiles_parse_args( $raw_properties, array(
			'id'   => profiles_get_the_profile_field_input_name() . '_day',
			'name' => profiles_get_the_profile_field_input_name() . '_day'
		) );

		$month_r = profiles_parse_args( $raw_properties, array(
			'id'   => profiles_get_the_profile_field_input_name() . '_month',
			'name' => profiles_get_the_profile_field_input_name() . '_month'
		) );

		$year_r = profiles_parse_args( $raw_properties, array(
			'id'   => profiles_get_the_profile_field_input_name() . '_year',
			'name' => profiles_get_the_profile_field_input_name() . '_year'
		) ); ?>
		...
		<fieldset class="datebox">
			...
			<div class="input-options datebox-selects">
				 ...
				 * field ID. The hooks will be similar to `profiles_field_12_errors`
				...
				<label for="<?php profiles_the_profile_field_input_name(); ?>_day" class="<?php echo is_admin() ? 'screen-reader-text' : 'profiles-screen-reader-text' ;?>"><?php
				...
				<label for="<?php profiles_the_profile_field_input_name(); ?>_month" class="<?php echo is_admin() ? 'screen-reader-text' : 'profiles-screen-reader-text' ;?>"><?php
					/* translators: accessibility text */
				...
				<label for="<?php profiles_the_profile_field_input_name(); ?>_year" class="<?php echo is_admin() ? 'screen-reader-text' : 'profiles-screen-reader-text' ;?>"><?php
					/* translators: accessibility text */
	...
	public function edit_field_options_html( array $args = array() ) {
		...
		$date       = Profiles_XProfile_ProfileData::get_value_byid( $this->field_obj->id, $args['user_id'] );
		$day        = 0;
		$month      = 0;
		$year       = 0;
		$html       = '';
		 ...
		 */
		echo apply_filters( 'profiles_get_the_profile_field_datebox', $html, $args['type'], $day, $month, $year, $this->field_obj->id, $date );
	}
	...
	public function admin_field_html( array $raw_properties = array() ) {
		...
		<label for="<?php profiles_the_profile_field_input_name(); ?>_day" class="screen-reader-text"><?php
			/* translators: accessibility text */
			esc_html_e( 'Select day', 'profiles' );
		?></label>
		<select <?php echo $this->get_edit_field_html_elements( $day_r ); ?>>
			<?php profiles_the_profile_field_options( array( 'type' => 'day' ) ); ?>
		</select>

		<label for="<?php profiles_the_profile_field_input_name(); ?>_month" class="screen-reader-text"><?php
			/* translators: accessibility text */
			esc_html_e( 'Select month', 'profiles' );
		?></label>
		<select <?php echo $this->get_edit_field_html_elements( $month_r ); ?>>
			<?php profiles_the_profile_field_options( array( 'type' => 'month' ) ); ?>
		</select>

		<label for="<?php profiles_the_profile_field_input_name(); ?>_year" class="screen-reader-text"><?php
			/* translators: accessibility text */
			esc_html_e( 'Select year', 'profiles' );
		?></label>
		<select <?php echo $this->get_edit_field_html_elements( $year_r ); ?>>
			<?php profiles_the_profile_field_options( array( 'type' => 'year' ) ); ?>
		</select>
	 ...
	 * @return string Date formatted by profiles_format_time().
	 */
	public static function display_filter( $field_value, $field_id = '' ) {
		...
		return profiles_format_time( $field_value, true, false );
	}
}
