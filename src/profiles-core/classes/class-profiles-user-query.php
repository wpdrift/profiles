 ...
 *                                                  See {@see Profiles_XProfile_Query} for details.
 ...
 *     @type string            $count_total         Determines how Profiles_User_Query will do a count of total users matching
...
class Profiles_User_Query {
	 ...
	 * @since 1.7.0
	 *
	 * @param string|array|null $query See {@link Profiles_User_Query}.
	 */
	public function __construct( $query = null ) {
		...
		if ( ! empty( $this->query_vars_raw ) ) {
			...
			/**
			 * Fires before the construction of the Profiles_User_Query query.
			 *
			 * @since 1.7.0
			 *
			 * @param Profiles_User_Query $this Current instance of the Profiles_User_Query. Passed by reference.
			 */
			do_action_ref_array( 'profiles_pre_user_query_construct', array( &$this ) );
	 ...
	 *
	 * When extending Profiles_User_Query, you may need to use some of its
	 ...
	 * Profiles_User_Query::__construct() contains a fair amount of logic that
	 ...
	 * {@link Profiles_Group_Member_Query::setup_hooks()} for an example.
	 *
	 * @since 1.8.0
	 */
	...
	public function prepare_user_ids_query() {
		...
		switch ( $type ) {
			...
			// Filter 'profiles_user_query_online_interval' to modify the
			...
			case 'online' :
				 ...
				 */
				$sql['where'][] = $wpdb->prepare( "u.date_recorded >= DATE_SUB( UTC_TIMESTAMP(), INTERVAL %d MINUTE )", apply_filters( 'profiles_user_query_online_interval', 15 ) );
			...
			case 'popular' :
				...
				$sql['select']  = "SELECT u.{$this->uid_name} as id FROM {$this->uid_table} u";
				$sql['where'][] = $wpdb->prepare( "u.meta_key = %s", profiles_get_user_meta_key( 'total_friend_count' ) );
			...
			case 'alphabetical' :
				...
				//
				// @todo remove need for profiles_is_active() check.
				if ( ! profiles_disable_profile_sync() || ! profiles_is_active( 'xprofile' ) ) {
					$this->uid_name = 'ID';
				...
				// the xprofile table.
					...
					$sql['select']  = "SELECT u.{$this->uid_name} as id FROM {$this->uid_table} u";
					$sql['where'][] = $wpdb->prepare( "u.field_id = %d", profiles_xprofile_fullname_field_id() );
				...
				$sql['where'][] = "u.{$this->uid_name} IN ( SELECT ID FROM {$wpdb->users} WHERE " . profiles_core_get_status_sql( '' ) . " )";
		...
		// @todo remove need for profiles_is_active() check.
		if ( ! empty( $user_id ) && profiles_is_active( 'friends' ) ) {
		...
		// xprofile field matches happen in profiles_xprofile_profiles_user_query_search().
		if ( false !== $search_terms ) {
			$search_terms = profiles_esc_like( wp_kses_normalize_entities( $search_terms ) );
		 ...
		 * @param Profiles_User_Query $this Current Profiles_User_Query instance.
		 */
		$sql = apply_filters_ref_array( 'profiles_user_query_uid_clauses', array( $sql, &$this ) );
		...
		/**
		 * Fires before the Profiles_User_Query query is made.
		 *
		 * @since 1.7.0
		 *
		 * @param Profiles_User_Query $this Current Profiles_User_Query instance. Passed by reference.
		 */
		do_action_ref_array( 'profiles_pre_user_query', array( &$this ) );
	}
	...
	public function do_user_ids_query() {
		...
		if ( 'sql_calc_found_rows' == $this->query_vars['count_total'] ) {
			 ...
			 * @param Profiles_User_Query $this  Current Profiles_User_Query instance.
			 */
			$this->total_users = $wpdb->get_var( apply_filters( 'profiles_found_user_query', "SELECT FOUND_ROWS()", $this ) );
			...
			/** This filter is documented in profiles-core/classes/class-profiles-user-query.php */
			$this->total_users = $wpdb->get_var( apply_filters( 'profiles_found_user_query', "{$count_select} {$this->uid_clauses['where']}", $this ) );
		}
	}
	...
	public function do_wp_user_query() {
		 ...
		 * @param Profiles_User_Query $this  Current Profiles_User_Query instance.
		 */
		$wp_user_query = new WP_User_Query( apply_filters( 'profiles_wp_user_query_args', array(
		...
		foreach ( $wp_user_query->results as $u ) {
			wp_cache_set( 'profiles_core_userdata_' . $u->ID, $u, 'profiles' );
		}
	 ...
	 * ids passed to the class constructor. See {@link Profiles_Group_Member_Query}
	 * for an example.
	 *
	 * @since 1.8.0
	 *
	 ...
	 *
	 * Different components will hook into the 'profiles_user_query_populate_extras'
	...
	public function populate_extras() {
		 ...
		 *
		 * @see profiles_xprofile_filter_user_query_populate_extras()
		 * @see profiles_friends_filter_user_query_populate_extras()
		 *
		 * @since 1.7.0
		 *
		 * @param Profiles_User_Query $this         Current Profiles_User_Query instance.
		 ...
		 */
		do_action_ref_array( 'profiles_user_query_populate_extras', array( $this, $user_ids_sql ) );

		// Fetch last_active data from the activity table.
		$last_activities = Profiles_Core_User::get_last_activity( $this->user_ids );
		...
		// - friend count
		// - latest update.
		$total_friend_count_key = profiles_get_user_meta_key( 'total_friend_count' );
		$profiles_latest_update_key   = profiles_get_user_meta_key( 'profiles_latest_update'   );
	...
	protected function get_sql_clause_for_member_types( $member_types, $operator ) {
		...
		foreach ( $member_types as $mt ) {
			if ( profiles_get_member_type_object( $mt ) ) {
				$types[] = $mt;
			}
		}
		...
		$site_id  = profiles_get_taxonomy_term_site_id( 'profiles_member_type' );
		$switched = false;
