	...
	if ( ! empty( $max_links ) ) {
		...
		if ( $num_links >= $max_links ) {
			if ( 'bool' === $error_type ) {
				...
				return new WP_Error( 'bp_moderation_too_many_links', __( 'You have posted too many links', 'profiles' ) );
			}
		}
	}
			...
			foreach ( $_post as $post_data ) {
				...
				if ( preg_match( $pattern, $post_data ) ) {
					if ( 'bool' === $error_type ) {
						...
						return new WP_Error( 'bp_moderation_word_match', _x( 'You have posted an inappropriate word.', 'Comment moderation', 'profiles' ) );
					}
				}
			}
		}
		...
		foreach( $_post as $post_data ) {
			...
			if ( preg_match( $pattern, $post_data ) ) {
				if ( 'bool' === $error_type ) {
					...
					return new WP_Error( 'bp_moderation_blacklist_match', _x( 'You have posted an inappropriate word.', 'Comment blacklist', 'profiles' ) );
				}
			}
		}
	}
