 ...
 * @since 2.6.0
 */
abstract class Profiles_Core_oEmbed_Extension {
	 ...
	 *
	 * You'll want to return a boolean here. eg. profiles_is_single_activity().
	 *
	 * @since 2.6.0
	 *
	 * @return bool
	...
	protected function set_permalink() {
		$url = profiles_get_requested_url();

		// Remove querystring from profiles_get_requested_url().
		if ( false !== strpos( profiles_get_requested_url(), '?' ) ) {
			$url = substr( profiles_get_requested_url(), 0, strpos( profiles_get_requested_url(), '?' ) );
		}
	...
	protected function setup_hooks() {
		...
		add_action( 'profiles_embed_content', array( $this, 'inject_content' ) );
	...
	public function content_buffer_end( $name ) {
		...
		profiles_get_asset_template_part( 'embeds/header', profiles_current_component() );

		/**
		 * Inject Profiles embed content on this hook.
		 *
		 * You shouldn't really need to use this if you extend the
		 * {@link Profiles_oEmbed_Component} class.
		 *
		 * @since 2.6.0
		 */
		do_action( 'profiles_embed_content' );

		// Template part for our embed footer.
		profiles_get_asset_template_part( 'embeds/footer', profiles_current_component() );
	 ...
	 * @since 2.6.0
	 *
	 * @see profiles_activity_embed_rest_route_callback()
	 *
	...
	public function filter_embed_url( $retval ) {
		...
		// @see profiles_activity_embed_comments_button().
	...
	public function filter_embed_html( $retval ) {
		 ...
		 * @var string $retval
		 */
		return apply_filters( "profiles_{$this->slug_endpoint}_embed_html", $fallback_html . $retval );
	}
