...
if ( !class_exists( 'Profiles_Members_Admin' ) ) :
...
class Profiles_Members_Admin {
	...
	private function setup_globals() {
		...
		// Paths and URLs
		$this->admin_dir = trailingslashit( $profiles->plugin_dir  . 'profiles-members/admin' ); // Admin path.
		$this->admin_url = trailingslashit( $profiles->plugin_url  . 'profiles-members/admin' ); // Admin URL.
		$this->css_url   = trailingslashit( $this->admin_url . 'css' ); // Admin CSS URL.
		$this->js_url    = trailingslashit( $this->admin_url . 'js'  ); // Admin CSS URL.

		// Capability depends on config.
		$this->capability = profiles_core_do_network_admin() ? 'manage_network_users' : 'edit_users';
		...
		$this->edit_profile_args = array( 'page' => 'profiles-profile-edit' );
		...
		$this->users_page   = '';
		$this->signups_page = '';
		$this->users_url    = profiles_get_admin_url( 'users.php' );
		$this->users_screen = profiles_core_do_network_admin() ? 'users-network' : 'users';

		// Specific config: Profiles is not network activated.
		$this->subsite_activated = (bool) is_multisite() && ! profiles_is_network_activated();
	...
	private function setup_actions() {
		...
		add_action( 'profiles_members_admin_load', array( $this, 'process_member_type_update' ) );
		...
		if ( is_admin() ) {
			...
			if ( current_user_can( $this->capability ) ) {
				 ...
				 */
				if ( profiles_is_network_activated() && profiles_is_multiblog_mode() && false === strpos( $user_screen, '-network' ) ) {
					$user_screen .= '-network';
				}
	 ...
	 *
	 * Self profile editing / or profiles_moderate check.
	...
	private function member_can_edit( $user_id = 0 ) {
		...
		// Trust the 'profiles_moderate' capability.
		} else {
			$retval = profiles_current_user_can( 'profiles_moderate' );
		}
	...
	public function admin_menus() {
		...
		if ( ( ! profiles_is_network_activated() && ! is_network_admin() ) || ( is_network_admin() && profiles_is_network_activated() ) ) {
	...
	public function profile_admin_head() {
		...
		} elseif ( isset( $_GET['user_id' ] ) || ( isset( $_GET['page'] ) && ( 'profiles-profile-edit' === $_GET['page'] ) ) ) {
	...
	public function admin_head() {
		remove_submenu_page( 'users.php',   'profiles-profile-edit' );
		remove_submenu_page( 'profile.php', 'profiles-profile-edit' );
	}
	...
	public function enqueue_scripts() {
		 ...
		 */
		$css = apply_filters( 'profiles_members_admin_css', $css );

		wp_enqueue_style( 'profiles-members-css', $css, array(), profiles_get_version() );

		wp_style_add_data( 'profiles-members-css', 'rtl', true );
		if ( $min ) {
			wp_style_add_data( 'profiles-members-css', 'suffix', $min );
		}
		...
		if ( get_current_screen()->id == $this->user_page ) {
			 ...
			 */
			$js = apply_filters( 'profiles_members_admin_js', $js );
			wp_enqueue_script( 'profiles-members-js', $js, array( 'jquery' ), profiles_get_version(), true );
		}
		 ...
		 */
		do_action( 'profiles_members_admin_enqueue_scripts', get_current_screen()->id, $this->screen_id );
	}
	...
	public function user_admin_load() {
		...
		if ( ! empty( $_REQUEST['user_status'] ) ) {
			...
			if ( $spam !== profiles_is_user_spammer( $user_id ) ) {
		 ...
		 */
		do_action_ref_array( 'profiles_members_admin_load', array( $doaction, $_REQUEST ) );
		 ...
		 */
		$allowed_actions = apply_filters( 'profiles_members_admin_allowed_actions', array( 'update', 'delete_avatar', 'spam', 'ham' ) );
		...
		if ( ! in_array( $doaction, $allowed_actions ) ) {
			...
			get_current_screen()->add_help_tab( array(
				'id'      => 'profiles-profile-edit-overview',
			 ...
			 *
			 * Plugins should not use this hook, please use 'profiles_members_admin_user_metaboxes' instead.
			 *
			 * @since 2.0.0
			 *
			 ...
			 */
			do_action_ref_array( 'profiles_members_admin_xprofile_metabox', array( $user_id, get_current_screen()->id, $this->stats_metabox ) );
			...
			// User Stat metabox.
			add_meta_box(
				'profiles_members_admin_user_stats',
			...
			$member_types = profiles_get_member_types();
			if ( ! empty( $member_types ) ) {
				add_meta_box(
					'profiles_members_admin_member_type',
			 ...
			 *
			 * Plugins can restrict metabox to "profiles_moderate" admins by checking if
			 ...
			 */
			do_action( 'profiles_members_admin_user_metaboxes', $this->is_self_profile, $user_id );
		...
		// Spam or Ham user.
			...
			if ( profiles_core_process_spammer_status( $user_id, $doaction ) ) {
		...
		// Update other stuff once above ones are done.
			 ...
			 */
			do_action_ref_array( 'profiles_members_admin_update_user', array( $doaction, $user_id, $_REQUEST, $this->redirect ) );

			profiles_core_redirect( $this->redirect );
		}
	}
	...
	public function user_admin() {
		...
		if ( ! profiles_current_user_can( 'profiles_moderate' ) && empty( $this->is_self_profile ) ) {
			die( '-1' );
		}
		...
		<div class="wrap" id="community-profile-page">
			...
			<?php if ( ! empty( $user ) ) :
				...
				<form action="<?php echo esc_url( $form_action_url ); ?>" id="your-profile" method="post">
					...
					<?php wp_nonce_field( 'edit-profiles-profile_' . $user->ID ); ?>
			...
			<?php else : ?>
				...
				<p><?php
					printf(
						...
						esc_url( profiles_get_admin_url( 'users.php' ) ),
		...
		<div class="submitbox" id="submitcomment">
			<div id="minor-publishing">
				<div id="misc-publishing-actions">
					...
					$is_spammer = profiles_is_user_spammer( $user->ID );
			...
			<div id="major-publishing-actions">
				...
				<div id="publishing-action">
					<a class="button profiles-view-profile" href="<?php echo esc_url( profiles_core_get_user_domain( $user->ID ) ); ?>" target="_blank"><?php esc_html_e( 'View Profile', 'profiles' ); ?></a>
	...
	public function user_admin_stats_metabox( $user = null ) {
		...
		// Account is activated, getting user's last activity.
			...
			$last_active = profiles_get_user_last_activity( $user->ID );
		}
		...
		<ul>
			<li class="profiles-members-profile-stats"><?php printf( __( 'Last active: %1$s', 'profiles' ), '<strong>' . $date . '</strong>' ); ?></li>
			...
			if ( empty( $user->user_status ) ) {
				 ...
				 */
				do_action( 'profiles_members_admin_user_stats', array( 'user_id' => $user->ID ), $user );
			}
			?>
		</ul>
	...
	public function user_admin_member_type_metabox( $user = null ) {
		...
		$current_type = profiles_get_member_type( $user->ID );
		?>

		<label for="profiles-members-profile-member-type" class="screen-reader-text"><?php
			/* translators: accessibility text */
			esc_html_e( 'Select member type', 'profiles' );
		?></label>
		<select name="profiles-members-profile-member-type" id="profiles-members-profile-member-type">
		...
		wp_nonce_field( 'profiles-member-type-change-' . $user->ID, 'profiles-member-type-nonce' );
	}
	...
	public function process_member_type_update() {
		if ( ! isset( $_POST['profiles-member-type-nonce'] ) || ! isset( $_POST['profiles-members-profile-member-type'] ) ) {
			return;
		}

		$user_id = $this->get_user_id();

		check_admin_referer( 'profiles-member-type-change-' . $user_id, 'profiles-member-type-nonce' );

		// Permission check.
		if ( ! current_user_can( 'profiles_moderate' ) && $user_id != profiles_loggedin_user_id() ) {
			return;
		}

		// Member type string must either reference a valid member type, or be empty.
		$member_type = stripslashes( $_POST['profiles-members-profile-member-type'] );
		 ...
		 */
		if ( profiles_set_member_type( $user_id, $member_type ) ) {
	...
	public function row_actions( $actions = '', $user = null ) {
		...
		if ( current_user_can( 'edit_user', $user->ID ) || profiles_current_user_can( 'profiles_moderate' ) ) {
	...
	public function add_edit_profile_url_filter() {
		add_filter( 'profiles_members_edit_profile_url', array( $this, 'filter_adminbar_profile_link' ), 10, 3 );
	}
	...
	public function remove_edit_profile_url_filter() {
		remove_filter( 'profiles_members_edit_profile_url', array( $this, 'filter_adminbar_profile_link' ), 10, 3 );
	}
	...
	public function signup_screen_options( $value = 0, $option = '', $new_value = 0 ) {
		if ( 'users_page_profiles_signups_network_per_page' != $option && 'users_page_profiles_signups_per_page' != $option ) {
			return $value;
		}
	...
	/**
	 * Filter the WP Users List Table views to include 'profiles-signups'.
	 *
	 * @since 2.0.0
	 *
	...
	public function signup_filter_view( $views = array() ) {
		...
		$signups = Profiles_Signup::count_signups();

		if ( is_network_admin() ) {
			$base_url = network_admin_url( 'users.php' );
		} else {
			$base_url = profiles_get_admin_url( 'users.php' );
		}

		$url     = add_query_arg( 'page', 'profiles-signups', $base_url );
	...
	public static function get_list_table_class( $class = '', $required = '' ) {
		...
		if ( ! empty( $required ) ) {
			...
			if ( ! profiles()->do_autoload ) {
				require_once( profiles()->members->admin->admin_dir . 'profiles-members-admin-classes.php' );
			}
		}
	...
	public function signups_admin_load() {
		...
		$doaction    = profiles_admin_list_table_current_bulk_action();
		 ...
		 * @param array  $_REQUEST Current $_REQUEST global.
		 */
		do_action( 'profiles_signups_admin_load', $doaction, $_REQUEST );
		 ...
		 * @param array $value Array of allowed actions to use.
		 */
		$allowed_actions = apply_filters( 'profiles_signups_admin_allowed_actions', array( 'do_delete', 'do_activate', 'do_resend' ) );

		// Prepare the display of the Community Profile screen.
		if ( ! in_array( $doaction, $allowed_actions ) || ( -1 == $doaction ) ) {

			if ( is_network_admin() ) {
				$profiles_members_signup_list_table = self::get_list_table_class( 'Profiles_Members_MS_List_Table', 'ms-users' );
			} else {
				$profiles_members_signup_list_table = self::get_list_table_class( 'Profiles_Members_List_Table', 'users' );
			}
			...
			if ( profiles_get_major_wp_version() >= 4.4 ) {
			...
			if ( 'do_resend' == $doaction ) {
				...
				$resent = Profiles_Signup::resend( $signups );
				...
				if ( empty( $resent ) ) {
					...
					if ( ! empty( $resent['errors'] ) ) {
						...
						set_transient( '_profiles_admin_signups_errors', $resent['errors'], 30 );
					}
			...
			// Handle activated accounts.
				...
				$activated = Profiles_Signup::activate( $signups );
				...
				if ( empty( $activated ) ) {
					...
					if ( ! empty( $activated['errors'] ) ) {
						...
						set_transient( '_profiles_admin_signups_errors', $activated['errors'], 30 );
					}
			...
			// Handle sign-ups delete.
				...
				$deleted = Profiles_Signup::delete( $signups );
				...
				if ( empty( $deleted ) ) {
					...
					if ( ! empty( $deleted['errors'] ) ) {
						...
						set_transient( '_profiles_admin_signups_errors', $deleted['errors'], 30 );
					}
			...
			// Plugins can update other stuff from here.
				 ...
				 * @param string $redirect Determined redirect url to send user to.
				 */
				do_action( 'profiles_members_admin_update_signups', $doaction, $_REQUEST, $this->redirect );
	...
	public function signups_display_errors() {
		...
		$errors = get_transient( '_profiles_admin_signups_errors' );
		...
		delete_transient( '_profiles_admin_signups_errors' );
	}
	...
	public function signups_admin_index() {
		...
		if ( is_network_admin() ) {
			...
			$form_url = profiles_get_admin_url( 'users.php' );
		}

		$form_url = add_query_arg(
			array(
				'page' => 'profiles-signups',
			),
			$form_url
		);
		...
		<div class="wrap">
			...
			<form id="profiles-signups-search-form" action="<?php echo esc_url( $search_form_url ) ;?>">
				<input type="hidden" name="page" value="<?php echo esc_attr( $plugin_page ); ?>" />
				<?php $profiles_members_signup_list_table->search_box( __( 'Search Pending Users', 'profiles' ), 'profiles-signups' ); ?>
			</form>

			<form id="profiles-signups-form" action="<?php echo esc_url( $form_url );?>" method="post">
	...
	public function signups_admin_manage( $action = '' ) {
		...
		$signups_query = Profiles_Signup::get( array(
			'include' => $ids,
		) );
		...
		$url_args = array( 'page' => 'profiles-signups' );
		...
		<div class="wrap">
			...
			<ol class="profiles-signups-list">
