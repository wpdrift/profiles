 ...
 * the list of default services in {@link profiles_core_get_suggestions()}. If you're building a plugin,
 * it's recommend that you use the `profiles_suggestions_services` filter to do this. :)
 *
 ...
 * @since 2.1.0
 */
abstract class Profiles_Suggestions {
	...
	public function validate() {
		...
		/**
		 * Filters the arguments to be validated for the Profiles_Suggestions query.
		 *
		 * @since 2.1.0
		 *
		 * @param Profiles_Suggestions $value Arguments to be validated.
		 * @param Profiles_Suggestions $this  Current Profiles_Suggestions instance.
		 */
		$this->args          = apply_filters( 'profiles_suggestions_args', $this->args, $this );
		...
		if ( is_user_logged_in() && ! profiles_is_user_active( get_current_user_id() ) ) {
			return new WP_Error( 'invalid_user' );
		}

		/**
		 * Filters the status of validation for the Profiles_Suggestions query.
		 *
		 * @since 2.1.0
		 *
		 ...
		 * @param Profiles_Suggestions $this  Current Profiles_Suggestions instance.
		 */
		return apply_filters( 'profiles_suggestions_validate_args', true, $this );
	}
