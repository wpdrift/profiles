 ...
 */
function profiles_core_check_for_flood( $user_id = 0 ) {

	// Option disabled. No flood checks.
	if ( !$throttle_time = profiles_get_option( '_profiles_throttle_time' ) ) {
		return true;
	}
	...
	$last_posted = get_user_meta( $user_id, '_profiles_last_posted', true );
 ...
 */
function profiles_core_check_for_moderation( $user_id = 0, $title = '', $content = '', $error_type = 'bool' ) {
	 ...
	 * @param string $content $the content being posted.
	 */
	if ( apply_filters( 'profiles_bypass_check_for_moderation', false, $user_id, $title, $content ) ) {
		return true;
	}
	...
	$_post['user_ip'] = profiles_core_current_user_ip();
	$_post['user_ua'] = profiles_core_current_user_ua();
	...
	if ( ! empty( $max_links ) ) {
		...
		if ( $num_links >= $max_links ) {
			if ( 'bool' === $error_type ) {
				...
				return new WP_Error( 'profiles_moderation_too_many_links', __( 'You have posted too many links', 'profiles' ) );
			}
		}
	}
			...
			foreach ( $_post as $post_data ) {
				...
				if ( preg_match( $pattern, $post_data ) ) {
					if ( 'bool' === $error_type ) {
						...
						return new WP_Error( 'profiles_moderation_word_match', _x( 'You have posted an inappropriate word.', 'Comment moderation', 'profiles' ) );
					}
				}
			}
		}
 ...
 * @return bool True if test is passed, false if fail.
 */
function profiles_core_check_for_blacklist( $user_id = 0, $title = '', $content = '', $error_type = 'bool' ) {
	 ...
	 * @param string $content $the content being posted.
	 */
	if ( apply_filters( 'profiles_bypass_check_for_blacklist', false, $user_id, $title, $content ) ) {
		return true;
	}
		...
		foreach( $_post as $post_data ) {
			...
			if ( preg_match( $pattern, $post_data ) ) {
				if ( 'bool' === $error_type ) {
					...
					return new WP_Error( 'profiles_moderation_blacklist_match', _x( 'You have posted an inappropriate word.', 'Comment blacklist', 'profiles' ) );
				}
			}
		}
	}
 ...
 */
function profiles_core_current_user_ip() {
	 ...
	 */
	return apply_filters( 'profiles_core_current_user_ip', $retval );
}
 ...
 */
function profiles_core_current_user_ua() {
	 ...
	 */
	return apply_filters( 'profiles_core_current_user_ua', $retval );
}
