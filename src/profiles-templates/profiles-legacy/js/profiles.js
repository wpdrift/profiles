...
var profiles_ajax_request = null;
...
jq(document).ready( function() {
	...
	profiles_init_activity();

	var objects  = [ 'members', 'groups', 'blogs', 'forums', 'group_members' ],
		$whats_new = jq('#whats-new');

	/* Object filter and scope set. */
	profiles_init_objects( objects );

	/* @mention Compose Scrolling */
	if ( $whats_new.length && profiles_get_querystring('r') ) {
	...
	jq('#aw-whats-new-submit').on( 'click', function() {
		...
		post_data = jq.extend( {
			action: 'post_update',
			'cookie': profiles_get_cookies(),
			'_wpnonce_post_update': jq('#_wpnonce_post_update').val(),
			'content': content,
			'object': object,
			'item_id': item_id,
			'since': last_date_recorded,
			'_profiles_as_nonce': jq('#_profiles_as_nonce').val() || ''
		}, inputs );
		...
		jq.post( ajaxurl, post_data, function( response ) {
			...
			if ( response[0] + response[1] === '-1' ) {
				...
				if ( 0 !== jq('#latest-update').length ) {
					...
					u += '<a href="' + v + '" rel="nofollow">' + Profiles_DTheme.view + '</a>';
	...
	jq('div.activity-type-tabs').on( 'click', function(event) {
		...
		/* Reset the page */
		jq.cookie( 'profiles-activity-oldestpage', 1, {
			path: '/',
		...
		profiles_activity_request(scope, filter);
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.hasClass('fav') || target.hasClass('unfav') ) {
			...
			function(response) {
				...
				target.fadeOut( 200, function() {
					...
					jq(this).attr('title', 'fav' === type ? Profiles_DTheme.remove_fav : Profiles_DTheme.mark_as_fav);
					jq(this).fadeIn(200);
				});
				...
				if ( 'fav' === type ) {
					if ( !jq('.item-list-tabs #activity-favs-personal-li').length ) {
						if ( !jq('.item-list-tabs #activity-favorites').length ) {
							jq('.item-list-tabs ul #activity-mentions').before( '<li id="activity-favorites"><a href="#">' + Profiles_DTheme.my_favs + ' <span>0</span></a></li>');
						}
					...
					if ( !Number( jq('.item-list-tabs ul #activity-favorites span').html() ) ) {
						if ( jq('.item-list-tabs ul #activity-favorites').hasClass('selected') ) {
							profiles_activity_request( null, null );
						}
		...
		if ( target.parent().hasClass('load-more') ) {
			if ( profiles_ajax_request ) {
				profiles_ajax_request.abort();
			}

			jq('#profiles li.load-more').addClass('loading');

			if ( null === jq.cookie('profiles-activity-oldestpage') ) {
				jq.cookie('profiles-activity-oldestpage', 1, {
					path: '/',
					secure: ( 'https:' === window.location.protocol )
				} );
			...
			oldest_page = ( jq.cookie('profiles-activity-oldestpage') * 1 ) + 1;
			just_posted = [];
			...
			load_more_search = profiles_get_querystring('s');

			if ( load_more_search ) {
				load_more_args.search_terms = load_more_search;
			}

			profiles_ajax_request = jq.post( ajaxurl, load_more_args,
			function(response)
			{
				jq('#profiles li.load-more').removeClass('loading');
				jq.cookie( 'profiles-activity-oldestpage', oldest_page, {
					path: '/',
					secure: ( 'https:' === window.location.protocol )
				} );
	...
	if ( jq('.activity-comments').length ) {
		profiles_legacy_theme_hide_comments();
	}
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.attr('name') === 'ac_form_submit' ) {
			...
			// Akismet
			ak_nonce = jq('#_profiles_as_nonce_' + comment_id).val();
			if ( ak_nonce ) {
				ajaxdata['_profiles_as_nonce_' + comment_id] = ak_nonce;
			}
			...
			jq.post( ajaxurl, ajaxdata, function(response) {
				...
				if ( response[0] + response[1] === '-1' ) {
					...
					if ( show_all_a ) {
						...
						show_all_a.html( Profiles_DTheme.show_x_comments.replace( '%d', new_count ) );
					}
				}
		...
		if ( target.hasClass( 'spam-activity-comment' ) ) {
			...
			jq.post( ajaxurl, {
				action: 'profiles_spam_activity_comment',
				'cookie': encodeURIComponent( document.cookie ),
	...
	jq( '.dir-search, .groups-members-search' ).on( 'click', function(event) {
		...
		if ( target.attr('type') === 'submit' ) {
			...
			profiles_filter_request( object, jq.cookie('profiles-' + object + '-filter'), jq.cookie('profiles-' + object + '-scope') , 'div.' + object, target.parent().children('label').children('input').val(), 1, jq.cookie('profiles-' + object + '-extras'), null, template );
	...
	jq('div.item-list-tabs').on( 'click', function(event) {
		...
		if ( 'LI' === target[0].nodeName && !target.hasClass( 'last' ) ) {
			...
			profiles_filter_request( object, filter, scope, 'div.' + object, search_terms, 1, jq.cookie('profiles-' + object + '-extras') );
	...
	jq('li.filter select').change( function() {
		...
		profiles_filter_request( object, filter, scope, 'div.' + object, search_terms, 1, jq.cookie('profiles-' + object + '-extras'), null, template );
	...
	jq('#profiles').on( 'click', function(event) {
		...
		if ( target.parent().parent().hasClass('pagination') && !target.parent().parent().hasClass('no-ajax') ) {
			...
			if ( jq('div.dir-search input').length ) {
				...
				if ( ! search_terms.val() && profiles_get_querystring( search_terms.attr( 'name' ) ) ) {
			...
			profiles_filter_request( object, jq.cookie('profiles-' + object + '-filter'), jq.cookie('profiles-' + object + '-scope'), 'div.' + object, search_terms, page_number, jq.cookie('profiles-' + object + '-extras'), caller, template );
	...
	jq('#forum-directory-tags a').on( 'click', function() {
		profiles_filter_request( 'forums', 'tags', jq.cookie('profiles-forums-scope'), 'div.forums', jq(this).html().replace( /&nbsp;/g, '-' ), 1, jq.cookie('profiles-forums-extras') );
		return false;
	});
	...
	jq('#send-invite-form').on( 'click', '#invite-list input', function() {
		...
		function(response)
		{
			...
			if ( invites_new_template ) {
				...
				// entire list
				profiles_filter_request( 'invite', 'profiles-invite-filter', 'profiles-invite-scope', 'div.invite', false, 1, '', '', '' );
			} else {
	...
	jq('#profile-edit-form input:not(:submit), #profile-edit-form textarea, #profile-edit-form select, #signup_form input:not(:submit), #signup_form textarea, #signup_form select').change( function() {
		...
		window.onbeforeunload = function(e) {
			if ( shouldconfirm ) {
				return Profiles_DTheme.unsaved_changes;
			}
		};
	});
			...
			if ( response[0] + response[1] === '-1' ) {
				...
				button.fadeOut( 100, function() {
					if ( jq(this).hasClass('accept') ) {
						...
						jq(this).html( Profiles_DTheme.accepted ).contents().unwrap();
					} else {
						action_div.children('a.accept').hide();
						jq(this).html( Profiles_DTheme.rejected ).contents().unwrap();
					}
				});
			}
		});
	...
	jq('#profiles').on('click', '.group-button .leave-group', function() {
		if ( false === confirm( Profiles_DTheme.leave_group_confirm ) ) {
			return false;
		}
	});
	...
	jq('#groups-dir-list').on('click', '.group-button a', function() {
		...
		// AJAX request
		if ( thelink.hasClass( 'leave-group' ) && false === confirm( Profiles_DTheme.leave_group_confirm ) ) {
			return false;
		}
	...
	jq('.message-search').on( 'click', function(event) {
		...
		if ( target.attr('type') === 'submit' || target.attr('type') === 'button' ) {
			...
			profiles_filter_request(
				object,
				jq.cookie('profiles-' + object + '-filter'),
				jq.cookie('profiles-' + object + '-scope'),
				'div.' + object, jq('#messages_search').val(),
				1,
				jq.cookie('profiles-' + object + '-extras')
			);
		...
		function(response) {
			if ( 1 === parseInt( response, 10 ) ) {
				if ( 'unstar' === link.data('star-status') ) {
					...
					link.find('.profiles-screen-reader-text').text( Profiles_PM_Star.strings.text_star );

					if ( 1 === Profiles_PM_Star.is_single_thread ) {
						link.prop('title', Profiles_PM_Star.strings.title_star );
					} else {
						link.prop('title', Profiles_PM_Star.strings.title_star_thread );
					}

				} else {
					link.data('star-status', 'unstar');
					link.removeClass('message-action-star').addClass('message-action-unstar');
					link.find('.profiles-screen-reader-text').text(Profiles_PM_Star.strings.text_unstar);

					if ( 1 === Profiles_PM_Star.is_single_thread ) {
						link.prop('title', Profiles_PM_Star.strings.title_unstar );
					} else {
						link.prop('title', Profiles_PM_Star.strings.title_unstar_thread );
					}
				}
			}
		});
	...
	jq('#message-threads td.bulk-select-check :checkbox').on('change', function() {
		...
		if ( box.prop('checked') ) {
			if( 'unstar' === star.data('star-status') ) {
				Profiles_PM_Star.star_counter++;
			} else {
				Profiles_PM_Star.unstar_counter++;
			}
		} else {
			if( 'unstar' === star.data('star-status') ) {
				Profiles_PM_Star.star_counter--;
		...
		if ( Profiles_PM_Star.star_counter > 0 && parseInt( Profiles_PM_Star.unstar_counter, 10 ) === 0 ) {
			jq('option[value="star"]').hide();
		} else {
			jq('option[value="star"]').show();
		}

		if ( Profiles_PM_Star.unstar_counter > 0 && parseInt( Profiles_PM_Star.star_counter, 10 ) === 0 ) {
	...
	jq('#wp-admin-bar-logout, a.logout').on( 'click', function() {
		jq.removeCookie('profiles-activity-scope', {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		});
		jq.removeCookie('profiles-activity-filter', {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		});
		jq.removeCookie('profiles-activity-oldestpage', {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		});
		...
		jq(objects).each( function(i) {
			jq.removeCookie('profiles-' + objects[i] + '-scope', {
				path: '/',
				secure: ( 'https:' === window.location.protocol )
			} );
			jq.removeCookie('profiles-' + objects[i] + '-filter', {
				path: '/',
				secure: ( 'https:' === window.location.protocol )
			} );
			jq.removeCookie('profiles-' + objects[i] + '-extras', {
				path: '/',
				secure: ( 'https:' === window.location.protocol )
			} );
		});
	...
	if ( typeof wp !== 'undefined' && typeof wp.heartbeat !== 'undefined' && typeof Profiles_DTheme.pulse !== 'undefined' ) {

		wp.heartbeat.interval( Number( Profiles_DTheme.pulse ) );
	...
	jq( document ).on( 'heartbeat-send.profiles', function( e, data ) {
		...
		data.profiles_activity_last_recorded = activity_last_recorded;

		last_recorded_search = profiles_get_querystring('s');

		if ( last_recorded_search ) {
			data.profiles_activity_last_recorded_search_terms = last_recorded_search;
		}
	});
	...
	jq( document ).on( 'heartbeat-tick', function( e, data ) {
		...
		if ( ! data.profiles_activity_newest_activities ) {
			return;
		}

		newest_activities = data.profiles_activity_newest_activities.activities + newest_activities;
		activity_last_recorded  = Number( data.profiles_activity_newest_activities.last_recorded );

		if ( jq( '#profiles ul.activity-list li' ).first().hasClass( 'load-newest' ) ) {
			return;
		}

		jq( '#profiles ul.activity-list' ).prepend( '<li class="load-newest"><a href="#newest">' + Profiles_DTheme.newest + '</a></li>' );
	});
});

/* Setup activity scope and filter based on the current cookie settings. */
function profiles_init_activity() {
	/* Reset the page */
	jq.cookie( 'profiles-activity-oldestpage', 1, {
		path: '/',
		secure: ( 'https:' === window.location.protocol )
	...
	if ( undefined !== jq.cookie('profiles-activity-filter') && jq('#activity-filter-select').length ) {
		jq('#activity-filter-select select option[value="' + jq.cookie('profiles-activity-filter') + '"]').prop( 'selected', true );
	}

	/* Activity Tab Set */
	if ( undefined !== jq.cookie('profiles-activity-scope') && jq('.activity-type-tabs').length ) {
		jq('.activity-type-tabs li').each( function() {
			jq(this).removeClass('selected');
		});
		jq('#activity-' + jq.cookie('profiles-activity-scope') + ', .item-list-tabs li.current').addClass('selected');
	}
}

/* Setup object scope and filter based on the current cookie settings for the object. */
function profiles_init_objects(objects) {
	jq(objects).each( function(i) {
		if ( undefined !== jq.cookie('profiles-' + objects[i] + '-filter') && jq('#' + objects[i] + '-order-select select').length ) {
			jq('#' + objects[i] + '-order-select select option[value="' + jq.cookie('profiles-' + objects[i] + '-filter') + '"]').prop( 'selected', true );
		}

		if ( undefined !== jq.cookie('profiles-' + objects[i] + '-scope') && jq('div.' + objects[i]).length ) {
			jq('.item-list-tabs li').each( function() {
				jq(this).removeClass('selected');
			});
			jq('#' + objects[i] + '-' + jq.cookie('profiles-' + objects[i] + '-scope') + ', #object-nav li.current').addClass('selected');
		}
	});
}

/* Filter the current content list (groups/members/blogs/topics) */
function profiles_filter_request( object, filter, scope, target, search_terms, page, extras, caller, template ) {
	if ( 'activity' === object ) {
		return false;
	}
	...
	jq.cookie( 'profiles-' + object + '-scope', scope, {
		path: '/',
		secure: ( 'https:' === window.location.protocol )
	} );
	jq.cookie( 'profiles-' + object + '-filter', filter, {
		path: '/',
		secure: ( 'https:' === window.location.protocol )
	} );
	jq.cookie( 'profiles-' + object + '-extras', extras, {
		path: '/',
		secure: ( 'https:' === window.location.protocol )
	} );
	...
	profiles_ajax_request = jq.post( ajaxurl, {
...
function profiles_activity_request(scope, filter) {
	/* Save the type and filter to a session cookie */
	if ( null !== scope ) {
		jq.cookie( 'profiles-activity-scope', scope, {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		} );
	}
	if ( null !== filter ) {
		jq.cookie( 'profiles-activity-filter', filter, {
			path: '/',
			secure: ( 'https:' === window.location.protocol )
		} );
	}
	...
	jq('.widget_profiles_activity_widget h2 span.ajax-loader').show();
	...
	function(response)
	{
		jq('.widget_profiles_activity_widget h2 span.ajax-loader').hide();
...
function profiles_legacy_theme_hide_comments() {
	...
	comments_divs.each( function() {
		...
		comment_lis.each( function(i) {
			...
			if ( i < comment_lis.length - 5 ) {
				...
				if ( !i ) {
					jq(this).before( '<li class="show-all"><a href="#' + parent_li.attr('id') + '/show-all/" title="' + Profiles_DTheme.show_all_comments + '">' + Profiles_DTheme.show_x_comments.replace( '%d', comment_count ) + '</a></li>' );
				}
			}
		});
...
/* Returns a querystring of BP cookies (cookies beginning with 'profiles-') */
function profiles_get_cookies() {
	var allCookies = document.cookie.split(';'),  // get all cookies and split into an array
		profilesCookies      = {},
		cookiePrefix   = 'profiles-',
	...
	for (i = 0; i < allCookies.length; i++) {
		...
		if ( name.indexOf(cookiePrefix) === 0 ) {
			profilesCookies[name] = value;
		}
	}

	// returns BP cookies as querystring
	return encodeURIComponent( jq.param(profilesCookies) );
}
