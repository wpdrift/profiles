 ...
 * @return string Path to located template. See {@link profiles_locate_template()}.
 */
function profiles_get_template_part( $slug, $name = null ) {

	/**
	 * Fires at the start of profiles_get_template_part().
	 *
	 ...
	 * @param string $name      Template part name requested.
	 */
	$templates = apply_filters( 'profiles_get_template_part', $templates, $slug, $name );

	// Return the part that is found.
	return profiles_locate_template( $templates, true, false );
}

/**
 * Get an asset template part.
 *
 * Basically the same as {@link profiles_get_template_part()}, but with 'assets/'
 * prepended to the slug.
 *
 * @since 2.6.0
 *
 * @see profiles_get_template_part() for full documentation.
 *
 * @param string      $slug Template slug.
 * @param string|null $name Template name.
 * @return string
 */
function profiles_get_asset_template_part( $slug, $name = null ) {
	return profiles_get_template_part( "assets/{$slug}", $name );
}
 ...
 */
function profiles_locate_template( $template_names, $load = false, $require_once = true ) {

	// No file found yet.
	$located            = false;
	$template_locations = profiles_get_template_stack();
	 ...
	 * 'profiles_get_template_part' or add a new location to the template stack.
	 */
	do_action( 'profiles_locate_template', $located, $template_name, $template_names, $template_locations, $load, $require_once );
	 ...
	 */
	$load_template = (bool) apply_filters( 'profiles_locate_template_and_load', true );
 ...
 *
 * Similar to {@link profiles_locate_template()}, but for files like CSS and JS.
 *
 * @since 2.6.0
 *
 ...
 */
function profiles_locate_template_asset( $filename ) {
	// Ensure assets can be located when running from /src/.
	if ( defined( 'Profiles_SOURCE_SUBDIRECTORY' ) && 'src' === Profiles_SOURCE_SUBDIRECTORY ) {
		$filename = str_replace( '.min', '', $filename );
	}

	// Use profiles_locate_template() to find our asset.
	$located = profiles_locate_template( $filename, false );
	if ( false === $located ) {
		return false;
	}
	...
	$find = array(
		get_theme_root(),
		profiles_get_theme_compat_dir()
	);

	$replace = array(
		get_theme_root_uri(),
		profiles_get_theme_compat_url()
	);
 ...
 * with profiles_locate_template(), this allows for easy template overrides.
 *
 * @since 1.7.0
 *
 ...
 */
function profiles_register_template_stack( $location_callback = '', $priority = 10 ) {
	...
	return add_filter( 'profiles_template_stack', $location_callback, (int) $priority );
}
 ...
 * @since 1.7.0
 *
 * @see profiles_register_template_stack()
 *
 * @param string $location_callback Callback function that returns the stack location.
 * @param int    $priority          Optional. The priority parameter passed to
 *                                  {@link profiles_register_template_stack()}. Default: 10.
 * @return bool See {@link remove_filter()}.
 */
function profiles_deregister_template_stack( $location_callback = '', $priority = 10 ) {
	...
	// Add location callback to template stack.
	return remove_filter( 'profiles_template_stack', $location_callback, (int) $priority );
}

/**
 * Get the "template stack", a list of registered directories where templates can be found.
 *
 * Calls the functions added to the 'profiles_template_stack' filter hook, and return
 ...
 */
function profiles_get_template_stack() {
	global $wp_filter, $merged_filters, $wp_current_filter;

	// Setup some default variables.
	$tag  = 'profiles_template_stack';
	$args = $stack = array();

	// Add 'profiles_template_stack' to the current filter array.
	...
	// Loop through 'profiles_template_stack' filters, and call callback functions.
	do {
	...
	// Remove 'profiles_template_stack' from the current filter array.
	 ...
	 */
	return (array) apply_filters( 'profiles_get_template_stack', $stack ) ;
}
 ...
 * @since 1.7.0
 *
 * @see profiles_get_template_part() for a description of $slug and $name params.
 *
 * @param string      $slug See {@link profiles_get_template_part()}.
 * @param string|null $name See {@link profiles_get_template_part()}.
 ...
 */
function profiles_buffer_template_part( $slug, $name = null, $echo = true ) {
	ob_start();

	// Remove 'profiles_replace_the_content' filter to prevent infinite loops.
	remove_filter( 'the_content', 'profiles_replace_the_content' );

	profiles_get_template_part( $slug, $name );
 ...
 * the use of {@link profiles_locate_template()}. Allows for more generic template
 ...
 */
function profiles_get_query_template( $type, $templates = array() ) {
	 ...
	 * profiles_get_query_template.
	 *
	 * @since 1.7.0
	 *
	 * @param array $templates Array of template files already prepared.
	 */
	$templates = apply_filters( "profiles_get_{$type}_template", $templates );
	...
	$templates = profiles_set_theme_compat_templates( $templates );
	$template  = profiles_locate_template( $templates );
	$template  = profiles_set_theme_compat_template( $template );
	 ...
	 */
	return apply_filters( "profiles_{$type}_template", $template );
}
 ...
 */
function profiles_get_template_locations( $templates = array() ) {
	$locations = array(
		'profiles',
		'community',
		''
	 ...
	 */
	return apply_filters( 'profiles_get_template_locations', $locations, $templates );
}
 ...
 */
function profiles_add_template_stack_locations( $stacks = array() ) {
	$retval = array();

	// Get alternate locations.
	$locations = profiles_get_template_locations();
	 ...
	 * @param array $stacks Array of template locations.
	 */
	return apply_filters( 'profiles_add_template_stack_locations', array_unique( $retval ), $stacks );
}
 ...
 */
function profiles_parse_query( $posts_query ) {
	...
	/**
	 * Fires at the end of the profiles_parse_query function.
	 *
	 ...
	 */
	do_action_ref_array( 'profiles_parse_query', array( &$posts_query ) );
}
 ...
 */
function profiles_template_include_theme_supports( $template = '' ) {
	 ...
	 * @param string $template The path to the template file that is being used.
	 */
	$new_template = apply_filters( 'profiles_get_root_template', false, $template );
	...
	if ( ! empty( $new_template ) ) {
		$template = profiles_set_template_included( $new_template );
	}
	 ...
	 * @param string $template The path to the template file that is being used.
	 */
	return apply_filters( 'profiles_template_include_theme_supports', $template );
}
 ...
 */
function profiles_set_template_included( $template = false ) {
 ...
 */
function profiles_is_template_included() {
 ...
 */
function profiles_load_theme_functions() {
	...
	if ( ! defined( 'WP_INSTALLING' ) || 'wp-activate.php' === $pagenow ) {
		profiles_locate_template( 'profiles-functions.php', true );
	}
}
 ...
 */
function profiles_get_theme_compat_templates() {
	return profiles_get_query_template( 'profiles', array(
		'plugin-profiles.php',
		'profiles.php',
		'community.php',
		'generic.php',
