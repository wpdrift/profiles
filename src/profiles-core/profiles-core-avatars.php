 ...
 * @since 1.2.0
 */
function profiles_core_set_avatar_constants() {

	$profiles = profiles();

	if ( !defined( 'Profiles_AVATAR_THUMB_WIDTH' ) )
		define( 'Profiles_AVATAR_THUMB_WIDTH', 50 );

	if ( !defined( 'Profiles_AVATAR_THUMB_HEIGHT' ) )
		define( 'Profiles_AVATAR_THUMB_HEIGHT', 50 );

	if ( !defined( 'Profiles_AVATAR_FULL_WIDTH' ) )
		define( 'Profiles_AVATAR_FULL_WIDTH', 150 );

	if ( !defined( 'Profiles_AVATAR_FULL_HEIGHT' ) )
		define( 'Profiles_AVATAR_FULL_HEIGHT', 150 );

	if ( !defined( 'Profiles_AVATAR_ORIGINAL_MAX_WIDTH' ) )
		define( 'Profiles_AVATAR_ORIGINAL_MAX_WIDTH', 450 );

	if ( !defined( 'Profiles_AVATAR_ORIGINAL_MAX_FILESIZE' ) ) {
		define( 'Profiles_AVATAR_ORIGINAL_MAX_FILESIZE', profiles_attachments_get_max_upload_file_size( 'avatar' ) );
	}

	if ( ! defined( 'Profiles_SHOW_AVATARS' ) ) {
		define( 'Profiles_SHOW_AVATARS', profiles_get_option( 'show_avatars' ) );
	}
}
add_action( 'profiles_init', 'profiles_core_set_avatar_constants', 3 );
 ...
 * @since 1.5.0
 */
function profiles_core_set_avatar_globals() {
	$profiles = profiles();
	...
	// Dimensions.
	$profiles->avatar->thumb->width  = Profiles_AVATAR_THUMB_WIDTH;
	$profiles->avatar->thumb->height = Profiles_AVATAR_THUMB_HEIGHT;
	$profiles->avatar->full->width   = Profiles_AVATAR_FULL_WIDTH;
	$profiles->avatar->full->height  = Profiles_AVATAR_FULL_HEIGHT;

	// Upload maximums.
	$profiles->avatar->original_max_width    = Profiles_AVATAR_ORIGINAL_MAX_WIDTH;
	$profiles->avatar->original_max_filesize = Profiles_AVATAR_ORIGINAL_MAX_FILESIZE;

	// Defaults.
	$profiles->avatar->thumb->default = profiles_core_avatar_default_thumb();
	$profiles->avatar->full->default  = profiles_core_avatar_default();

	// These have to be set on page load in order to avoid infinite filter loops at runtime.
	$profiles->avatar->upload_path = profiles_core_avatar_upload_path();
	$profiles->avatar->url = profiles_core_avatar_url();

	// Cache the root blog's show_avatars setting, to avoid unnecessary
	// calls to switch_to_blog().
	$profiles->avatar->show_avatars = (bool) Profiles_SHOW_AVATARS;

	// Backpat for pre-1.5.
	if ( ! defined( 'Profiles_AVATAR_UPLOAD_PATH' ) )
		define( 'Profiles_AVATAR_UPLOAD_PATH', $profiles->avatar->upload_path );

	// Backpat for pre-1.5.
	if ( ! defined( 'Profiles_AVATAR_URL' ) )
		define( 'Profiles_AVATAR_URL', $profiles->avatar->url );
	 ...
	 * @since 1.5.0
	 */
	do_action( 'profiles_core_set_avatar_globals' );
}
add_action( 'profiles_setup_globals', 'profiles_core_set_avatar_globals' );
 ...
 * locally:
 *    add_filter( 'profiles_core_fetch_avatar_no_grav', '__return_true' );
 *
 * @since 1.1.0
 ...
 *     will, if not provided, be auto-detected by profiles_core_fetch_avatar(). This
 ...
 *                                   {@link profiles_get_activity_avatar()} for an example. Default: ''.
 ...
 *                                   "{$item_id}-{$object}@{profiles_get_root_domain()}". The user query adds
 ...
 *                                   fallbacks globally, see the 'profiles_core_fetch_avatar_no_grav' filter.
 ...
 */
function profiles_core_fetch_avatar( $args = '' ) {
	$profiles = profiles();
	...
	if ( empty( $params['item_id'] ) ) {
		...
		switch ( $params['object'] ) {
			...
			case 'user'  :
			default      :
				$params['item_id'] = profiles_displayed_user_id();
				break;
		}
		 ...
		 */
		$params['item_id'] = apply_filters( 'profiles_core_avatar_item_id', $params['item_id'], $params['object'], $params );
	...
	if ( empty( $params['avatar_dir'] ) ) {
		 ...
		 * @param array  $params Array of parameters for the request.
		 */
		$params['avatar_dir'] = apply_filters( 'profiles_core_avatar_dir', $params['avatar_dir'], $params['object'], $params );
	...
	if ( false !== strpos( $params['alt'], '%s' ) || false !== strpos( $params['alt'], '%1$s' ) ) {
		...
		switch ( $params['object'] ) {
			...
			case 'group' :
				$item_name = profiles_get_group_name( groups_get_group( array( 'group_id' => $params['item_id'] ) ) );
				break;

			case 'user'  :
			default :
				$item_name = profiles_core_get_user_displayname( $params['item_id'] );
				break;
		}
		 ...
		 * @param array  $params Array of parameters for the request.
		 */
		$item_name = apply_filters( 'profiles_core_avatar_alt', $item_name, $params['item_id'], $params['object'], $params );
	 ...
	 * @param array  $params Array of parameters for the request.
	 */
	$params['title'] = apply_filters( 'profiles_core_avatar_title', $params['title'], $params['item_id'], $params['object'], $params );
	 ...
	 * @param array  $params Array of parameters for the request.
	 */
	$params['css_id'] = apply_filters( 'profiles_core_css_id', $params['css_id'], $params['item_id'], $params['object'], $params );
	...
	if ( false !== $params['width'] ) {
		...
		$params['width'] = profiles_core_avatar_thumb_width();
	} else {
		$params['width'] = profiles_core_avatar_full_width();
	}
	...
	if ( false !== $params['height'] ) {
		...
		$params['height'] = profiles_core_avatar_thumb_height();
	} else {
		$params['height'] = profiles_core_avatar_full_height();
	}
	 ...
	 * @param array        $params Array of parameters for the request.
	 */
	$params['class'] = apply_filters( 'profiles_core_avatar_class', $params['class'], $params['item_id'], $params['object'], $params );
	...
	$avatar_loc->path  = trailingslashit( profiles_core_avatar_upload_path() );
	$avatar_loc->url   = trailingslashit( profiles_core_avatar_url() );
	 ...
	 */
	$avatar_folder_url = apply_filters( 'profiles_core_avatar_folder_url', ( $avatar_loc->url  . $avatar_loc->dir . $params['item_id'] ), $params['item_id'], $params['object'], $params['avatar_dir'] );
	 ...
	 * @param string $value Subdirectory where the requested avatar should be found.
	 */
	$avatar_folder_dir = apply_filters( 'profiles_core_avatar_folder_dir', ( $avatar_loc->path . $avatar_loc->dir . $params['item_id'] ), $params['item_id'], $params['object'], $params['avatar_dir'] );
	 ...
	 * or thumbnail image.
	 */
	$avatar_size              = ( 'full' == $params['type'] ) ? '-profilesfull' : '-profilesthumb';
	...
	if ( file_exists( $avatar_folder_dir ) ) {
		...
		if ( isset( $avatar_url ) ) {
			...
			if ( true === $params['html'] ) {
				 ...
				 */
				return apply_filters( 'profiles_core_fetch_avatar', '<img src="' . $avatar_url . '"' . $html_class . $html_css_id  . $html_width . $html_height . $html_alt . $html_title . $extra_attr . ' />', $params, $params['item_id'], $params['avatar_dir'], $html_css_id, $html_width, $html_height, $avatar_folder_url, $avatar_folder_dir );
			...
			// ...or only the URL
				 ...
				 * @param array  $params     Array of parameters for the request.
				 */
				return apply_filters( 'profiles_core_fetch_avatar_url', $avatar_url, $params );
			}
		}
	}
	 ...
	 */
	if ( ! apply_filters( 'profiles_core_fetch_avatar_no_grav', $params['no_grav'], $params ) ) {
		...
		if ( empty( $profiles->grav_default->{$params['object']} ) ) {
			 ...
			 */
			$default_grav = apply_filters( 'profiles_core_mysteryman_src', 'mm', $params['width'] );
		} else {
		...
		if ( empty( $params['email'] ) ) {
			if ( 'user' == $params['object'] ) {
				$params['email'] = profiles_core_get_user_email( $params['item_id'] );
			} elseif ( 'group' == $params['object'] || 'blog' == $params['object'] ) {
				$params['email'] = $params['item_id'] . '-' . $params['object'] . '@' . profiles_get_root_domain();
			}
		}
		 ...
		 */
		$params['email'] = apply_filters( 'profiles_core_gravatar_email', $params['email'], $params['item_id'], $params['object'] );
		 ...
		 */
		$gravatar = apply_filters( 'profiles_gravatar_url', '//www.gravatar.com/avatar/' );
		 ...
		 */
		$default_grav = apply_filters( 'profiles_core_avatar_default', $default_grav, $params );
	...
	// No avatar was found, and we've been told not to use a gravatar.
		 ...
		 * @param array  $params Array of parameters for the avatar request.
		 */
		$gravatar = apply_filters( 'profiles_core_default_avatar_' . $params['object'], profiles_core_avatar_default( 'local', $params ), $params );
	}

	if ( true === $params['html'] ) {

		/** This filter is documented in profiles-core/profiles-core-avatars.php */
		return apply_filters( 'profiles_core_fetch_avatar', '<img src="' . $gravatar . '"' . $html_css_id . $html_class . $html_width . $html_height . $html_alt . $html_title . $extra_attr . ' />', $params, $params['item_id'], $params['avatar_dir'], $html_css_id, $html_width, $html_height, $avatar_folder_url, $avatar_folder_dir );
	} else {

		/** This filter is documented in profiles-core/profiles-core-avatars.php */
		return apply_filters( 'profiles_core_fetch_avatar_url', $gravatar, $params );
	}
}
 ...
 */
function profiles_core_delete_existing_avatar( $args = '' ) {
	 ...
	 * }
	 */
	if ( ! apply_filters( 'profiles_core_pre_delete_existing_avatar', true, $args ) ) {
		return true;
	}
	...
	if ( empty( $item_id ) ) {
		...
		/** This filter is documented in profiles-core/profiles-core-avatars.php */
		$item_id = apply_filters( 'profiles_core_avatar_item_id', $item_id, $object );
	...
	if ( empty( $avatar_dir ) ) {
		...
		/** This filter is documented in profiles-core/profiles-core-avatars.php */
		$avatar_dir = apply_filters( 'profiles_core_avatar_dir', $avatar_dir, $object );

		if ( !$avatar_dir ) return false;
	}

	/** This filter is documented in profiles-core/profiles-core-avatars.php */
	$avatar_folder_dir = apply_filters( 'profiles_core_avatar_folder_dir', profiles_core_avatar_upload_path() . '/' . $avatar_dir . '/' . $item_id, $item_id, $object, $avatar_dir );
	...
	if ( $av_dir = opendir( $avatar_folder_dir ) ) {
		while ( false !== ( $avatar_file = readdir($av_dir) ) ) {
			if ( ( preg_match( "/-profilesfull/", $avatar_file ) || preg_match( "/-profilesthumb/", $avatar_file ) ) && '.' != $avatar_file && '..' != $avatar_file )
	 ...
	 */
	do_action( 'profiles_core_delete_existing_avatar', $args );
 ...
 */
function profiles_avatar_ajax_delete() {
	...
	$nonce = 'profiles_delete_avatar_link';
	if ( 'group' === $avatar_data['object'] ) {
		$nonce = 'profiles_group_avatar_delete';
	}
	...
	// Capability check.
	if ( ! profiles_attachments_current_user_can( 'edit_avatar', $avatar_data ) ) {
		wp_send_json_error();
	}

	// Handle delete.
	if ( profiles_core_delete_existing_avatar( array( 'item_id' => $avatar_data['item_id'], 'object' => $avatar_data['object'] ) ) ) {
		$return = array(
			'avatar' => html_entity_decode( profiles_core_fetch_avatar( array(
...
add_action( 'wp_ajax_profiles_avatar_delete', 'profiles_avatar_ajax_delete' );

/**
 * Handle avatar uploading.
 *
 * The functions starts off by checking that the file has been uploaded
 * properly using profiles_core_check_avatar_upload(). It then checks that the file
 ...
 * @since 1.1.0
 *
 * @see profiles_core_check_avatar_upload()
 * @see profiles_core_check_avatar_type()
 *
 * @param array  $file              The appropriate entry the from $_FILES superglobal.
 * @param string $upload_dir_filter A filter to be applied to 'upload_dir'.
 * @return bool True on success, false on failure.
 */
function profiles_core_avatar_handle_upload( $file, $upload_dir_filter ) {
	 ...
	 */
	if ( ! apply_filters( 'profiles_core_pre_avatar_handle_upload', true, $file, $upload_dir_filter ) ) {
		return true;
	}

	// Setup some variables.
	$profiles          = profiles();
	$upload_path = profiles_core_avatar_upload_path();

	// Upload the file.
	$avatar_attachment = new Profiles_Attachment_Avatar();
	...
	if ( ! empty( $profiles->avatar_admin->original['error'] ) ) {
		profiles_core_add_message( sprintf( __( 'Upload Failed! Error was: %s', 'profiles' ), $profiles->avatar_admin->original['error'] ), 'error' );
		return false;
	}
	...
	if ( is_wp_error( $profiles->avatar_admin->image->dir ) ) {
		profiles_core_add_message( sprintf( __( 'Upload failed! Error was: %s', 'profiles' ), $profiles->avatar_admin->image->dir->get_error_message() ), 'error' );
		return false;
	}
	...
	if ( $avatar_attachment->is_too_small( $profiles->avatar_admin->image->file ) ) {
		profiles_core_add_message( sprintf( __( 'You have selected an image that is smaller than recommended. For best results, upload a picture larger than %d x %d pixels.', 'profiles' ), profiles_core_avatar_full_width(), profiles_core_avatar_full_height() ), 'error' );
	}

	// Set the url value for the image.
	$profiles->avatar_admin->image->url = profiles_core_avatar_url() . $profiles->avatar_admin->image->dir;
 ...
 *                     error message otherwise.
 */
function profiles_avatar_ajax_upload() {
	// Bail if not a POST action.
	if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) ) {
		wp_die();
	}
	...
	// Check the nonce.
	check_admin_referer( 'profiles-uploader' );

	// Init the Profiles parameters.
	$profiles_params = array();

	// We need it to carry on.
	if ( ! empty( $_POST['profiles_params' ] ) ) {
		$profiles_params = $_POST['profiles_params' ];
	} else {
		profiles_attachments_json_response( false, $is_html4 );
	}
	...
	// Capability check.
	if ( ! profiles_attachments_current_user_can( 'edit_avatar', $profiles_params ) ) {
		profiles_attachments_json_response( false, $is_html4 );
	}
	...
	if ( 'user' === $profiles_params['object'] && profiles_is_active( 'xprofile' ) ) {
	...
	} elseif ( 'group' === $profiles_params['object'] && profiles_is_active( 'groups' ) ) {
		$profiles_params['upload_dir_filter'] = 'groups_avatar_upload_dir';

		if ( ! profiles_get_current_group_id() && ! empty( $profiles_params['item_id'] ) ) {
		 ...
		 */
		$profiles_params = apply_filters( 'profiles_core_avatar_ajax_upload_params', $profiles_params );
	}
	...
	// Upload the avatar.
	$avatar = profiles_core_avatar_handle_upload( $_FILES, $profiles_params['upload_dir_filter'] );
	...
	if ( empty( $avatar ) ) {
		...
		// Upload error reply.
		profiles_attachments_json_response( false, $is_html4, array(
			'type'    => 'upload_error',
			'message' => $message,
		) );
	}
	...
	profiles_attachments_json_response( true, $is_html4, array(
		'name'      => $name,
...
add_action( 'wp_ajax_profiles_avatar_upload', 'profiles_avatar_ajax_upload' );
  ...
  * @return bool True on success, false on failure.
  */
function profiles_avatar_handle_capture( $data = '', $item_id = 0 ) {
	 ...
	 * @param int    $item_id Item to associate.
	 */
	if ( ! apply_filters( 'profiles_avatar_pre_handle_capture', true, $data, $item_id ) ) {
		return true;
	}

	$avatar_dir = profiles_core_avatar_upload_path() . '/avatars';
	 ...
	 */
	$avatar_folder_dir = apply_filters( 'profiles_core_avatar_folder_dir', $avatar_dir . '/' . $item_id, $item_id, 'user', 'avatars' );
	...
	if ( file_put_contents( $original_file, $data ) ) {
		$avatar_to_crop = str_replace( profiles_core_avatar_upload_path(), '', $original_file );

		// Crop to default values.
		$crop_args = array( 'item_id' => $item_id, 'original_file' => $avatar_to_crop, 'crop_x' => 0, 'crop_y' => 0 );

		return profiles_core_avatar_handle_crop( $crop_args );
	} else {
		return false;
	}
}
 ...
 *                                      as retrieved by profiles_core_avatar_full_width().
 *     @type int         $crop_h        Crop height. Default: the global 'full' avatar height,
 *                                      as retrieved by profiles_core_avatar_full_height().
 ...
 * @return bool True on success, false on failure.
 */
function profiles_core_avatar_handle_crop( $args = '' ) {
	...
	$r = wp_parse_args( $args, array(
		...
		'item_id'       => false,
		'original_file' => false,
		'crop_w'        => profiles_core_avatar_full_width(),
		'crop_h'        => profiles_core_avatar_full_height(),
		'crop_x'        => 0,
		'crop_y'        => 0
	) );
	 ...
	 */
	if ( ! apply_filters( 'profiles_core_pre_avatar_handle_crop', true, $r ) ) {
		return true;
	}
 ...
 *                     error message otherwise.
 */
function profiles_avatar_ajax_set() {
	// Bail if not a POST action.
	if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) ) {
		wp_send_json_error();
	}
	...
	// Check the nonce.
	check_admin_referer( 'profiles_avatar_cropstore', 'nonce' );
	...
	if ( ! empty( $avatar_data['type'] ) && 'camera' === $avatar_data['type'] && 'user' === $avatar_data['object'] ) {
		...
		if ( ! profiles_avatar_handle_capture( $webcam_avatar, $avatar_data['item_id'] ) ) {
			wp_send_json_error( array(
				'feedback_code' => 1
			) );
	...
	$original_file = str_replace( profiles_core_avatar_url(), '', $avatar_data['original_file'] );
	...
	// Handle crop.
	if ( profiles_core_avatar_handle_crop( $r ) ) {
		$return = array(
			'avatar' => html_entity_decode( profiles_core_fetch_avatar( array(
				'object'  => $avatar_data['object'],
				'item_id' => $avatar_data['item_id'],
...
add_action( 'wp_ajax_profiles_avatar_set', 'profiles_avatar_ajax_set' );
 ...
 */
function profiles_core_fetch_avatar_filter( $avatar, $user, $size, $default, $alt = '', $args = array() ) {
	global $pagenow;
	...
	// Image alt tag.
	if ( empty( $alt ) ) {
		$alt = sprintf( __( 'Profile photo of %s', 'profiles' ), profiles_core_get_user_displayname( $id ) );
	}

	// Use the 'thumb' type, unless the requested width is bigger than
	// BP's thumb width.
	$type = 'thumb';
	if ( (int) $size > profiles_core_avatar_thumb_width() ) {
		$type = 'full';
	}
 ...
 */
function profiles_core_check_avatar_upload( $file ) {
 ...
 */
function profiles_core_check_avatar_size( $file ) {
	if ( $file['file']['size'] > profiles_core_avatar_original_max_filesize() )
		return false;
 ...
 * @return array
 */
function profiles_core_get_allowed_avatar_types() {
	$allowed_types = profiles_attachments_get_allowed_types( 'avatar' );
	 ...
	 */
	$avatar_types = (array) apply_filters( 'profiles_core_get_allowed_avatar_types', $allowed_types );
 ...
 * @return array
 */
function profiles_core_get_allowed_avatar_mimes() {
	$allowed_types  = profiles_core_get_allowed_avatar_types();

	return profiles_attachments_get_allowed_mimes( 'avatar', $allowed_types );
}
 ...
 */
function profiles_core_check_avatar_type( $file ) {
	return profiles_attachments_check_filetype( $file['file']['tmp_name'], $file['file']['name'], profiles_core_get_allowed_avatar_mimes() );
}
 ...
 */
function profiles_core_get_upload_dir( $type = 'upload_path' ) {
	$profiles = profiles();

	switch ( $type ) {
		case 'upload_path' :
			$constant = 'Profiles_AVATAR_UPLOAD_PATH';
			$key      = 'basedir';

			break;

		case 'url' :
			$constant = 'Profiles_AVATAR_URL';
			$key      = 'baseurl';
 ...
 */
function profiles_core_avatar_upload_path() {
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_upload_path', profiles_core_get_upload_dir() );
}
 ...
 */
function profiles_core_avatar_url() {
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_url', profiles_core_get_upload_dir( 'url' ) );
}
 ...
 */
function profiles_get_user_has_avatar( $user_id = 0 ) {

	if ( empty( $user_id ) )
		$user_id = profiles_displayed_user_id();

	$retval = false;
	if ( profiles_core_fetch_avatar( array( 'item_id' => $user_id, 'no_grav' => true, 'html' => false, 'type' => 'full' ) ) != profiles_core_avatar_default( 'local' ) )
		$retval = true;
	 ...
	 */
	return (bool) apply_filters( 'profiles_get_user_has_avatar', $retval, $user_id );
}
 ...
 */
function profiles_core_avatar_dimension( $type = 'thumb', $h_or_w = 'height' ) {
	$profiles  = profiles();
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_dimension', $dim, $type, $h_or_w );
}
 ...
 */
function profiles_core_avatar_thumb_width() {
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_thumb_width', profiles_core_avatar_dimension( 'thumb', 'width' ) );
}
 ...
 */
function profiles_core_avatar_thumb_height() {
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_thumb_height', profiles_core_avatar_dimension( 'thumb', 'height' ) );
}
 ...
 */
function profiles_core_avatar_full_width() {
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_full_width', profiles_core_avatar_dimension( 'full', 'width' ) );
}
 ...
 */
function profiles_core_avatar_full_height() {
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_full_height', profiles_core_avatar_dimension( 'full', 'height' ) );
}
 ...
 */
function profiles_core_avatar_original_max_width() {
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_original_max_width', (int) profiles()->avatar->original_max_width );
}
 ...
 */
function profiles_core_avatar_original_max_filesize() {
	 ...
	 */
	return apply_filters( 'profiles_core_avatar_original_max_filesize', (int) profiles()->avatar->original_max_filesize );
}
 ...
 * @param array  $params Parameters passed to profiles_core_fetch_avatar().
 * @return string The URL of the default avatar.
 */
function profiles_core_avatar_default( $type = 'gravatar', $params = array() ) {
	// Local override.
	if ( defined( 'Profiles_AVATAR_DEFAULT' ) ) {
		$avatar = Profiles_AVATAR_DEFAULT;

	// Use the local default image.
	} elseif ( 'local' === $type ) {
		$size = '';
		if (
			( isset( $params['type'] ) && 'thumb' === $params['type'] && profiles_core_avatar_thumb_width() <= 50 ) ||
		...
		$avatar = profiles()->plugin_url . "profiles-core/images/mystery-man{$size}.jpg";

	// Use Gravatar's mystery person as fallback.
	} else {
		$size = '';
		if ( isset( $params['type'] ) && 'thumb' === $params['type'] ) {
			$size = profiles_core_avatar_thumb_width();
		} else {
	 ...
	 * @param array  $params Params provided to profiles_core_fetch_avatar().
	 */
	return apply_filters( 'profiles_core_avatar_default', $avatar, $params );
}

/**
 * Get the URL of the 'thumb' default avatar.
 *
 * Uses Gravatar's mystery-person avatar, unless Profiles_AVATAR_DEFAULT_THUMB has been
 * defined.
 *
 * @since 1.5.0
 ...
 */
function profiles_core_avatar_default_thumb( $type = 'gravatar', $params = array() ) {
	// Local override.
	if ( defined( 'Profiles_AVATAR_DEFAULT_THUMB' ) ) {
		$avatar = Profiles_AVATAR_DEFAULT_THUMB;

	// Use the local default image.
	} elseif ( 'local' === $type ) {
		$avatar = profiles()->plugin_url . 'profiles-core/images/mystery-man-50.jpg';

	// Use Gravatar's mystery person as fallback.
	} else {
		$avatar = '//www.gravatar.com/avatar/00000000000000000000000000000000?d=mm&amp;s=' . profiles_core_avatar_thumb_width();
	}
	 ...
	 *
	 * @param string $avatar URL of the default avatar.
	 * @param string $params Params provided to profiles_core_fetch_avatar().
	 */
	return apply_filters( 'profiles_core_avatar_thumb', $avatar, $params );
}
 ...
 */
function profiles_core_avatar_reset_query( $posts_query = null ) {
	$reset_w = false;

	// Group's avatar edit screen.
	if ( profiles_is_group_admin_page() ) {
		$reset_w = profiles_is_group_admin_screen( 'group-avatar' );

	// Group's avatar create screen.
	} elseif ( profiles_is_group_create() ) {
		/**
		 * We can't use profiles_get_groups_current_create_step().
		 * as it's not set yet
		 */
		$reset_w = 'group-avatar' === profiles_action_variable( 1 );

	// User's change avatar screen.
	} else {
		$reset_w = profiles_is_user_change_avatar();
	}
...
add_action( 'profiles_parse_query', 'profiles_core_avatar_reset_query', 10, 1 );
 ...
 */
function profiles_avatar_is_front_edit() {
	$retval = false;

	// No need to carry on if the current WordPress version is not supported.
	if ( ! profiles_attachments_is_wp_version_supported() ) {
		return $retval;
	}

	if ( profiles_is_user_change_avatar() && 'crop-image' !== profiles_get_avatar_admin_step() ) {
		$retval = ! profiles_core_get_root_option( 'profiles-disable-avatar-uploads' );
	}

	if ( profiles_is_active( 'groups' ) ) {
		// Group creation.
		if ( profiles_is_group_create() && profiles_is_group_creation_step( 'group-avatar' ) && 'crop-image' !== profiles_get_avatar_admin_step() ) {
			$retval = ! profiles_disable_group_avatar_uploads();

		// Group Manage.
		} elseif ( profiles_is_group_admin_page() && profiles_is_group_admin_screen( 'group-avatar' ) && 'crop-image' !== profiles_get_avatar_admin_step() ) {
			$retval = ! profiles_disable_group_avatar_uploads();
		}
	}
	 ...
	 */
	return apply_filters( 'profiles_avatar_is_front_edit', $retval );
}
 ...
 */
function profiles_avatar_use_webcam() {
	 ...
	 */
	return apply_filters( 'profiles_avatar_use_webcam', true );
}
 ...
 * @since 2.3.0
 */
function profiles_avatar_get_templates() {
	if ( ! profiles_avatar_is_front_edit() ) {
		return;
	}

	profiles_attachments_get_template_part( 'avatars/index' );
}
 ...
 * @since 2.3.0
 */
function profiles_avatar_template_check() {
	if ( ! profiles_avatar_is_front_edit() ) {
		return;
	}

	if ( ! did_action( 'profiles_attachments_avatar_check_template' ) ) {
		profiles_attachments_get_template_part( 'avatars/index' );
	}
}
