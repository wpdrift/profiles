 ...
 * @since 2.1.0
 */
class Profiles_Members_Suggestions extends Profiles_Suggestions {
	...
	public function validate() {
		 ...
		 * @param Profiles_Members_Suggestions $this Current Profiles_Members_Suggestions instance.
		 */
		$this->args                 = apply_filters( 'profiles_members_suggestions_args', $this->args, $this );

		// Check for invalid or missing mandatory parameters.
		if ( $this->args['only_friends'] && ( ! profiles_is_active( 'friends' ) || ! is_user_logged_in() ) ) {
		 ...
		 * @param Profiles_Members_Suggestions $this  Current Profiles_Members_Suggestions instance.
		 */
		return apply_filters( 'profiles_members_suggestions_validate_args', parent::validate(), $this );
	}
	...
	public function get_suggestions() {
		 ...
		 * @param Profiles_Members_Suggestions $this       Current Profiles_Members_Suggestions instance.
		 */
		$user_query = apply_filters( 'profiles_members_suggestions_query_args', $user_query, $this );
		if ( is_wp_error( $user_query ) ) {
			return $user_query;
		}


		$user_query = new Profiles_User_Query( $user_query );
		$results    = array();

		foreach ( $user_query->results as $user ) {
			$result        = new stdClass();
			$result->ID    = $user->user_nicename;
			$result->image = profiles_core_fetch_avatar( array( 'html' => false, 'item_id' => $user->ID ) );
			$result->name  = profiles_core_get_user_displayname( $user->ID );
		 ...
		 * @param Profiles_Members_Suggestions $this    Current Profiles_Members_Suggestions instance.
		 */
		return apply_filters( 'profiles_members_suggestions_get_suggestions', $results, $this );
	}
}
