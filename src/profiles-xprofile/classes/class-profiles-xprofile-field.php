...
class Profiles_XProfile_Field {
	 ...
	 *
	 * @since 2.0.0
	 * @var Profiles_XProfile_Field_Type Field type object used for validation.
	 */
	 ...
	 *
	 * @since 2.0.0
	 * @var Profiles_XProfile_ProfileData Field data for user ID.
	 */
	public $data;
	...
	public function __construct( $id = null, $user_id = null, $get_data = true ) {
		...
		// Initialise the type obj to prevent fatals when creating new profile fields.
			...
			$this->type_obj            = profiles_xprofile_create_field_type( 'textbox' );
	...
	public function populate( $id, $user_id = null, $get_data = true ) {
		...
		$field = wp_cache_get( $id, 'profiles_xprofile_fields' );
		if ( false === $field ) {
			$profiles = profiles();
			...
			wp_cache_add( $id, $field, 'profiles_xprofile_fields' );
		}
	...
	/**
	 * Retrieve a `Profiles_XProfile_Field` instance.
	 *
	 * @since 2.4.0
	 *
	 * @static
	 ...
	 * @return Profiles_XProfile_Field|false Field object if found, otherwise false.
	 */
	 ...
	 * @since 2.4.0
	 *
	 * @param array|object $args Array or object representing the `Profiles_XProfile_Field` properties.
	...
	public function fill_data( $args ) {
		...
		$this->type_obj            = profiles_xprofile_create_field_type( $this->type );
		$this->type_obj->field_obj = $this;
	}
	...
	public function delete( $delete_data = false ) {
		...
		if ( true === $delete_data ) {
			Profiles_XProfile_ProfileData::delete_for_field( $this->id );
		}
	...
	public function save() {
		 ...
		 * @since 1.0.0
		 *
		 * @param Profiles_XProfile_Field $this Current instance of the field being saved.
		 */
		...
		if ( $wpdb->query( $sql ) !== null ) {
			...
			if ( $this->type_obj->supports_options ) {
				 ...
				 * @param Profiles_XProfile_Field $type        Current field type being saved for.
				 */
	...
	public function get_field_data( $user_id = 0 ) {
		return new Profiles_XProfile_ProfileData( $this->id, $user_id );
	}
	...
	public function get_children( $for_editing = false ) {
		 ...
		 * @param bool   $for_editing Whether or not the field is for editing.
		 */
		return apply_filters( 'profiles_xprofile_field_get_children', $children, $for_editing );
	}
	...
	public function get_member_types() {
		...
		$raw_types = profiles_xprofile_get_meta( $this->id, 'field', 'member_type', false );
		...
		if ( ! in_array( '_none', $raw_types ) ) {
			$registered_types = profiles_get_member_types();
		 ...
		 */
		$this->member_types = apply_filters( 'profiles_xprofile_field_member_types', $types, $this );
	...
	public function set_member_types( $member_types, $append = false ) {
		...
		foreach ( $member_types as $member_type ) {
			// 'null' is a special case - it represents users without a type.
			if ( 'null' === $member_type || profiles_get_member_type_object( $member_type ) ) {
				$types[] = $member_type;
			}
		}
		...
		if ( ! $append ) {
			profiles_xprofile_delete_meta( $this->id, 'field', 'member_type' );
			 ...
			 */
			if ( empty( $types ) ) {
				profiles_xprofile_add_meta( $this->id, 'field', 'member_type', '_none' );
			}
		}
		 ...
		 */
		$_rtypes  = profiles_get_member_types();
		...
		if ( $types !== $rtypes ) {
			...
			foreach ( $types as $type ) {
				profiles_xprofile_add_meta( $this->id, 'field', 'member_type', $type );
			}
		}
		 ...
		 * @since 2.4.0
		 *
		 * @param Profiles_XProfile_Field $this Field object.
		 */
		do_action( 'profiles_xprofile_field_set_member_type', $this );
	...
	public function get_member_type_label() {
		...
		$all_types = profiles_get_member_types();
		if ( empty( $all_types ) ) {
			return '';
		}
		...
		if ( ! empty( $member_types ) ) {
			...
			foreach ( $member_types as $member_type ) {
				if ( 'null' === $member_type ) {
					...
					$mt_obj = profiles_get_member_type_object( $member_type );
			...
			if ( $has_null ) {
				$member_type_labels[] = __( 'Users with no member type', 'profiles' );
			}

			$label = sprintf( __( '(Member types: %s)', 'profiles' ), implode( ', ', array_map( 'esc_html', $member_type_labels ) ) );
		} else {
			$label = '<span class="member-type-none-notice">' . __( '(Unavailable to all members)', 'profiles' ) . '</span>';
		}
	...
	public function get_default_visibility() {
		if ( ! isset( $this->default_visibility ) ) {
			$this->default_visibility = profiles_xprofile_get_meta( $this->id, 'field', 'default_visibility' );
	...
	public function get_allow_custom_visibility() {
		if ( ! isset( $this->allow_custom_visibility ) ) {
			$allow_custom_visibility = profiles_xprofile_get_meta( $this->id, 'field', 'allow_custom_visibility' );
	...
	public function get_do_autolink() {
		if ( ! isset( $this->do_autolink ) ) {
			$do_autolink = profiles_xprofile_get_meta( $this->id, 'field', 'do_autolink' );
	...
	public static function update_position( $field_id, $position = null, $field_group_id = null ) {
		...
		$table_name = profiles()->profile->table_name_fields;
	...
	public static function get_fields_for_member_type( $member_types ) {
		...
		$mt_meta = wp_cache_get( 'field_member_types', 'profiles_xprofile' );
		if ( false === $mt_meta ) {
			...
			wp_cache_set( 'field_member_types', $mt_meta, 'profiles_xprofile' );
		}
		...
		if ( ! in_array( '_none', $member_types ) ) {
			...
			$all_member_types   = profiles_get_member_types();
	...
	public static function admin_validate() {
		...
		if ( ! isset( $_POST['title'] ) || ( '' === $_POST['title'] ) ) {
			$message = esc_html__( 'Profile fields must have a name.', 'profiles' );
			return false;
		}

		// Check field requirement.
		if ( ! isset( $_POST['required'] ) ) {
			$message = esc_html__( 'Profile field requirement is missing.', 'profiles' );
			return false;
		}

		// Check field type.
		if ( empty( $_POST['fieldtype'] ) ) {
			$message = esc_html__( 'Profile field type is missing.', 'profiles' );
			return false;
		}
		...
		$field_type = profiles_xprofile_create_field_type( $_POST['fieldtype'] );
		...
		if ( true === $field_type->supports_options ) {
			...
			if ( empty( $_POST[ $option_name ] ) || ! is_array( $_POST[ $option_name ] ) ) {
				$message = esc_html__( 'These field options are invalid.', 'profiles' );
				return false;
			}
			...
			// Check for missing or malformed options.
			if ( 0 === $field_count ) {
				$message = sprintf( esc_html__( '%s require at least one option.', 'profiles' ), $field_type->name );
				return false;
			}
	...
	public function render_admin_form( $message = '' ) {
		if ( empty( $this->id ) ) {
			$title  = __( 'Add New Field', 'profiles' );
			$action	= "users.php?page=profiles-profile-setup&amp;group_id=" . $this->group_id . "&amp;mode=add_field#tabs-" . $this->group_id;
			...
			$title  = __( 'Edit Field', 'profiles' );
			$action = "users.php?page=profiles-profile-setup&amp;mode=edit_field&amp;group_id=" . $this->group_id . "&amp;field_id=" . $this->id . "#tabs-" . $this->group_id;
		...
		<div class="wrap">
			...
			<form id="profiles-xprofile-add-field" action="<?php echo esc_url( $action ); ?>" method="post">
				<div id="poststuff">
					<div id="post-body" class="metabox-holder columns-<?php echo ( 1 == get_current_screen()->get_columns() ) ? '1' : '2'; ?>">
						...
						<div id="postbox-container-1" class="postbox-container">
							 ...
							 * @since 2.2.0
							 *
							 * @param Profiles_XProfile_Field $this Current XProfile field.
							 */
	...
	private function submit_metabox( $button_text = '' ) {
		...
		<div id="submitdiv" class="postbox">
			<h2><?php esc_html_e( 'Submit', 'profiles' ); ?></h2>
			<div class="inside">
				<div id="submitcomment" class="submitbox">
					<div id="major-publishing-actions">
						...
						<div id="delete-action">
							<a href="users.php?page=profiles-profile-setup" class="deletion"><?php esc_html_e( 'Cancel', 'profiles' ); ?></a>
						</div>
	...
	?>
		...
		<div id="titlediv">
			<div class="titlewrap">
				<label id="title-prompt-text" for="title"><?php echo esc_html_x( 'Name', 'XProfile admin edit field', 'profiles' ); ?></label>
				<input type="text" name="title" id="title" value="<?php echo esc_attr( $this->name ); ?>" autocomplete="off" />
			</div>
		</div>

		<div class="postbox">
			<h2><?php echo esc_html_x( 'Description', 'XProfile admin edit field', 'profiles' ); ?></h2>
			<div class="inside">
	...
	private function member_type_metabox() {
		...
		if ( ! $member_types = profiles_get_member_types( array(), 'objects' ) ) {
			return;
		}
		...
		<div id="member-types-div" class="postbox">
			<h2><?php _e( 'Member Types', 'profiles' ); ?></h2>
			<div class="inside">
				<p class="description"><?php _e( 'This field should be available to:', 'profiles' ); ?></p>
				...
				<ul>
					...
					<li>
						<label for="member-type-none">
							...
							<?php _e( 'Users with no member type', 'profiles' ); ?>
						</label>
					</li>

				</ul>
				<p class="description member-type-none-notice<?php if ( ! empty( $field_member_types ) ) : ?> hide<?php endif; ?>"><?php _e( 'Unavailable to all members.', 'profiles' ) ?></p>
			</div>
	...
	private function visibility_metabox() {
		...
		<div class="postbox">
			<h2><label for="default-visibility"><?php esc_html_e( 'Visibility', 'profiles' ); ?></label></h2>
			<div class="inside">
				<div>
				...
				<div>
					<ul>
						<li>
							...
							<label for="allow-custom-visibility-allowed"><?php esc_html_e( 'Allow members to override', 'profiles' ); ?></label>
						</li>
						<li>
							...
							<label for="allow-custom-visibility-disabled"><?php esc_html_e( 'Enforce field visibility', 'profiles' ); ?></label>
						</li>
					</ul>
				</div>
			</div>
	...
	private function required_metabox() {
		...
		<div class="postbox">
			<h2><label for="required"><?php esc_html_e( 'Requirement', 'profiles' ); ?></label></h2>
			<div class="inside">
				<select name="required" id="required">
					<option value="0"<?php selected( $this->is_required, '0' ); ?>><?php esc_html_e( 'Not Required', 'profiles' ); ?></option>
					<option value="1"<?php selected( $this->is_required, '1' ); ?>><?php esc_html_e( 'Required',     'profiles' ); ?></option>
				</select>
			</div>
		</div>
	...
	private function autolink_metabox() {
		...
		<div class="postbox">
			<h2><?php esc_html_e( 'Autolink', 'profiles' ); ?></h2>
			<div class="inside">
				<p class="description"><?php esc_html_e( 'On user profiles, link this field to a search of the Members directory, using the field value as a search term.', 'profiles' ); ?></p>

				<p>
					<label for="do-autolink" class="screen-reader-text"><?php
						/* translators: accessibility text */
						esc_html_e( 'Autolink status for this field', 'profiles' );
					?></label>
					<select name="do_autolink" id="do-autolink">
						<option value="on" <?php selected( $this->get_do_autolink() ); ?>><?php esc_html_e( 'Enabled', 'profiles' ); ?></option>
						<option value="" <?php selected( $this->get_do_autolink(), false ); ?>><?php esc_html_e( 'Disabled', 'profiles' ); ?></option>
					</select>
				</p>
			</div>
		</div>
	...
	private function type_metabox() {
		...
		<div class="postbox">
			<h2><label for="fieldtype"><?php esc_html_e( 'Type', 'profiles'); ?></label></h2>
			<div class="inside">
				...
				// Deprecated filter, don't use. Go look at {@link Profiles_XProfile_Field_Type::admin_new_field_html()}.
