 ...
 *
 */
function profiles_version() {
	echo profiles_get_version();
}
	/**
 ...
 *
 */
function profiles_db_version() {
	echo profiles_get_db_version();
}
	/**
	 ...
	 */
	function profiles_get_db_version() {
 ...
 *
 */
function profiles_db_version_raw() {
	echo profiles_get_db_version_raw();
}
	/**
	 * Return the Profiles database version.
	 *
	 ...
	 */
	function profiles_get_db_version_raw() {
		$profiles = profiles();
...
/**
 * Get the $wpdb base prefix, run through the 'profiles_core_get_table_prefix' filter.
 *
 ...
 */
function profiles_core_get_table_prefix() {
	global $wpdb;
	 ...
	 */
	return apply_filters( 'profiles_core_get_table_prefix', $wpdb->base_prefix );
}
 ...
 */
function profiles_sort_by_key( $items, $key, $type = 'alpha' ) {
	usort( $items, array( new Profiles_Core_Sort_By_Key_Callback( $key, $type ), 'sort_callback' ) );
 ...
 *     $sorted_posts = profiles_alpha_sort_by_key( $posts, 'post_name' );
 *
 * @since 1.9.0
 *
 * @param array      $items The items to be sorted. Its constituent items can be either associative arrays or objects.
 ...
 * @return array $items The sorted array.
 */
function profiles_alpha_sort_by_key( $items, $key ) {
	return profiles_sort_by_key( $items, $key, 'alpha' );
}
 ...
 */
function profiles_core_number_format( $number = 0, $decimals = false ) {
	 ...
	 */
	return apply_filters( 'profiles_core_number_format', number_format_i18n( $number, $decimals ), $number, $decimals );
}
 ...
 */
function profiles_core_parse_args_array( $old_args_keys, $func_args ) {
	$new_args = array();
 ...
 */
function profiles_parse_args( $args, $defaults = array(), $filter_key = '' ) {
	...
	if ( !empty( $filter_key ) ) {
		 ...
		 */
		$r = apply_filters( 'profiles_before_' . $filter_key . '_parse_args', $r );
	}
	...
	if ( !empty( $filter_key ) ) {
		 ...
		 */
		$r = apply_filters( 'profiles_after_' . $filter_key . '_parse_args', $r );
	}
 ...
 */
function profiles_sanitize_pagination_arg( $page_arg = '', $page = 1 ) {
	...
	if ( isset( $_REQUEST[ $page_arg ] ) ) {
		...
		// @see https://profiles.trac.wordpress.org/ticket/5796.
		if ( $int ) {
			$page = $int;
		}
	}
 ...
 */
function profiles_esc_sql_order( $order = '' ) {
 ...
 */
function profiles_esc_like( $text ) {
	global $wpdb;
 ...
 *              Default: false.
 */
function profiles_is_username_compatibility_mode() {
	 ...
	 */
	return apply_filters( 'profiles_is_username_compatibility_mode', defined( 'Profiles_ENABLE_USERNAME_COMPATIBILITY_MODE' ) && Profiles_ENABLE_USERNAME_COMPATIBILITY_MODE );
}
...
function profiles_use_wp_admin_bar() {
	 ...
	 */
	return (bool) apply_filters( 'profiles_use_wp_admin_bar', $use_admin_bar );
}
 ...
 * @return array
 */
function profiles_core_get_packaged_component_ids() {
	$components = array(
		'activity',
		'members',
		'groups',
	...
	if ( class_exists( 'Profiles_Forums_Component' ) ) {
		$components[] = 'forums';
	}
 ...
 */
function profiles_core_get_directory_page_ids( $status = 'active' ) {
	$page_ids = profiles_get_option( 'profiles-pages' );
	...
	if ( !empty( $page_ids ) && is_array( $page_ids ) ) {
		foreach( (array) $page_ids as $component_name => $page_id ) {
			...
			if ( 'active' === $status && ! profiles_is_active( $component_name ) ) {
				unset( $page_ids[ $component_name ] );
			}
		}
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_directory_page_ids', $page_ids );
}
 ...
 */
function profiles_core_get_directory_page_id( $component = null ) {
	if ( ! $component ) {
		$component = profiles_current_component();
	}

	$profiles_pages = profiles_core_get_directory_page_ids( 'all' );
 ...
 *
 * The profiles-pages data is stored in site_options (falls back to options on non-MS),
 * in an array keyed by blog_id. This allows you to change your
 * profiles_get_root_blog_id() and go through the setup process again.
 *
 * @since 1.5.0
 *
 ...
 */
function profiles_core_update_directory_page_ids( $blog_page_ids ) {
	profiles_update_option( 'profiles-pages', $blog_page_ids );
}
 ...
 */
function profiles_core_get_directory_pages() {
	global $wpdb;

	// Look in cache first.
	$pages = wp_cache_get( 'directory_pages', 'profiles_pages' );
	...
	if ( false === $pages ) {
		...
		// Get pages and IDs.
		$page_ids = profiles_core_get_directory_page_ids();
		if ( !empty( $page_ids ) ) {

			// Always get page data from the root blog, except on multiblog mode, when it comes
			// from the current blog.
			$posts_table_name = profiles_is_multiblog_mode() ? $wpdb->posts : $wpdb->get_blog_prefix( profiles_get_root_blog_id() ) . 'posts';
		...
		wp_cache_set( 'directory_pages', $pages, 'profiles_pages' );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_directory_pages', $pages );
}
 ...
 */
function profiles_core_add_page_mappings( $components, $existing = 'keep' ) {
	...
	$pages = profiles_core_get_directory_page_ids( 'all' );
	...
	$page_titles = array(
		'activity' => _x( 'Activity', 'Page title for the Activity directory.',       'profiles' ),
		'groups'   => _x( 'Groups',   'Page title for the Groups directory.',         'profiles' ),
		'blogs'    => _x( 'Sites',    'Page title for the Sites directory.',          'profiles' ),
		'members'  => _x( 'Members',  'Page title for the Members directory.',        'profiles' ),
		'activate' => _x( 'Activate', 'Page title for the user activation screen.',   'profiles' ),
		'register' => _x( 'Register', 'Page title for the user registration screen.', 'profiles' ),
	);
	...
	profiles_update_option( 'profiles-pages', $pages );
...
/**
 * Remove the entry from profiles_pages when the corresponding WP page is deleted.
 *
 * Bails early on multisite installations when not viewing the root site.
 *
 * @link https://profiles.trac.wordpress.org/ticket/6226
 *
 * @since 2.2.0
 *
 * @param int $post_id Post ID.
 */
function profiles_core_on_directory_page_delete( $post_id ) {
	...
	$page_ids       = profiles_core_get_directory_page_ids( 'all' );
	...
	profiles_core_update_directory_page_ids( $page_ids );
}
add_action( 'delete_post', 'profiles_core_on_directory_page_delete' );
 ...
 *
 * Filter on 'profiles_core_component_slug_from_root_slug' to override this method
 * in general, or define a specific component slug constant (e.g.
 * Profiles_MEMBERS_SLUG) to override specific component slugs.
 *
 * @since 1.5.0
 *
 ...
 */
function profiles_core_component_slug_from_root_slug( $root_slug ) {
	 ...
	 * @param string $root_slug The root slug which comes from $profiles->pages-[component]->slug.
	 */
	return apply_filters( 'profiles_core_component_slug_from_root_slug', $slug, $root_slug );
}
 ...
 */
function profiles_core_add_root_component( $slug ) {
	$profiles = profiles();
 ...
 * @since 1.5.0
 */
function profiles_core_create_root_component_page() {
	...
	$page_ids = array_merge( (array) $new_page_ids, (array) profiles_core_get_directory_page_ids( 'all' ) );
	profiles_core_update_directory_page_ids( $page_ids );
}
 ...
 * @todo Deprecate?
 */
function profiles_core_add_illegal_names() {
 ...
 */
function profiles_core_get_component_search_query_arg( $component ) {
	$query_arg = false;
	if ( isset( profiles()->{$component}->search_query_arg ) ) {
		$query_arg = sanitize_title( profiles()->{$component}->search_query_arg );
	}
	 ...
	 * @param string $component Component name.
	 */
	return apply_filters( 'profiles_core_get_component_search_query_arg', $query_arg, $component );
}

/**
 * Determine whether Profiles should register the profiles-themes directory.
 *
 * @since 1.9.0
 *
 * @return bool True if profiles-themes should be registered, false otherwise.
 */
function profiles_do_register_theme_directory() {
	// If profiles-default exists in another theme directory, bail.
	// This ensures that the version of profiles-default in the regular themes
	...
	foreach ( array_values( (array) $GLOBALS['wp_theme_directories'] ) as $directory ) {
		if ( is_dir( $directory . '/profiles-default' ) ) {
			return false;
		}
	}

	// If the current theme is profiles-default (or a profiles-default child), BP
	// should register its directory.
	$register = 'profiles-default' === get_stylesheet() || 'profiles-default' === get_template();

	// Legacy sites continue to have the theme registered.
	if ( empty( $register ) && ( 1 == get_site_option( '_profiles_retain_profiles_default' ) ) ) {
		$register = true;
	}

	/**
	 * Filters whether Profiles should register the profiles-themes directory.
	 *
	 * @since 1.9.0
	 *
	 * @param bool $register If profiles-themes should be registered.
	 */
	return apply_filters( 'profiles_do_register_theme_directory', $register );
}
 ...
 */
function profiles_core_get_root_domain() {

	$domain = get_home_url( profiles_get_root_blog_id() );
	 ...
	 */
	return apply_filters( 'profiles_core_get_root_domain', $domain );
}
 ...
 *                         headers. Default: 302.
 */
function profiles_core_redirect( $location = '', $status = 302 ) {
	...
	// have a valid URL.
	if ( empty( $location ) ) {
		$location = profiles_get_root_domain();
	}

	// Make sure we don't call status_header() in profiles_core_do_catch_uri() as this
	// conflicts with wp_redirect() and wp_safe_redirect().
	profiles()->no_status_set = true;
 ...
 */
function profiles_get_referer_path() {
 ...
 */
function profiles_core_get_site_path() {
	global $current_site;
	 ...
	 */
	return apply_filters( 'profiles_core_get_site_path', $site_path );
}
 ...
 */
function profiles_core_current_time( $gmt = true, $type = 'mysql' ) {
	 ...
	 */
	return apply_filters( 'profiles_core_current_time', current_time( $type, $gmt ) );
}
 ...
 */
function profiles_core_time_since( $older_date, $newer_date = false ) {
	 ...
	 */
	$pre_value = apply_filters( 'profiles_core_time_since_pre', false, $older_date, $newer_date );
	if ( false !== $pre_value ) {
		return $pre_value;
	}
	 ...
	 */
	$unknown_text   = apply_filters( 'profiles_core_time_since_unknown_text',   __( 'sometime',  'profiles' ) );
	 ...
	 * @param string $value String representing the time since the older date.
	 */
	$right_now_text = apply_filters( 'profiles_core_time_since_right_now_text', __( 'right now', 'profiles' ) );
	 ...
	 * @param string $value String representing the time since the older date.
	 */
	$ago_text       = apply_filters( 'profiles_core_time_since_ago_text',       __( '%s ago',    'profiles' ) );
	 ...
	 */
	$newer_date = ( !$newer_date ) ? profiles_core_current_time( true, 'timestamp' ) : $newer_date;
		...
		if ( !isset( $chunks[$i] ) ) {
			...
			switch ( $seconds ) {
				case YEAR_IN_SECONDS :
					$output = sprintf( _n( '%s year',   '%s years',   $count, 'profiles' ), $count );
					break;
				case 30 * DAY_IN_SECONDS :
					$output = sprintf( _n( '%s month',  '%s months',  $count, 'profiles' ), $count );
					break;
				case WEEK_IN_SECONDS :
					$output = sprintf( _n( '%s week',   '%s weeks',   $count, 'profiles' ), $count );
					break;
				case DAY_IN_SECONDS :
					$output = sprintf( _n( '%s day',    '%s days',    $count, 'profiles' ), $count );
					break;
				case HOUR_IN_SECONDS :
					$output = sprintf( _n( '%s hour',   '%s hours',   $count, 'profiles' ), $count );
					break;
				case MINUTE_IN_SECONDS :
					$output = sprintf( _n( '%s minute', '%s minutes', $count, 'profiles' ), $count );
					break;
				default:
					$output = sprintf( _n( '%s second', '%s seconds', $count, 'profiles' ), $count );
			}
			...
			if ( $i + 2 < $j ) {
				...
				// Add to output var.
				if ( 0 != $count2 ) {
					$output .= _x( ',', 'Separator in time since', 'profiles' ) . ' ';

					switch ( $seconds2 ) {
						case 30 * DAY_IN_SECONDS :
							$output .= sprintf( _n( '%s month',  '%s months',  $count2, 'profiles' ), $count2 );
							break;
						case WEEK_IN_SECONDS :
							$output .= sprintf( _n( '%s week',   '%s weeks',   $count2, 'profiles' ), $count2 );
							break;
						case DAY_IN_SECONDS :
							$output .= sprintf( _n( '%s day',    '%s days',    $count2, 'profiles' ), $count2 );
							break;
						case HOUR_IN_SECONDS :
							$output .= sprintf( _n( '%s hour',   '%s hours',   $count2, 'profiles' ), $count2 );
							break;
						case MINUTE_IN_SECONDS :
							$output .= sprintf( _n( '%s minute', '%s minutes', $count2, 'profiles' ), $count2 );
							break;
						default:
							$output .= sprintf( _n( '%s second', '%s seconds', $count2, 'profiles' ), $count2 );
					}
				}
			}
	 ...
	 * @param string $newer_date Unix timestamp of date to compare older time to.
	 */
	return apply_filters( 'profiles_core_time_since', $output, $older_date, $newer_date );
}
 ...
 *                        Default: 'success'.
 */
function profiles_core_add_message( $message, $type = '' ) {
	...
	@setcookie( 'profiles-message',      $message, time() + 60 * 60 * 24, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
	@setcookie( 'profiles-message-type', $type,    time() + 60 * 60 * 24, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
 ...
 *
 */
function profiles_core_setup_message() {

	// Get Profiles.
	$profiles = profiles();

	if ( empty( $profiles->template_message ) && isset( $_COOKIE['profiles-message'] ) ) {
		$profiles->template_message = stripslashes( $_COOKIE['profiles-message'] );
	}

	if ( empty( $profiles->template_message_type ) && isset( $_COOKIE['profiles-message-type'] ) ) {
		$profiles->template_message_type = stripslashes( $_COOKIE['profiles-message-type'] );
	}

	add_action( 'template_notices', 'profiles_core_render_message' );

	if ( isset( $_COOKIE['profiles-message'] ) ) {
		@setcookie( 'profiles-message', false, time() - 1000, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
	}

	if ( isset( $_COOKIE['profiles-message-type'] ) ) {
		@setcookie( 'profiles-message-type', false, time() - 1000, COOKIEPATH, COOKIE_DOMAIN, is_ssl() );
	}
}
add_action( 'profiles_actions', 'profiles_core_setup_message', 5 );
 ...
 * @since 1.1.0
 */
function profiles_core_render_message() {
	...
	if ( !empty( $profiles->template_message ) ) :
		 ...
		 */
		$content = apply_filters( 'profiles_core_render_message_content', $profiles->template_message, $type ); ?>

		<div id="message" class="profiles-template-notice <?php echo esc_attr( $type ); ?>">
	...
	<?php
		 ...
		 * @since 1.1.0
		 */
		do_action( 'profiles_core_render_message' );
 ...
 */
function profiles_core_record_activity() {
	...
	// Get the user ID.
	$user_id = profiles_loggedin_user_id();

	// Bail if user is not active.
	if ( profiles_is_user_inactive( $user_id ) ) {
		return false;
	}

	// Get the user's last activity.
	$activity = profiles_get_user_last_activity( $user_id );
	...
	// Get current time.
	$current_time = profiles_core_current_time();
	...
	if ( empty( $activity ) ) {
		 ...
		 */
		do_action( 'profiles_first_activity_for_member', $user_id );
	}
	...
	if ( empty( $activity ) || ( strtotime( $current_time ) >= strtotime( '+5 minutes', $activity ) ) ) {
		profiles_update_user_last_activity( $user_id, $current_time );
	}
}
add_action( 'wp_head', 'profiles_core_record_activity' );
 ...
 */
function profiles_core_get_last_activity( $last_activity_date = '', $string = '' ) {
	...
	$last_active = empty( $last_activity_date )
		? __( 'Not recently active', 'profiles' )
		: sprintf( $string, profiles_core_time_since( $last_activity_date ) );
	 ...
	 * @param string $string             A sprintf()-able statement of the form 'active %s'.
	 */
	return apply_filters( 'profiles_core_get_last_activity', $last_active, $last_activity_date, $string );
}
 ...
 * profiles_get_user_meta_key():
 *    $friend_count = profiles_get_user_meta( $user_id, 'total_friend_count', true );
 ...
 *    $friend_count = get_user_meta( $user_id, profiles_get_user_meta_key( 'total_friend_count' ), true );
 ...
 */
function profiles_get_user_meta_key( $key = false ) {
	 ...
	 */
	return apply_filters( 'profiles_get_user_meta_key', $key );
}
 ...
 * profiles_get_user_meta_key(), thereby increasing compatibility with non-standard
 * BP setups.
 *
 * @since 1.5.0
 *
 ...
 *               is true.
 */
function profiles_get_user_meta( $user_id, $key, $single = false ) {
	return get_user_meta( $user_id, profiles_get_user_meta_key( $key ), $single );
}
 ...
 */
function profiles_update_user_meta( $user_id, $key, $value, $prev_value = '' ) {
	return update_user_meta( $user_id, profiles_get_user_meta_key( $key ), $value, $prev_value );
}
 ...
 */
function profiles_delete_user_meta( $user_id, $key, $value = '' ) {
	return delete_user_meta( $user_id, profiles_get_user_meta_key( $key ), $value );
}

/** Embeds ********************************************************************/

/**
 * Initializes {@link Profiles_Embed} after everything is loaded.
 *
 * @since 1.5.0
 */
function profiles_embed_init() {

	// Get Profiles.
	$profiles = profiles();

	if ( empty( $profiles->embed ) ) {
		$profiles->embed = new Profiles_Embed();
	}
}
add_action( 'profiles_init', 'profiles_embed_init', 9 );
 ...
 */
function profiles_use_embed_in_activity() {
	 ...
	 */
	return apply_filters( 'profiles_use_oembed_in_activity', !defined( 'Profiles_EMBED_DISABLE_ACTIVITY' ) || !Profiles_EMBED_DISABLE_ACTIVITY );
}
 ...
 */
function profiles_use_embed_in_activity_replies() {
	 ...
	 * @param bool $value Whether or not oEmbeds are allowed.
	 */
	return apply_filters( 'profiles_use_embed_in_activity_replies', !defined( 'Profiles_EMBED_DISABLE_ACTIVITY_REPLIES' ) || !Profiles_EMBED_DISABLE_ACTIVITY_REPLIES );
}
 ...
 *              enabled. Default: true.
 */
function profiles_use_embed_in_forum_posts() {
	 ...
	 * @param bool $value Whether or not oEmbeds are allowed.
	 */
	return apply_filters( 'profiles_use_embed_in_forum_posts', !defined( 'Profiles_EMBED_DISABLE_FORUM_POSTS' ) || !Profiles_EMBED_DISABLE_FORUM_POSTS );
}
 ...
 *              enabled. Default: true.
 */
function profiles_use_embed_in_private_messages() {
	 ...
	 * @param bool $value Whether or not oEmbeds are allowed.
	 */
	return apply_filters( 'profiles_use_embed_in_private_messages', !defined( 'Profiles_EMBED_DISABLE_PRIVATE_MESSAGES' ) || !Profiles_EMBED_DISABLE_PRIVATE_MESSAGES );
}
 ...
 *                             Profiles_Media_Extractor class for more info.
 * @return array|bool          If media exists, will return array of media metadata. Else, boolean false.
 */
function profiles_core_extract_media_from_content( $content = '', $type = 'all' ) {
	if ( is_string( $type ) ) {
		$class = new ReflectionClass( 'Profiles_Media_Extractor' );
	...
	$x = new Profiles_Media_Extractor;
 ...
 * @since 1.5.0
 *
 * @see profiles_get_admin_url() For description of parameters.
 *
 * @param string $path   See {@link profiles_get_admin_url()}.
 * @param string $scheme See {@link profiles_get_admin_url()}.
 */
function profiles_admin_url( $path = '', $scheme = 'admin' ) {
	echo esc_url( profiles_get_admin_url( $path, $scheme ) );
}
	/**
	 ...
	 */
	function profiles_get_admin_url( $path = '', $scheme = 'admin' ) {
 ...
 *              otherwise false.
 */
function profiles_core_do_network_admin() {

	// Default.
	$retval = profiles_is_network_activated();

	if ( profiles_is_multiblog_mode() ) {
		$retval = false;
	}
	 ...
	 */
	return (bool) apply_filters( 'profiles_core_do_network_admin', $retval );
}
 ...
 * functions as add_submenu_page()) should use profiles_core_admin_hook() for the
 ...
 *
 *     add_action( profiles_core_admin_hook(), 'myplugin_dashboard_panel_setup' );
 *
 * @since 1.5.0
 *
 * @return string $hook The proper hook ('network_admin_menu' or 'admin_menu').
 */
function profiles_core_admin_hook() {
	$hook = profiles_core_do_network_admin() ? 'network_admin_menu' : 'admin_menu';
	 ...
	 */
	return apply_filters( 'profiles_core_admin_hook', $hook );
}
 ...
 * @return bool $is_root_blog Returns true if this is profiles_get_root_blog_id().
 */
function profiles_is_root_blog( $blog_id = 0 ) {
	...
	if ( profiles_get_root_blog_id() === $blog_id ) {
		$is_root_blog = true;
	}
	 ...
	 */
	return (bool) apply_filters( 'profiles_is_root_blog', (bool) $is_root_blog );
}
 ...
 */
function profiles_get_root_blog_id() {
	 ...
	 */
	return (int) apply_filters( 'profiles_get_root_blog_id', (int) profiles()->root_blog_id );
}

/**
 * Are we running multiblog mode?
 *
 * Note that Profiles_ENABLE_MULTIBLOG is different from (but dependent on) WordPress
 ...
 * Profiles_ENABLE_MULTIBLOG constant beyond 1.7, as Profiles can now be activated
 * on individual sites.
 *
 ...
 *              Default: false.
 */
function profiles_is_multiblog_mode() {

	// Setup some default values.
	$retval         = false;
	$is_multisite   = is_multisite();
	$network_active = profiles_is_network_activated();
	$is_multiblog   = defined( 'Profiles_ENABLE_MULTIBLOG' ) && Profiles_ENABLE_MULTIBLOG;
	 ...
	 */
	return apply_filters( 'profiles_is_multiblog_mode', $retval );
}
 ...
 * 'profiles_is_network_activated' and override the auto-determined value.
 *
 * @since 1.7.0
 *
 * @return bool True if Profiles is network activated.
 */
function profiles_is_network_activated() {
	 ...
	 */
	return (bool) apply_filters( 'profiles_is_network_activated', $retval );
}
 ...
 *                             component.
 */
function profiles_update_is_directory( $is_directory = false, $component = '' ) {
	 ...
	 */
	profiles()->is_directory = apply_filters( 'profiles_update_is_directory', $is_directory, $component );
}
 ...
 *                              component.
 */
function profiles_update_is_item_admin( $is_item_admin = false, $component = '' ) {
	 ...
	 * @param string $component     Component name. Default: the current component.
	 */
	profiles()->is_item_admin = apply_filters( 'profiles_update_is_item_admin', $is_item_admin, $component );
}
 ...
 *                            component.
 */
function profiles_update_is_item_mod( $is_item_mod = false, $component = '' ) {
	 ...
	 * @param string $component   Component name. Default: the current component.
	 */
	profiles()->is_item_mod = apply_filters( 'profiles_update_is_item_mod', $is_item_mod, $component );
}
 ...
 */
function profiles_do_404( $redirect = 'remove_canonical_direct' ) {
	global $wp_query;
	 ...
	 */
	do_action( 'profiles_do_404', $redirect );
 ...
 */
function profiles_verify_nonce_request( $action = '', $query_arg = '_wpnonce' ) {
	 ...
	 */
	$matched_url = apply_filters( 'profiles_verify_nonce_request_url', $requested_url );
	 ...
	 */
	do_action( 'profiles_verify_nonce_request', $action, $result );
 ...
 * @return bool
 */
function profiles_is_post_request() {
 ...
 * @return bool
 */
function profiles_is_get_request() {
...
/**
 * Load the profiles translation file for current language.
 *
 * @since 1.0.2
 *
 ...
 */
function profiles_core_load_profiles_textdomain() {
	$domain = 'profiles';
	 ...
	 */
	$mofile_custom = sprintf( '%s-%s.mo', $domain, apply_filters( 'profiles_locale', get_locale() ) );
	 ...
	 */
	$locations = apply_filters( 'profiles_locale_locations', array(
	...
	foreach ( $locations as $location ) {
		if ( load_textdomain( 'profiles', $location . $mofile_custom ) ) {
			return true;
		}
	}
...
add_action( 'profiles_core_loaded', 'profiles_core_load_profiles_textdomain' );
 ...
 */
function profiles_core_action_search_site( $slug = '' ) {

	if ( ! profiles_is_current_component( profiles_get_search_slug() ) ) {
		return;
	}

	if ( empty( $_POST['search-terms'] ) ) {
		profiles_core_redirect( profiles_get_root_domain() );
		return;
	}
	...
	if ( empty( $slug ) ) {
		switch ( $search_which ) {
			...
			case 'blogs':
				$slug = profiles_is_active( 'blogs' )  ? profiles_get_blogs_root_slug()  : '';
				break;

			case 'forums':
				$slug = profiles_is_active( 'forums' ) ? profiles_get_forums_root_slug() : '';
				$query_string = '/?fs=';
				break;

			case 'groups':
				$slug = profiles_is_active( 'groups' ) ? profiles_get_groups_root_slug() : '';
				break;

			case 'members':
			default:
				$slug = profiles_get_members_root_slug();
				break;
		}
	 ...
	 */
	profiles_core_redirect( apply_filters( 'profiles_core_search_site', home_url( $slug . $query_string . urlencode( $search_terms ) ), $search_terms ) );
}
add_action( 'profiles_init', 'profiles_core_action_search_site', 7 );
 ...
 * @since 2.1.0
 */
function profiles_remove_adjacent_posts_rel_link() {
	if ( ! is_profiles() ) {
		return;
	}
...
add_action( 'profiles_init', 'profiles_remove_adjacent_posts_rel_link' );
 ...
 * @return string
 */
function _profiles_strip_spans_from_title( $title_part = '' ) {
	$title = $title_part;
 ...
 * @return string
 */
function profiles_core_get_minified_asset_suffix() {
 ...
 */
function profiles_core_get_components( $type = 'all' ) {
	$required_components = array(
		'core' => array(
			'title'       => __( 'Profiles Core', 'profiles' ),
			'description' => __( 'It&#8216;s what makes <del>time travel</del> Profiles possible!', 'profiles' )
		),
		'members' => array(
			'title'       => __( 'Community Members', 'profiles' ),
			'description' => __( 'Everything in a Profiles community revolves around its members.', 'profiles' )
		),
	);

	$retired_components = array(
		'forums' => array(
			'title'       => __( 'Group Forums', 'profiles' ),
			'description' => sprintf( __( 'Profiles Forums are retired. Use %s.', 'profiles' ), '<a href="https://bprofilesress.org/">bbPress</a>' )
		),
	);

	$optional_components = array(
		'xprofile' => array(
			'title'       => __( 'Extended Profiles', 'profiles' ),
			'description' => __( 'Customize your community with fully editable profile fields that allow your users to describe themselves.', 'profiles' )
		),
		'settings' => array(
			'title'       => __( 'Account Settings', 'profiles' ),
			'description' => __( 'Allow your users to modify their account and notification settings directly from within their profiles.', 'profiles' )
		),
		'friends'  => array(
			'title'       => __( 'Friend Connections', 'profiles' ),
			'description' => __( 'Let your users make connections so they can track the activity of others and focus on the people they care about the most.', 'profiles' )
		),
		'messages' => array(
			'title'       => __( 'Private Messaging', 'profiles' ),
			'description' => __( 'Allow your users to talk to each other directly and in private. Not just limited to one-on-one discussions, messages can be sent between any number of members.', 'profiles' )
		),
		'activity' => array(
			'title'       => __( 'Activity Streams', 'profiles' ),
			'description' => __( 'Global, personal, and group activity streams with threaded commenting, direct posting, favoriting, and @mentions, all with full RSS feed and email notification support.', 'profiles' )
		),
		'notifications' => array(
			'title'       => __( 'Notifications', 'profiles' ),
			'description' => __( 'Notify members of relevant activity with a toolbar bubble and/or via email, and allow them to customize their notification settings.', 'profiles' )
		),
		'groups'   => array(
			'title'       => __( 'User Groups', 'profiles' ),
			'description' => __( 'Groups allow your users to organize themselves into specific public, private or hidden sections with separate activity streams and member listings.', 'profiles' )
		),
		'forums'   => array(
			'title'       => __( 'Group Forums (Legacy)', 'profiles' ),
			'description' => __( 'Group forums allow for focused, bulletin-board style conversations.', 'profiles' )
		),
		'blogs'    => array(
			'title'       => __( 'Site Tracking', 'profiles' ),
			'description' => __( 'Record activity for new posts and comments from your site.', 'profiles' )
		)
	);

	// Add blogs tracking if multisite.
	if ( is_multisite() ) {
		$optional_components['blogs']['description'] = __( 'Record activity for new sites, posts, and comments across your network.', 'profiles' );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_components', $components, $type );
}
 ...
 */
function profiles_nav_menu_get_loggedin_pages() {

	// Try to catch the cached version first.
	if ( ! empty( profiles()->wp_nav_menu_items->loggedin ) ) {
		return profiles()->wp_nav_menu_items->loggedin;
	}
	...
	// Some BP nav menu items will not be represented in profiles_nav, because
	...
	foreach ( $profiles_menu_items as $profiles_item ) {
		...
		$item_name = _profiles_strip_spans_from_title( $profiles_item['name'] );
	...
	if ( empty( profiles()->wp_nav_menu_items ) ) {
		profiles()->wp_nav_menu_items = new stdClass;
	}

	profiles()->wp_nav_menu_items->loggedin = $page_args;
 ...
 * @return mixed A URL or an array of dummy pages.
 */
function profiles_nav_menu_get_loggedout_pages() {

	// Try to catch the cached version first.
	if ( ! empty( profiles()->wp_nav_menu_items->loggedout ) ) {
	...
	$profiles_directory_page_ids = profiles_core_get_directory_page_ids();
	...
	profiles()->wp_nav_menu_items->loggedout = $page_args;
 ...
 */
function profiles_nav_menu_get_item_url( $slug ) {
	$nav_item_url   = '';
	$nav_menu_items = profiles_nav_menu_get_loggedin_pages();
 ...
 */
function profiles_core_get_suggestions( $args ) {
	$args = profiles_parse_args( $args, array(), 'get_suggestions' );
	...
	if ( $args['type'] === 'members' ) {
		$class = 'Profiles_Members_Suggestions';

		// Members @name suggestions for users in a specific Group.
		if ( isset( $args['group_id'] ) ) {
			$class = 'Profiles_Groups_Member_Suggestions';
		}
		 ...
		 */
		$class = apply_filters( 'profiles_suggestions_services', '', $args );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_suggestions', $retval, $args );
}
 ...
 * @return string
 */
function profiles_upload_dir() {
	$profiles = profiles();

	if ( empty( $profiles->upload_dir ) ) {
		$need_switch = (bool) ( is_multisite() && ! profiles_is_root_blog() );
 ...
 * @since 2.5.0
 */
function profiles_email_post_type() {
	echo profiles_get_email_post_type();
}
	/**
	 ...
	 */
	function profiles_get_email_post_type() {
		 ...
		 */
		return apply_filters( 'profiles_get_email_post_type', profiles()->email_post_type );
	}
 ...
 * @return array
 */
function profiles_get_email_post_type_labels() {
	 ...
	 */
	return apply_filters( 'profiles_get_email_post_type_labels', array(
		'add_new'               => _x( 'Add New', 'email post type label', 'profiles' ),
		'add_new_item'          => _x( 'Add a New Email', 'email post type label', 'profiles' ),
		'all_items'             => _x( 'All Emails', 'email post type label', 'profiles' ),
		'edit_item'             => _x( 'Edit Email', 'email post type label', 'profiles' ),
		'filter_items_list'     => _x( 'Filter email list', 'email post type label', 'profiles' ),
		'items_list'            => _x( 'Email list', 'email post type label', 'profiles' ),
		'items_list_navigation' => _x( 'Email list navigation', 'email post type label', 'profiles' ),
		'menu_name'             => _x( 'Emails', 'email post type name', 'profiles' ),
		'name'                  => _x( 'Profiles Emails', 'email post type label', 'profiles' ),
		'new_item'              => _x( 'New Email', 'email post type label', 'profiles' ),
		'not_found'             => _x( 'No emails found', 'email post type label', 'profiles' ),
		'not_found_in_trash'    => _x( 'No emails found in Trash', 'email post type label', 'profiles' ),
		'search_items'          => _x( 'Search Emails', 'email post type label', 'profiles' ),
		'singular_name'         => _x( 'Email', 'email post type singular name', 'profiles' ),
		'uploaded_to_this_item' => _x( 'Uploaded to this email', 'email post type label', 'profiles' ),
		'view_item'             => _x( 'View Email', 'email post type label', 'profiles' ),
	) );
}
 ...
 * @return array
 */
function profiles_get_email_post_type_supports() {
	 ...
	 */
	return apply_filters( 'profiles_get_email_post_type_supports', array(
		'custom-fields',
		'editor',
		'excerpt',
		'revisions',
 ...
 * @since 2.5.0
 */
function profiles_email_tax_type() {
	echo profiles_get_email_tax_type();
}
	/**
	 ...
	 */
	function profiles_get_email_tax_type() {
		 ...
		 */
		return apply_filters( 'profiles_get_email_tax_type', profiles()->email_taxonomy_type );
	}
 ...
 * @return array
 */
function profiles_get_email_tax_type_labels() {
	 ...
	 * @param array $value Associative array (name => label).
	 */
	return apply_filters( 'profiles_get_email_tax_type_labels', array(
		'add_new_item'          => _x( 'New Email Situation', 'email type taxonomy label', 'profiles' ),
		'all_items'             => _x( 'All Email Situations', 'email type taxonomy label', 'profiles' ),
		'edit_item'             => _x( 'Edit Email Situations', 'email type taxonomy label', 'profiles' ),
		'items_list'            => _x( 'Email list', 'email type taxonomy label', 'profiles' ),
		'items_list_navigation' => _x( 'Email list navigation', 'email type taxonomy label', 'profiles' ),
		'menu_name'             => _x( 'Situations', 'email type taxonomy label', 'profiles' ),
		'name'                  => _x( 'Situation', 'email type taxonomy name', 'profiles' ),
		'new_item_name'         => _x( 'New email situation name', 'email type taxonomy label', 'profiles' ),
		'not_found'             => _x( 'No email situations found.', 'email type taxonomy label', 'profiles' ),
		'no_terms'              => _x( 'No email situations', 'email type taxonomy label', 'profiles' ),
		'popular_items'         => _x( 'Popular Email Situation', 'email type taxonomy label', 'profiles' ),
		'search_items'          => _x( 'Search Emails', 'email type taxonomy label', 'profiles' ),
		'singular_name'         => _x( 'Email', 'email type taxonomy singular name', 'profiles' ),
		'update_item'           => _x( 'Update Email Situation', 'email type taxonomy label', 'profiles' ),
		'view_item'             => _x( 'View Email Situation', 'email type taxonomy label', 'profiles' ),
	) );
}
...
/**
 * Get an Profiles_Email object for the specified email type.
 *
 ...
 * @return Profiles_Email|WP_Error Profiles_Email object, or WP_Error if there was a problem.
 */
function profiles_get_email( $email_type ) {
	$switched = false;
	 ...
	 * @param string $email_type Unique identifier for a particular type of email.
	 */
	$args = apply_filters( 'profiles_get_email_args', $args, $email_type );
	$post = get_posts( $args );
	if ( ! $post ) {
		if ( $switched ) {
			restore_current_blog();
	...
	/**
	 * Filters arguments used to create the Profiles_Email object.
	 *
	 * @since 2.5.0
	 *
	 ...
	 */
	$post  = apply_filters( 'profiles_get_email_post', $post[0], $email_type, $args, $post );
	$email = new Profiles_Email( $email_type );
	...
	/**
	 * Filters the Profiles_Email object returned by profiles_get_email().
	 *
	 * @since 2.5.0
	 *
	 * @param Profiles_Email $email      An object representing a single email, ready for mailing.
	 * @param string   $email_type Unique identifier for a particular type of email.
	 * @param array    $args       Arguments used with get_posts() to fetch a post object.
	 * @param WP_Post  $post       All posts retrieved by get_posts( $args ). May only contain $post.
	 */
	$retval = apply_filters( 'profiles_get_email', $email, $email_type, $args, $post );
 ...
 */
function profiles_send_email( $email_type, $to, $args = array() ) {
	...
	$args = profiles_parse_args( $args, array(
		'tokens' => array(),
	), 'send_email' );
	...
	$email = profiles_get_email( $email_type );
	 ...
	 */
	$must_use_wpmail = apply_filters( 'profiles_email_use_wp_mail', $wp_html_emails || ! $is_default_wpmail );
	 ...
	 *
	 * Defaults to Profiles_PHPMailer, which as you can guess, implements PHPMailer.
	 *
	 * @since 2.5.0
	 *
	 ...
	 * }
	 */
	$delivery_class = apply_filters( 'profiles_send_email_delivery_class', 'Profiles_PHPMailer', $email_type, $to, $args );
	if ( ! class_exists( $delivery_class ) ) {
		return new WP_Error( 'missing_class', __CLASS__, $this );
	}

	$delivery = new $delivery_class();
	$status   = $delivery->profiles_email( $email );
	...
	if ( is_wp_error( $status ) ) {
		 ...
		 * @param Profiles_Email $email  The email we tried to send.
		 */
		do_action( 'profiles_send_email_failure', $status, $email );
		 ...
		 * @param Profiles_Email $email  The email sent.
		 */
		do_action( 'profiles_send_email_success', $status, $email );
	}
 ...
 * @return array
 */
function profiles_email_get_appearance_settings() {
	$default_args = array(
		'body_bg'           => '#FFFFFF',
		...
		'footer_text' => sprintf(
			...
			_x( '&copy; %s %s', 'email', 'profiles' ),
			date_i18n( 'Y' ),
			profiles_get_option( 'blogname' )
		),
	);

	return profiles_parse_args(
		profiles_get_option( 'profiles_email_options', array() ),
		$default_args,
 ...
 * @return array
 */
function profiles_email_get_template( WP_Post $object ) {
	 ...
	 */
	return apply_filters( 'profiles_email_get_template', array(
 ...
 * @return string
 */
function profiles_core_replace_tokens_in_text( $text, $tokens ) {
	$unescaped = array();
	$escaped   = array();
	 ...
	 * @param array $tokens Token names and replacement values for the $text.
	 */
	return apply_filters( 'profiles_core_replace_tokens_in_text', $text, $tokens );
}
 ...
 * @return array
 */
function profiles_email_get_schema() {
	return array(
		'activity-comment' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] {{poster.name}} replied to one of your updates', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "{{poster.name}} replied to one of your updates:\n\n<blockquote>&quot;{{usermessage}}&quot;</blockquote>\n\n<a href=\"{{{thread.url}}}\">Go to the discussion</a> to reply or catch up on the conversation.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "{{poster.name}} replied to one of your updates:\n\n\"{{usermessage}}\"\n\nGo to the discussion to reply or catch up on the conversation: {{{thread.url}}}", 'profiles' ),
		),
		'activity-comment-author' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] {{poster.name}} replied to one of your comments', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
		...
		'activity-at-message' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] {{poster.name}} mentioned you in a status update', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "{{poster.name}} mentioned you in a status update:\n\n<blockquote>&quot;{{usermessage}}&quot;</blockquote>\n\n<a href=\"{{{mentioned.url}}}\">Go to the discussion</a> to reply or catch up on the conversation.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "{{poster.name}} mentioned you in a status update:\n\n\"{{usermessage}}\"\n\nGo to the discussion to reply or catch up on the conversation: {{{mentioned.url}}}", 'profiles' ),
		),
		'groups-at-message' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] {{poster.name}} mentioned you in an update', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
		...
		'core-user-registration' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] Activate your account', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "Thanks for registering!\n\nTo complete the activation of your account, go to the following link: <a href=\"{{{activate.url}}}\">{{{activate.url}}}</a>", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "Thanks for registering!\n\nTo complete the activation of your account, go to the following link: {{{activate.url}}}", 'profiles' ),
		),
		'core-user-registration-with-blog' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] Activate {{{user-site.url}}}', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "Thanks for registering!\n\nTo complete the activation of your account and site, go to the following link: <a href=\"{{{activate-site.url}}}\">{{{activate-site.url}}}</a>.\n\nAfter you activate, you can visit your site at <a href=\"{{{user-site.url}}}\">{{{user-site.url}}}</a>.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "Thanks for registering!\n\nTo complete the activation of your account and site, go to the following link: {{{activate-site.url}}}\n\nAfter you activate, you can visit your site at {{{user-site.url}}}.", 'profiles' ),
		),
		'friends-request' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] New friendship request from {{initiator.name}}', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "<a href=\"{{{initiator.url}}}\">{{initiator.name}}</a> wants to add you as a friend.\n\nTo accept this request and manage all of your pending requests, visit: <a href=\"{{{friend-requests.url}}}\">{{{friend-requests.url}}}</a>", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "{{initiator.name}} wants to add you as a friend.\n\nTo accept this request and manage all of your pending requests, visit: {{{friend-requests.url}}}\n\nTo view {{initiator.name}}'s profile, visit: {{{initiator.url}}}", 'profiles' ),
		),
		'friends-request-accepted' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] {{friend.name}} accepted your friendship request', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "<a href=\"{{{friendship.url}}}\">{{friend.name}}</a> accepted your friend request.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "{{friend.name}} accepted your friend request.\n\nTo learn more about them, visit their profile: {{{friendship.url}}}", 'profiles' ),
		),
		'groups-details-updated' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] Group details updated', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "Group details for the group &quot;<a href=\"{{{group.url}}}\">{{group.name}}</a>&quot; were updated:\n<blockquote>{{changed_text}}</blockquote>", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "Group details for the group \"{{group.name}}\" were updated:\n\n{{changed_text}}\n\nTo view the group, visit: {{{group.url}}}", 'profiles' ),
		),
		'groups-invitation' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] You have an invitation to the group: "{{group.name}}"', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "<a href=\"{{{inviter.url}}}\">{{inviter.name}}</a> has invited you to join the group: &quot;{{group.name}}&quot;.\n<a href=\"{{{invites.url}}}\">Go here to accept your invitation</a> or <a href=\"{{{group.url}}}\">visit the group</a> to learn more.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "{{inviter.name}} has invited you to join the group: \"{{group.name}}\".\n\nTo accept your invitation, visit: {{{invites.url}}}\n\nTo learn more about the group, visit {{{group.url}}}.\nTo view {{inviter.name}}'s profile, visit: {{{inviter.url}}}", 'profiles' ),
		),
		'groups-member-promoted' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] You have been promoted in the group: "{{group.name}}"', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "You have been promoted to <b>{{promoted_to}}</b> in the group &quot;<a href=\"{{{group.url}}}\">{{group.name}}</a>&quot;.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "You have been promoted to {{promoted_to}} in the group: \"{{group.name}}\".\n\nTo visit the group, go to: {{{group.url}}}", 'profiles' ),
		),
		'groups-membership-request' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] Membership request for group: {{group.name}}', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "<a href=\"{{{profile.url}}}\">{{requesting-user.name}}</a> wants to join the group &quot;{{group.name}}&quot;. As you are an administrator of this group, you must either accept or reject the membership request.\n\n<a href=\"{{{group-requests.url}}}\">Go here to manage this</a> and all other pending requests.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "{{requesting-user.name}} wants to join the group \"{{group.name}}\". As you are the administrator of this group, you must either accept or reject the membership request.\n\nTo manage this and all other pending requests, visit: {{{group-requests.url}}}\n\nTo view {{requesting-user.name}}'s profile, visit: {{{profile.url}}}", 'profiles' ),
		),
		'messages-unread' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] New message from {{sender.name}}', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "{{sender.name}} sent you a new message: &quot;{{usersubject}}&quot;\n\n<blockquote>&quot;{{usermessage}}&quot;</blockquote>\n\n<a href=\"{{{message.url}}}\">Go to the discussion</a> to reply or catch up on the conversation.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "{{sender.name}} sent you a new message: \"{{usersubject}}\"\n\n\"{{usermessage}}\"\n\nGo to the discussion to reply or catch up on the conversation: {{{message.url}}}", 'profiles' ),
		),
		'settings-verify-email-change' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] Verify your new email address', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "You recently changed the email address associated with your account on {{site.name}} to {{user.email}}. If this is correct, <a href=\"{{{verify.url}}}\">go here to confirm the change</a>.\n\nOtherwise, you can safely ignore and delete this email if you have changed your mind, or if you think you have received this email in error.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
		...
		'groups-membership-request-accepted' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] Membership request for group "{{group.name}}" accepted', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "Your membership request for the group &quot;<a href=\"{{{group.url}}}\">{{group.name}}</a>&quot; has been accepted.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
		...
		'groups-membership-request-rejected' => array(
			/* translators: do not remove {} brackets or translate its contents. */
			'post_title'   => __( '[{{{site.name}}}] Membership request for group "{{group.name}}" rejected', 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_content' => __( "Your membership request for the group &quot;<a href=\"{{{group.url}}}\">{{group.name}}</a>&quot; has been rejected.", 'profiles' ),
			/* translators: do not remove {} brackets or translate its contents. */
			'post_excerpt' => __( "Your membership request for the group \"{{group.name}}\" has been rejected.\n\nTo request membership again, visit: {{{group.url}}}", 'profiles' ),
		),
	);
}
 ...
 *     }
 */
function profiles_email_get_type_schema( $field = 'description' ) {
	$activity_comment = array(
		'description'	=> __( 'A member has replied to an activity update that the recipient posted.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_activity_new_reply',
			'message'	=> __( 'You will no longer receive emails when someone replies to an update or comment you posted.', 'profiles' ),
			),
	);

	$activity_comment_author = array(
		'description'	=> __( 'A member has replied to a comment on an activity update that the recipient posted.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_activity_new_reply',
			'message'	=> __( 'You will no longer receive emails when someone replies to an update or comment you posted.', 'profiles' ),
			),
	...
	$activity_at_message = array(
		'description'	=> __( 'Recipient was mentioned in an activity update.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_activity_new_mention',
			'message'	=> __( 'You will no longer receive emails when someone mentions you in an update.', 'profiles' ),
		),
	);

	$groups_at_message = array(
		'description'	=> __( 'Recipient was mentioned in a group activity update.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_activity_new_mention',
			'message'	=> __( 'You will no longer receive emails when someone mentions you in an update.', 'profiles' ),
		),
	...
	$core_user_registration = array(
		'description'	=> __( 'Recipient has registered for an account.', 'profiles' ),
		'unsubscribe'	=> false,
	);

	$core_user_registration_with_blog = array(
		'description'	=> __( 'Recipient has registered for an account and site.', 'profiles' ),
		'unsubscribe'	=> false,
	);

	$friends_request = array(
		'description'	=> __( 'A member has sent a friend request to the recipient.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_friends_friendship_request',
			'message'	=> __( 'You will no longer receive emails when someone sends you a friend request.', 'profiles' ),
		),
	);

	$friends_request_accepted = array(
		'description'	=> __( 'Recipient has had a friend request accepted by a member.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_friends_friendship_accepted',
			'message'	=> __( 'You will no longer receive emails when someone accepts your friendship request.', 'profiles' ),
		),
	);

	$groups_details_updated = array(
		'description'	=> __( "A group's details were updated.", 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_groups_group_updated',
			'message'	=> __( 'You will no longer receive emails when one of your groups is updated.', 'profiles' ),
		),
	);
	...
	$groups_invitation = array(
		'description'	=> __( 'A member has sent a group invitation to the recipient.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_groups_invite',
			'message'	=> __( 'You will no longer receive emails when you are invited to join a group.', 'profiles' ),
		),
	);

	$groups_member_promoted = array(
		'description'	=> __( "Recipient's status within a group has changed.", 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_groups_admin_promotion',
			'message'	=> __( 'You will no longer receive emails when you have been promoted in a group.', 'profiles' ),
		),
	);
	...
	$groups_membership_request = array(
		'description'	=> __( 'A member has requested permission to join a group.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_groups_membership_request',
			'message'	=> __( 'You will no longer receive emails when someone requests to be a member of your group.', 'profiles' ),
		),
	);

	$messages_unread = array(
		'description'	=> __( 'Recipient has received a private message.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_messages_new_message',
			'message'	=> __( 'You will no longer receive emails when someone sends you a message.', 'profiles' ),
		),
	);

	$settings_verify_email_change = array(
		'description'	=> __( 'Recipient has changed their email address.', 'profiles' ),
		'unsubscribe'	=> false,
	);

	$groups_membership_request_accepted = array(
		'description'	=> __( 'Recipient had requested to join a group, which was accepted.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_membership_request_completed',
			'message'	=> __( 'You will no longer receive emails when your request to join a group has been accepted or denied.', 'profiles' ),
		),
	);

	$groups_membership_request_rejected = array(
		'description'	=> __( 'Recipient had requested to join a group, which was rejected.', 'profiles' ),
		'unsubscribe'	=> array(
			'meta_key'	=> 'notification_membership_request_completed',
			'message'	=> __( 'You will no longer receive emails when your request to join a group has been accepted or denied.', 'profiles' ),
		),
 ...
 * @since 2.7.0
 */
function profiles_email_unsubscribe_handler() {
	$emails         = profiles_email_get_type_schema( 'all' );
	$raw_email_type = ! empty( $_GET['nt'] ) ? $_GET['nt'] : '';
	$raw_hash       = ! empty( $_GET['nh'] ) ? $_GET['nh'] : '';
	$raw_user_id    = ! empty( $_GET['uid'] ) ? absint( $_GET['uid'] ) : 0;
	$new_hash       = hash_hmac( 'sha1', "{$raw_email_type}:{$raw_user_id}", profiles_email_get_salt() );

	// Check required values.
	if ( ! $raw_user_id || ! $raw_email_type || ! $raw_hash || ! array_key_exists( $raw_email_type, $emails ) ) {
		$redirect_to = site_url( 'wp-login.php' );
		$result_msg  = __( 'Something has gone wrong.', 'profiles' );
		$unsub_msg   = __( 'Please log in and go to your settings to unsubscribe from notification emails.', 'profiles' );
	...
	// Don't let authenticated users unsubscribe other users' email notifications.
		...
		$result_msg  = __( 'Something has gone wrong.', 'profiles' );
		$unsub_msg   = __( 'Please go to your notifications settings to unsubscribe from emails.', 'profiles' );

		if ( profiles_is_active( 'settings' ) ) {
			$redirect_to = sprintf(
				'%s%s/notifications/',
				profiles_core_get_user_domain( get_current_user_id() ),
				profiles_get_settings_slug()
			);
		} else {
			$redirect_to = profiles_core_get_user_domain( get_current_user_id() );
		}
		...
		if ( profiles_is_active( 'settings' ) ) {
			$redirect_to = sprintf(
				'%s%s/notifications/',
				profiles_core_get_user_domain( $raw_user_id ),
				profiles_get_settings_slug()
			);
		} else {
			$redirect_to = profiles_core_get_user_domain( $raw_user_id );
		}

		// Unsubscribe.
		$meta_key = $emails[ $raw_email_type ]['unsubscribe']['meta_key'];
		profiles_update_user_meta( $raw_user_id, $meta_key, 'no' );

		$result_msg = $emails[ $raw_email_type ]['unsubscribe']['message'];
		$unsub_msg  = __( 'You can change this or any other email notification preferences in your email settings.', 'profiles' );
	}
	...
	profiles_core_add_message( $message );
	profiles_core_redirect( profiles_core_get_user_domain( $raw_user_id ) );
 ...
 */
function profiles_email_get_unsubscribe_link( $args ) {
	$emails = profiles_email_get_type_schema( 'all' );
	...
	$link = add_query_arg(
		array(
			'action' => 'unsubscribe',
			'nh'     => hash_hmac( 'sha1', "{$email_type}:{$user_id}", profiles_email_get_salt() ),
	 ...
	 * @since 2.7.0
	 */
	return apply_filters( 'profiles_email_get_link', $link, $redirect_to, $args );
}
 ...
 */
function profiles_email_get_salt() {
	return profiles_get_option( 'profiles-emails-unsubscribe-salt', null );
}
