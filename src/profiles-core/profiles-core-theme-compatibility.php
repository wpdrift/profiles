 ...
 *
 * @package Profiles
 * @suprofilesackage ThemeCompatibility
 * @since 1.7.0
 */
...
if ( ! profiles()->do_autoload ) {
	require dirname( __FILE__ ) . '/classes/class-profiles-theme-compat.php';
}
 ...
 */
function profiles_setup_theme_compat( $theme = '' ) {
	$profiles = profiles();

	// Make sure theme package is available, set to default if not.
	if ( ! isset( $profiles->theme_compat->packages[$theme] ) || ! is_a( $profiles->theme_compat->packages[$theme], 'Profiles_Theme_Compat' ) ) {
		$theme = 'legacy';
	}
 ...
 */
function profiles_get_theme_compat_id() {
	 ...
	 */
	return apply_filters( 'profiles_get_theme_compat_id', profiles()->theme_compat->theme->id );
}
 ...
 */
function profiles_get_theme_compat_name() {
	 ...
	 */
	return apply_filters( 'profiles_get_theme_compat_name', profiles()->theme_compat->theme->name );
}
 ...
 */
function profiles_get_theme_compat_version() {
	 ...
	 */
	return apply_filters( 'profiles_get_theme_compat_version', profiles()->theme_compat->theme->version );
}
 ...
 */
function profiles_get_theme_compat_dir() {
	 ...
	 */
	return apply_filters( 'profiles_get_theme_compat_dir', profiles()->theme_compat->theme->dir );
}
 ...
 */
function profiles_get_theme_compat_url() {
	 ...
	 */
	return apply_filters( 'profiles_get_theme_compat_url', profiles()->theme_compat->theme->url );
}
 ...
 * do so using profiles_detect_theme_compat_with_current_theme().
 *
 * @since 1.9.0
 *
 * @return bool True if the current theme needs theme compatibility.
 */
function profiles_use_theme_compat_with_current_theme() {
	if ( ! isset( profiles()->theme_compat->use_with_current_theme ) ) {
		profiles_detect_theme_compat_with_current_theme();
	}
	 ...
	 */
	return apply_filters( 'profiles_use_theme_compat_with_current_theme', profiles()->theme_compat->use_with_current_theme );
}
 ...
 * 2) It is profiles-default, or a child theme of profiles-default
 * 3) A legacy template is found at members/members-loop.php. This is a
 *    fallback check for themes that were derived from profiles-default, and have
 ...
 * @return bool True if the current theme needs theme compatibility.
 */
function profiles_detect_theme_compat_with_current_theme() {
	if ( isset( profiles()->theme_compat->use_with_current_theme ) ) {
 ...
 */
function profiles_is_theme_compat_active() {
	$profiles = profiles();
 ...
 */
function profiles_set_theme_compat_active( $set = true ) {
 ...
 */
function profiles_set_theme_compat_templates( $templates = array() ) {
 ...
 */
function profiles_set_theme_compat_template( $template = '' ) {
 ...
 */
function profiles_set_theme_compat_original_template( $template = '' ) {
 ...
 */
function profiles_set_theme_compat_feature( $theme_id, $feature = array() ) {
	...
	if ( $theme_id !== profiles_get_theme_compat_id() ) {
		return;
	}
 ...
 */
function profiles_get_theme_compat_feature( $feature = '' ) {
	// Get current theme compat theme.
	$theme_compat_theme = profiles()->theme_compat->theme;
 ...
 * as it's loaded using profiles_locate_template(). That's why this function is here.
 *
 * @since 2.4.0
 *
 * @global string $content_width the content width of the theme
 */
function profiles_register_theme_compat_default_features() {
	global $content_width;

	// Do not set up default features on deactivation.
	if ( profiles_is_deactivation() ) {
		return;
	}
	...
	if ( 'legacy' !== profiles_get_theme_compat_id() ) {
		return;
	}
	...
	$top_offset    = 150;
	$avatar_height = apply_filters( 'profiles_core_avatar_full_height', $top_offset );

	if ( $avatar_height > $top_offset ) {
		$top_offset = $avatar_height;
	}

	profiles_set_theme_compat_feature( 'legacy', array(
		'name'     => 'cover_image',
		'settings' => array(
			'components'   => array( 'xprofile', 'groups' ),
			'width'        => $profiles_content_width,
			'height'       => $top_offset + round( $avatar_height / 2 ),
			'callback'     => 'profiles_legacy_theme_cover_image',
 ...
 */
function profiles_is_theme_compat_original_template( $template = '' ) {
	$profiles = profiles();
 ...
 * @since 1.7.0
 *
 * @see Profiles_Theme_Compat for a description of the $theme parameter arguments.
 *
 * @param array $theme    See {@link Profiles_Theme_Compat}.
 * @param bool  $override If true, overrides whatever package is currently set.
 *                        Default: true.
 */
function profiles_register_theme_package( $theme = array(), $override = true ) {

	// Create new Profiles_Theme_Compat object from the $theme array.
	if ( is_array( $theme ) ) {
		$theme = new Profiles_Theme_Compat( $theme );
	}

	// Bail if $theme isn't a proper object.
	if ( ! is_a( $theme, 'Profiles_Theme_Compat' ) ) {
		return;
	}
 ...
 */
function profiles_theme_compat_reset_post( $args = array() ) {
	 ...
	 *
	 * @see https://bprofilesress.trac.wordpress.org/ticket/1973
	 */
	if ( ! $wp_query->is_404() ) {
		status_header( 200 );
	}
	...
	profiles_set_theme_compat_active( true );

	// If we are in theme compat, we don't need the 'Edit' post link.
	add_filter( 'get_edit_post_link', 'profiles_core_filter_edit_post_link', 10, 2 );
}
 ...
 */
function profiles_template_include_theme_compat( $template = '' ) {
	 ...
	 * @since 1.7.0
	 */
	do_action( 'profiles_template_include_reset_dummy_post_data' );
	 ...
	 *
	 * Uses profiles_get_theme_compat_templates() to provide fall-backs that
	 * should be coded without superfluous mark-up and logic (prev/next
	 * navigation, comments, date/time, etc...)
	 *
	 * Hook into 'profiles_get_profiles_template' to override the array of
	 * possible templates, or 'profiles_profiles_template' to override the result.
	 */
	if ( profiles_is_theme_compat_active() ) {
		$template = profiles_get_theme_compat_templates();

		add_filter( 'the_content', 'profiles_replace_the_content' );

		// Add Profiles's head action to wp_head.
		if ( ! has_action( 'wp_head', 'profiles_head' ) ) {
			add_action( 'wp_head', 'profiles_head' );
		}
	}
	 ...
	 * @param string $template Template name.
	 */
	return apply_filters( 'profiles_template_include_theme_compat', $template );
}
 ...
 */
function profiles_replace_the_content( $content = '' ) {

	// Bail if not the main loop where theme compat is happening.
	if ( ! profiles_do_theme_compat() ) {
		return $content;
	}
	 ...
	 * @param string $content Original post content.
	 */
	$new_content = apply_filters( 'profiles_replace_the_content', $content );
 ...
 */
function profiles_do_theme_compat() {
	return (bool) ( ! profiles_is_template_included() && in_the_loop() && profiles_is_theme_compat_active() );
}
 ...
 */
function profiles_remove_all_filters( $tag, $priority = false ) {
...
/**
 * Restore filters that were removed using profiles_remove_all_filters().
 *
 * @since 1.7.0
 *
 * @global WP_filter $wp_filter
 ...
 * @return bool True on success.
 */
function profiles_restore_all_filters( $tag, $priority = false ) {
	global $wp_filter, $merged_filters;
 ...
 */
function profiles_comments_open( $open, $post_id = 0 ) {
	 ...
	 */
	return apply_filters( 'profiles_force_comment_status', $retval, $open, $post_id );
}
 ...
 * {@link profiles_theme_compat_reset_post()}, themes that use comments_template()
 ...
 */
function profiles_theme_compat_toggle_is_page( $retval = '' ) {
	global $wp_query;
...
add_filter( 'profiles_replace_the_content', 'profiles_theme_compat_toggle_is_page', 9999 );
 ...
 * @since 1.9.2
 *
 * @see profiles_theme_compat_toggle_is_page()
 * @param object $query The WP_Query object.
 */
function profiles_theme_compat_loop_end( $query ) {
...
add_action( 'loop_end', 'profiles_theme_compat_loop_end' );
