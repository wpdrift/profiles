 ...
 *                                                (those with the 'profiles_moderate' cap). Default: false.
 ...
 */
function profiles_core_new_nav_item( $args, $component = 'members' ) {
	if ( ! profiles_is_active( $component ) ) {
		return;
	}
	...
	$nav_item = profiles_core_create_nav_link( $r, $component );

	/*
	 * To mimic legacy behavior, if profiles_core_create_nav_link() returns false, we make
	...
	$hooked = profiles_core_register_nav_screen_function( $nav_item );
	if ( false === $hooked ){
		return false;
	}
	 ...
	 * `profiles_core_create_nav_link` or `profiles_core_register_nav_screen_function`
	 * should be used.
	 *
	 * @since 1.5.0
	 *
	 ...
	 */
	do_action( 'profiles_core_new_nav_item', $r, $args, $defaults );
}
 ...
 *
 * @since 2.4.0
 * @since 2.6.0 Introduced the `$component` parameter. Began returning a Profiles_Nav_Item object on success.
 *
 * @param array|string $args {
 *     Array describing the new nav item.
 *     @type string      $name                    Display name for the nav item.
 ...
 * @return bool|Profiles_Nav_Item Returns false on failure, new nav item on success.
 */
function profiles_core_create_nav_link( $args = '', $component = 'members' ) {
	$profiles = profiles();
	...
	if ( ! empty( $r['site_admin_only'] ) && ! profiles_current_user_can( 'profiles_moderate' ) ) {
		return false;
	}
	...
	$nav_item = array(
		'name'                    => $r['name'],
		'slug'                    => $r['slug'],
		'link'                    => trailingslashit( profiles_loggedin_user_domain() . $r['slug'] ),
	 ...
	 */
	do_action( 'profiles_core_create_nav_link', $r, $args, $defaults, $component );
 ...
 * @return bool|null Returns false on failure.
 */
function profiles_core_register_nav_screen_function( $args = '' ) {
	$profiles = profiles();
	...
	*/
	 ...
	 */
	if ( empty( $r['show_for_displayed_user'] ) && ! profiles_user_has_access() ) {
		return false;
	}
	 ...
	 */
	if ( ( -1 != $r['position'] ) && profiles_is_root_component( $r['slug'] ) && ! profiles_displayed_user_id() ) {
		return;
	}

	// Look for current component.
	if ( profiles_is_current_component( $r['slug'] ) || profiles_is_current_item( $r['slug'] ) ) {
		...
		if ( ! empty( $r['default_subnav_slug'] ) && profiles_is_current_action( $r['default_subnav_slug'] ) && ! profiles_action_variable( 0 ) ) {
			unset( $profiles->canonical_stack['action'] );
		} elseif ( ! profiles_current_action() ) {

			// Add our screen hook if screen function is callable.
			if ( is_callable( $r['screen_function'] ) ) {
				add_action( 'profiles_screens', $r['screen_function'], 3 );
			}
			...
			if ( ! empty( $r['default_subnav_slug'] ) ) {
				 ...
				 * @param array  $r     Parsed arguments for the nav item.
				 */
				$profiles->current_action = apply_filters( 'profiles_default_component_subnav', $r['default_subnav_slug'], $r );
			}
		}
	}
	 ...
	 * @param array $defaults Default arguments for a nav item.
	 */
	do_action( 'profiles_core_register_nav_screen_function', $r, $args, $defaults );
}
 ...
 * }
 */
function profiles_core_new_nav_default( $args = '' ) {
	$profiles = profiles();
	...
	if ( ! empty( $parent_nav->screen_function ) ) {
		...
		if ( is_callable( $parent_nav->screen_function ) ) {
			remove_action( 'profiles_screens', $parent_nav->screen_function, 3 );
		}
	}
	...
	if ( profiles_is_current_component( $parent_nav->slug ) ) {
		...
		if ( empty( $unfiltered_action ) ) {
			if ( ! profiles_is_current_action( $r['subnav_slug'] ) ) {
				if ( is_callable( $r['screen_function'] ) ) {
					add_action( 'profiles_screens', $r['screen_function'], 3 );
				}
 ...
 *                                          eg, use `profiles_is_my_profile()` to restrict access to profile owners only.
 *                                          Default: true.
 *     @type bool        $site_admin_only   Optional. Whether the nav item should be visible only to site admins
 *                                          (those with the 'profiles_moderate' cap). Default: false.
 ...
 * @return bool|null Returns false on failure.
 */
function profiles_core_new_subnav_item( $args, $component = null ) {
	// Backward compatibility for plugins using `profiles_core_new_subnav_item()` without `$component`
	// to add group subnav items.
	if ( null === $component && profiles_is_active( 'groups' ) && profiles_is_group() && isset( $args['parent_slug'] ) ) {
		/*
		 ...
		 */
		$group_slug = profiles_get_current_group_slug();
		if (
	...
	$subnav_item = profiles_core_create_subnav_link( $args, $component );

	/*
	 * To mimic legacy behavior, if profiles_core_create_subnav_link() returns false, we make an
	...
	$hooked = profiles_core_register_subnav_screen_function( $subnav_item, $component );
	if ( false === $hooked ) {
		return false;
	}
}
 ...
 *                                          when registering the subnav; eg, use profiles_is_my_profile()
 ...
 *                                          to site admins (those with the 'profiles_moderate' cap).
 *                                          Default: false.
 ...
 * }
 * @param string       $component The component the navigation is attached to. Defaults to 'members'.
 * @return bool|object Returns false on failure, new Profiles_Nav_Item instance on success.
 */
function profiles_core_create_subnav_link( $args = '', $component = 'members' ) {
	$profiles = profiles();
 ...
 *                                       only to site admins (those with the 'profiles_moderate' cap).
 *                                       Default: false.
 *     @type int      $position          Optional. Numerical index specifying where the item
 *                                       should appear in the subnav array. Default: 90.
 *     @type callable $screen_function   The callback function that will run
 ...
 * @return bool|null Returns false on failure.
 */
function profiles_core_register_subnav_screen_function( $args = '', $component = 'members' ) {
	$profiles = profiles();
	...
	if ( ! profiles_is_current_component( $r['parent_slug'] ) && ! profiles_is_current_item( $r['parent_slug'] ) ) {
		return;
	}
	...
	if ( ( profiles_current_action() && profiles_is_current_action( $r['slug'] ) ) || ( profiles_is_user() && ! profiles_current_action() && ! empty( $parent_nav->screen_function ) && $r['screen_function'] == $parent_nav->screen_function ) ) {
		...
		$hooked = profiles_core_maybe_hook_new_subnav_screen_function( $r, $component );
		...
		if ( ! empty( $hooked['status'] ) && 'failure' === $hooked['status'] && isset( $hooked['redirect_args'] ) ) {
			profiles_core_no_access( $hooked['redirect_args'] );
		}
	}
}
 ...
 *                            the component in profiles_core_new_subnav_item().
 * @param string $component   The component the navigation is attached to. Defaults to 'members'.
 * @return array
 */
function profiles_core_maybe_hook_new_subnav_screen_function( $subnav_item, $component = 'members' ) {
	$retval = array(
		'status' => '',
	);
	...
	if ( ! empty( $subnav_item['site_admin_only'] ) && ! profiles_current_user_can( 'profiles_moderate' ) ) {
	...
	if ( ! empty( $subnav_item['user_has_access'] ) && ! $site_admin_restricted ) {
		...
		// Success - hook to profiles_screens.
		} else {
			add_action( 'profiles_screens', $subnav_item['screen_function'], 3 );
	...
	// user status.
		...
		if ( is_user_logged_in() ) {
			...
			// redirect URL.
				...
				// publicly accessible.
				if ( profiles_is_my_profile() || ( isset( $parent_nav_default_item ) && $parent_nav_default_item->show_for_displayed_user ) ) {
					$message     = __( 'You do not have access to this page.', 'profiles' );
					$redirect_to = profiles_displayed_user_domain();
				...
				// know will be accessible.
					...
					// Then try 'profile'.
					$redirect_to = trailingslashit( profiles_displayed_user_domain() . ( 'xprofile' == $profiles->profile->id ? 'profile' : $profiles->profile->id ) );
			...
			// Fall back to the home page.
				...
				$message     = __( 'You do not have access to this page.', 'profiles' );
				$redirect_to = profiles_get_root_domain();
			}
			...
			// used in profiles_core_no_access().
			$retval['redirect_args'] = array();
		}
	}
 ...
 */
function profiles_nav_item_has_subnav( $nav_item = '', $component = 'members' ) {
	$profiles = profiles();
	...
	if ( ! $nav_item ) {
		$nav_item = profiles_current_component();

		if ( profiles_is_group() ) {
			$nav_item = profiles_current_item();
		}
	}
	 ...
	 * @param string $nav_item   The slug of the top-level nav item whose subnav items you're checking.
	 */
	return apply_filters( 'profiles_nav_item_has_subnav', $has_subnav, $nav_item );
}
 ...
 */
function profiles_core_remove_nav_item( $slug, $component = null ) {
	$profiles = profiles();

	// Backward compatibility for removing group nav items using the group slug as `$parent_slug`.
	if ( ! $component && profiles_is_active( 'groups' ) && isset( $profiles->groups->nav ) ) {
	...
	if ( profiles()->do_nav_backcompat ) {
		$profiles->profiles_nav->reset();
		$profiles->profiles_options_nav->reset();
	}
	...
	foreach ( $screen_functions as $screen_function ) {
		...
		if ( is_callable( $screen_function ) ) {
			remove_action( 'profiles_screens', $screen_function, 3 );
		}
	}
 ...
 * @return bool Returns false on failure, True on success.
 */
function profiles_core_remove_subnav_item( $parent_slug, $slug, $component = null ) {
	$profiles = profiles();
 ...
 * @param string $component   The component the navigation is attached to. Defaults to 'members'.
 */
function profiles_core_reset_subnav_items( $parent_slug, $component = 'members' ) {
	$profiles = profiles();
 ...
 */
function profiles_get_admin_bar_pref( $context, $user = 0 ) {
