 ...
 * @see WP_Embed
 */
class Profiles_Embed extends WP_Embed {
	...
	public function __construct() {
		...
		if ( profiles_use_embed_in_activity() ) {
			add_filter( 'profiles_get_activity_content_body', array( &$this, 'autoembed' ), 8 );
			add_filter( 'profiles_get_activity_content_body', array( &$this, 'run_shortcode' ), 7 );
		}

		if ( profiles_use_embed_in_activity_replies() ) {
			add_filter( 'profiles_get_activity_content', array( &$this, 'autoembed' ), 8 );
			add_filter( 'profiles_get_activity_content', array( &$this, 'run_shortcode' ), 7 );
		}

		if ( profiles_use_embed_in_forum_posts() ) {
			add_filter( 'profiles_get_the_topic_post_content', array( &$this, 'autoembed' ), 8 );
			add_filter( 'profiles_get_the_topic_post_content', array( &$this, 'run_shortcode' ), 7 );
		}

		if ( profiles_use_embed_in_private_messages() ) {
			add_filter( 'profiles_get_the_thread_message_content', array( &$this, 'autoembed' ), 8 );
			add_filter( 'profiles_get_the_thread_message_content', array( &$this, 'run_shortcode' ), 7 );
		}
		 ...
		 * @since 1.5.0
		 *
		 * @param Profiles_Embed $this Current instance of the Profiles_Embed. Passed by reference.
		 */
		do_action_ref_array( 'profiles_core_setup_oembed', array( &$this ) );
	}
	 ...
	 * enabled, then the URL will be passed to {@link Profiles_Embed::parse_oembed()}
	 * for oEmbed parsing.
	 *
	 *
	...
	public function shortcode( $attr, $url = '' ) {
		 ...
		 */
		$attr['discover'] = ( apply_filters( 'profiles_embed_oembed_discover', $default_discovery ) && $unfiltered_html );
	 ...
	 *
	 * View an example to add support in {@link profiles_activity_embed()}.
	 *
	 *       on success.
	 *       oEmbed failure.
	 *
	...
	public function parse_oembed( $id, $url, $attr, $rawattr ) {
		...
		if ( $id ) {
			 ...
			 */
			$cache = apply_filters( 'profiles_embed_get_cache', $cache, $id, $cachekey, $url, $attr, $rawattr );
			...
			if ( !empty( $cache ) ) {
				 ...
				 */
				return apply_filters( 'profiles_embed_oembed_html', $cache, $url, $attr, $rawattr );
			...
			// If no cache, ping the oEmbed provider and cache the result.
				 ...
				 * @param int    $id       ID to do the caching for.
				 */
				do_action( 'profiles_embed_update_cache', $cache, $cachekey, $id );

				// If there was a result, return it.
				if ( $html ) {

					/** This filter is documented in profiles-core/classes/class-profiles-embed.php */
					return apply_filters( 'profiles_embed_oembed_html', $html, $url, $attr, $rawattr );
				}
			}
		}
