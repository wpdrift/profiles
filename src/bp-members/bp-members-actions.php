...
function bp_core_action_set_spammer_status( $user_id = 0 ) {
	...
	if ( bp_is_current_component( 'admin' ) && ( in_array( bp_current_action(), array( 'mark-spammer', 'unmark-spammer' ) ) ) ) {
		...
		if ( 'spam' == $status ) {
			bp_core_add_message( __( 'User marked as spammer. Spam users are visible only to site admins.', 'profiles' ) );
		} else {
			bp_core_add_message( __( 'User removed as spammer.', 'profiles' ) );
		}
...
function bp_core_action_delete_user() {
	...
	if ( bp_is_current_component( 'admin' ) && bp_is_current_action( 'delete-user' ) ) {
		...
		if ( bp_core_delete_account( bp_displayed_user_id() ) ) {
			bp_core_add_message( sprintf( __( '%s has been deleted from the system.', 'profiles' ), bp_get_displayed_user_fullname() ) );
		} else {
			bp_core_add_message( sprintf( __( 'There was an error deleting %s from the system. Please try again.', 'profiles' ), bp_get_displayed_user_fullname() ), 'error' );
			$errors = true;
		}
