 ...
 * @return object
 */
function profiles_get_current_blog_roles() {
	global $wp_roles;
	 ...
	 */
	return apply_filters( 'profiles_get_current_blog_roles', $roles, $wp_roles );
}
...
function profiles_add_caps() {
	...
	foreach( $wp_roles->role_objects as $role ) {
		foreach ( profiles_get_caps_for_role( $role->name ) as $cap ) {
			$role->add_cap( $cap );
		}
	}
...
function profiles_remove_caps() {
	 ...
	 * @since 1.6.0
	 */
	do_action( 'profiles_remove_caps' );
}
 ...
 */
function profiles_map_meta_caps( $caps, $cap, $user_id, $args ) {
	 ...
	 */
	return apply_filters( 'profiles_map_meta_caps', $caps, $cap, $user_id, $args );
}
 ...
 */
function profiles_get_community_caps() {
	 ...
	 */
	return apply_filters( 'profiles_get_community_caps', $caps );
}
 ...
 */
function profiles_get_caps_for_role( $role = '' ) {
	 ...
	 */
	return apply_filters( 'profiles_get_caps_for_role', $caps, $role );
}
 ...
 * @since 1.6.0
 */
function profiles_set_current_user_default_role() {
	...
	if ( profiles_is_user_inactive() ) {
		return;
	}

	// Set the current users default role.
	profiles()->current_user->set_role( profiles_get_option( 'default_role', 'subscriber' ) );
}
 ...
 */
function profiles_current_user_can( $capability, $args = array() ) {
	$blog_id = 0;
	...
	// Backward compatibility for older profiles_current_user_can() checks.
	if ( empty( $args ) ) {
		$args = null;
	}
	...
	if ( empty( $blog_id ) ) {
		$blog_id = profiles_get_root_blog_id();
	}
	 ...
	 */
	return (bool) apply_filters( 'profiles_current_user_can', $retval, $capability, $blog_id, $args );
}
 ...
 * @return bool True if the user has the cap for the given parameters.
 */
function profiles_user_can( $user_id, $capability, $args = array() ) {
	 ...
	 * @param array  $args       Array of extra arguments passed.
	 */
	$retval = (bool) apply_filters( 'profiles_user_can', $retval, $user_id, $capability, $site_id, $args );
...
/**
 * Temporary implementation of 'profiles_moderate' cap.
 *
 * In Profiles 1.6, the 'profiles_moderate' cap was introduced. In order to
 * enforce that profiles_current_user_can( 'profiles_moderate' ) always returns true for
 * Administrators, we must manually add the 'profiles_moderate' cap to the list of
 ...
 * capability - and thus 'profiles_moderate' - to Super Admins on a Multisite
 * installation. See {@link WP_User::has_cap()}.
 *
 * This implementation of 'profiles_moderate' is temporary, until Profiles properly
 ...
 * @return array $allcaps The user's cap list, with 'profiles_moderate' appended, if relevant.
 */
function _profiles_enforce_profiles_moderate_cap_for_admins( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {

	// Bail if not checking the 'profiles_moderate' cap.
	if ( 'profiles_moderate' !== $cap ) {
		return $caps;
	}

	// Bail if Profiles is not network activated.
	if ( profiles_is_network_activated() ) {
		return $caps;
	}
	...
	// Only users that can 'manage_options' on this site can 'profiles_moderate'.
	return array( 'manage_options' );
}
add_filter( 'map_meta_cap', '_profiles_enforce_profiles_moderate_cap_for_admins', 10, 4 );
 ...
 * @deprecated 1.7.0
 */
function profiles_add_roles() {
	_doing_it_wrong( 'profiles_add_roles', __( 'Special community roles no longer exist. Use mapped capabilities instead', 'profiles' ), '1.7' );
}
 ...
 * @deprecated 1.7.0
 */
function profiles_remove_roles() {
	_doing_it_wrong( 'profiles_remove_roles', __( 'Special community roles no longer exist. Use mapped capabilities instead', 'profiles' ), '1.7' );
}
 ...
 * @deprecated 1.7.0
 */
function profiles_get_participant_role() {
	_doing_it_wrong( 'profiles_get_participant_role', __( 'Special community roles no longer exist. Use mapped capabilities instead', 'profiles' ), '1.7' );
}
 ...
 * @deprecated 1.7.0
 */
function profiles_get_moderator_role() {
	_doing_it_wrong( 'profiles_get_moderator_role', __( 'Special community roles no longer exist. Use mapped capabilities instead', 'profiles' ), '1.7' );
}
