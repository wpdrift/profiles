...
/**
 * API to create Profiles buttons.
 *
 * @since 1.2.6
 *
 * @param array $args {
...
class Profiles_Button {
	 ...
	 * @since 1.2.6
	 *
	 * @param array|string $args See {@Profiles_Button}.
	 */
	public function __construct( $args = '' ) {
		...
		if ( ! profiles_is_active( $this->component ) )
			return false;
		...
		if ( true == $this->block_self ) {
			...
			if ( profiles_get_member_user_id() ) {
				if ( is_user_logged_in() && profiles_loggedin_user_id() == profiles_get_member_user_id() ) {
					return false;
				}

			// No button if viewing your own profile (and not in
			// a members loop).
			} elseif ( profiles_is_my_profile() ) {
				return false;
			}
		}
		 ...
		 * @param Profiles_Button $this     Current Profiles_Button instance.
		 * @param string    $before   HTML appended before the actual button.
		 * @param string    $after    HTML appended after the actual button.
		 */
		$this->contents = apply_filters( 'profiles_button_' . $this->component . '_' . $this->id, $this->contents, $this, $before, $after );
	}
