 ...
 *
 * @package Profiles
 * @suprofilesackage MembersFunctions
 * @since 1.5.0
 */
 ...
 */
function profiles_members_has_directory() {
	$profiles = profiles();
 ...
 * Profiles_MEMBERS_SLUG, Profiles_REGISTER_SLUG, and Profiles_ACTIVATION_SLUG. If these
 * constants are not overridden in wp-config.php or profiles-custom.php, they are
 ...
 * @since 1.5.0
 */
function profiles_core_define_slugs() {
	$profiles = profiles();

	// No custom members slug.
	if ( !defined( 'Profiles_MEMBERS_SLUG' ) ) {
		if ( !empty( $profiles->pages->members ) ) {
			define( 'Profiles_MEMBERS_SLUG', $profiles->pages->members->slug );
		} else {
			define( 'Profiles_MEMBERS_SLUG', 'members' );
		}
	}

	// No custom registration slug.
	if ( !defined( 'Profiles_REGISTER_SLUG' ) ) {
		if ( !empty( $profiles->pages->register ) ) {
			define( 'Profiles_REGISTER_SLUG', $profiles->pages->register->slug );
		} else {
			define( 'Profiles_REGISTER_SLUG', 'register' );
		}
	}

	// No custom activation slug.
	if ( !defined( 'Profiles_ACTIVATION_SLUG' ) ) {
		if ( !empty( $profiles->pages->activate ) ) {
			define( 'Profiles_ACTIVATION_SLUG', $profiles->pages->activate->slug );
		} else {
			define( 'Profiles_ACTIVATION_SLUG', 'activate' );
		}
	}
}
add_action( 'profiles_setup_globals', 'profiles_core_define_slugs', 11 );

/**
 * Fetch an array of users based on the parameters passed.
 *
 * Since Profiles 1.7, profiles_core_get_users() uses Profiles_User_Query. If you
 * need backward compatibility with Profiles_Core_User::get_users(), filter the
 * profiles_use_legacy_user_query value, returning true.
 *
 * @since 1.2.0
 *
 * @param array|string $args {
 *     Array of arguments. All are optional. See {@link Profiles_User_Query} for
 ...
 * @return array
 */
function profiles_core_get_users( $args = '' ) {
	...
	// For legacy users. Use of Profiles_Core_User::get_users() is deprecated.
	if ( apply_filters( 'profiles_use_legacy_user_query', false, __FUNCTION__, $r ) ) {
		$retval = Profiles_Core_User::get_users(
			$r['type'],
			$r['per_page'],
			$r['page'],
			$r['user_id'],
	...
	// Default behavior as of Profiles 1.7.0.
		...
		$users = new Profiles_User_Query( $r );

		// ...but reformat the results to match profiles_core_get_users() behavior.
		$retval = array(
	 ...
	 */
	return apply_filters( 'profiles_core_get_users', $retval, $r );
}
 ...
 * @return string
 */
function profiles_core_get_user_domain( $user_id = 0, $user_nicename = false, $user_login = false ) {

	if ( empty( $user_id ) ) {
		return;
	}

	$username = profiles_core_get_username( $user_id, $user_nicename, $user_login );

	if ( profiles_is_username_compatibility_mode() ) {
		$username = rawurlencode( $username );
	}

	$after_domain = profiles_core_enable_root_profiles() ? $username : profiles_get_members_root_slug() . '/' . $username;
	$domain       = trailingslashit( profiles_get_root_domain() . '/' . $after_domain );

	// Don't use this filter.  Subject to removal in a future release.
	// Use the 'profiles_core_get_user_domain' filter instead.
	$domain       = apply_filters( 'profiles_core_get_user_domain_pre_cache', $domain, $user_id, $user_nicename, $user_login );
	 ...
	 */
	return apply_filters( 'profiles_core_get_user_domain', $domain, $user_id, $user_nicename, $user_login );
}
 ...
 * @return array
 */
function profiles_core_get_core_userdata( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
		return false;
	}

	if ( !$userdata = wp_cache_get( 'profiles_core_userdata_' . $user_id, 'profiles' ) ) {
		$userdata = Profiles_Core_User::get_core_userdata( $user_id );
		wp_cache_set( 'profiles_core_userdata_' . $user_id, $userdata, 'profiles' );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_core_userdata', $userdata );
}
 ...
 * @return int
 */
function profiles_core_get_displayed_userid( $user_login ) {
	return apply_filters( 'profiles_core_get_displayed_userid', profiles_core_get_userid( $user_login ) );
}
 ...
 */
function profiles_core_get_userid( $username = '' ) {
	if ( empty( $username ) ) {
		return false;
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_userid', ! empty( $user->ID ) ? $user->ID : NULL, $username );
}
 ...
 * @return int|null The ID of the matched user on success, null on failure.
 */
function profiles_core_get_userid_from_nicename( $user_nicename = '' ) {
	 ...
	 */
	return apply_filters( 'profiles_core_get_userid_from_nicename', ! empty( $user->ID ) ? $user->ID : NULL, $user_nicename );
}

/**
 * Return the username for a user based on their user id.
 *
 * This function is sensitive to the Profiles_ENABLE_USERNAME_COMPATIBILITY_MODE,
 ...
 */
function profiles_core_get_username( $user_id = 0, $user_nicename = false, $user_login = false ) {
	$profiles = profiles();

	// Check cache for user nicename.
	$username = wp_cache_get( 'profiles_user_username_' . $user_id, 'profiles' );
	if ( false === $username ) {
		...
		if ( empty( $user_nicename ) && empty( $user_login ) ) {
			...
			if ( profiles_loggedin_user_id() == $user_id ) {
				$userdata = &$profiles->loggedin_user->userdata;

			// User ID matches displayed in user.
			} elseif ( profiles_displayed_user_id() == $user_id ) {
			...
			if ( empty( $userdata ) ) {
				...
				if ( !$userdata = profiles_core_get_core_userdata( $user_id ) ) {
					return false;
				}
			}
		...
		$username = profiles_is_username_compatibility_mode() ? $user_login : $user_nicename;
	...
	if ( ( true === $update_cache ) && !empty( $username ) ) {
		wp_cache_set( 'profiles_user_username_' . $user_id, $username, 'profiles' );

	// @todo bust this cache if no $username found?
	// } else {
	// wp_cache_delete( 'profiles_user_username_' . $user_id );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_username', $username );
}
 ...
 * @return string|bool The username of the matched user, or false.
 */
function profiles_members_get_user_nicename( $user_id ) {
	$profiles = profiles();

	if ( !$user_nicename = wp_cache_get( 'profiles_members_user_nicename_' . $user_id, 'profiles' ) ) {
		$update_cache = true;
	...
	if ( true == $update_cache && !empty( $user_nicename ) ) {
		wp_cache_set( 'profiles_members_user_nicename_' . $user_id, $user_nicename, 'profiles' );
	}
	 ...
	 */
	return apply_filters( 'profiles_members_get_user_nicename', $user_nicename );
}
 ...
 *                matched the $uid.
 */
function profiles_core_get_user_email( $uid ) {

	if ( !$email = wp_cache_get( 'profiles_user_email_' . $uid, 'profiles' ) ) {

		// User exists.
		$ud = profiles_core_get_core_userdata( $uid );
		if ( ! empty( $ud ) ) {
			$email = $ud->user_email;
		...
		wp_cache_set( 'profiles_user_email_' . $uid, $email, 'profiles' );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_user_email', $email );
}
 ...
 *                     no match.
 */
function profiles_core_get_userlink( $user_id, $no_anchor = false, $just_link = false ) {
	$display_name = profiles_core_get_user_displayname( $user_id );
	...
	if ( !$url = profiles_core_get_user_domain( $user_id ) ) {
		return false;
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_userlink', '<a href="' . $url . '" title="' . $display_name . '">' . $display_name . '</a>', $user_id );
}
 ...
 * @return array
 */
function profiles_core_get_user_displaynames( $user_ids ) {
	...
	foreach ( $user_ids as $user_id ) {
		if ( false === wp_cache_get( 'profiles_user_fullname_' . $user_id, 'profiles' ) ) {
	...
	if ( ! empty( $uncached_ids ) ) {
		if ( profiles_is_active( 'xprofile' ) ) {
			$fullname_data = Profiles_XProfile_ProfileData::get_value_byid( 1, $uncached_ids );
		...
		foreach ( $fullnames as $fuser_id => $fname ) {
			wp_cache_set( 'profiles_user_fullname_' . $fuser_id, $fname, 'profiles' );
		}
	}

	$retval = array();
	foreach ( $user_ids as $user_id ) {
		$retval[ $user_id ] = wp_cache_get( 'profiles_user_fullname_' . $user_id, 'profiles' );
	}
 ...
 *                     user not found.
 */
function profiles_core_get_user_displayname( $user_id_or_username ) {
	if ( empty( $user_id_or_username ) ) {
		return false;
	}

	if ( ! is_numeric( $user_id_or_username ) ) {
		$user_id = profiles_core_get_userid( $user_id_or_username );
	} else {
	...
	$display_names = profiles_core_get_user_displaynames( array( $user_id ) );
	 ...
	 * @param int    $user_id  ID of the user to check.
	 */
	return apply_filters( 'profiles_core_get_user_displayname', $fullname, $user_id );
}
add_filter( 'profiles_core_get_user_displayname', 'strip_tags', 1 );
add_filter( 'profiles_core_get_user_displayname', 'trim'          );
add_filter( 'profiles_core_get_user_displayname', 'stripslashes'  );
add_filter( 'profiles_core_get_user_displayname', 'esc_html'      );
 ...
 */
function profiles_core_get_userlink_by_email( $email ) {
	 ...
	 */
	return apply_filters( 'profiles_core_get_userlink_by_email', profiles_core_get_userlink( $user->ID, false, false, true ) );
}
 ...
 * @since 1.0.0
 *
 * @param string $username If Profiles_ENABLE_USERNAME_COMPATIBILITY_MODE is set,
 *                         this should be user_login, otherwise it should
 *                         be user_nicename.
 * @return string|bool The link to the user's domain, false on no match.
 */
function profiles_core_get_userlink_by_username( $username ) {
	if ( profiles_is_username_compatibility_mode() ) {
		$user_id = profiles_core_get_userid( $username );
	} else {
		$user_id = profiles_core_get_userid_from_nicename( $username );
	}
	 ...
	 * @param string|bool $value URL for the user if found, otherwise false.
	 */
	return apply_filters( 'profiles_core_get_userlink_by_username', profiles_core_get_userlink( $user_id, false, false, true ) );
}
 ...
 * {@link profiles_core_get_active_member_count()}.
 *
 * @since 1.2.0
 *
 * @return int The total number of members.
 */
function profiles_core_get_total_member_count() {
	global $wpdb;

	$count = wp_cache_get( 'profiles_total_member_count', 'profiles' );

	if ( false === $count ) {
		$status_sql = profiles_core_get_status_sql();
		$count = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->users} WHERE {$status_sql}" );
		wp_cache_set( 'profiles_total_member_count', $count, 'profiles' );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_total_member_count', $count );
}
 ...
 */
function profiles_core_get_active_member_count() {
	global $wpdb;

	$count = get_transient( 'profiles_active_member_count' );
	if ( false === $count ) {
		$profiles = profiles();
		...
		set_transient( 'profiles_active_member_count', $count );
	}
	 ...
	 */
	return apply_filters( 'profiles_core_get_active_member_count', $count );
}
 ...
 *
 * - in profiles_settings_action_capabilities() (from the front-end)
 * - by profiles_core_mark_user_spam_admin()    (from wp-admin)
 * - profiles_core_mark_user_ham_admin()        (from wp-admin)
 *
 * @since 1.6.0
 *
 ...
 */
function profiles_core_process_spammer_status( $user_id, $status, $do_wp_cleanup = true ) {
	global $wpdb;
	...
	remove_action( 'make_spam_user', 'profiles_core_mark_user_spam_admin' );
	remove_action( 'make_ham_user',  'profiles_core_mark_user_ham_admin'  );
	...
	if ( $do_wp_cleanup ) {
		...
		foreach ( (array) array_values( $blogs ) as $details ) {
			...
			if ( 1 == $details->userblog_id || profiles_get_root_blog_id() == $details->userblog_id ) {
				continue;
			}
	...
	if ( true === $is_spam ) {
		 ...
		 */
		do_action( 'profiles_make_spam_user', $user_id );
	} else {
		 ...
		 * @param int $value Displayed user ID.
		 */
		do_action( 'profiles_make_ham_user', $user_id );
	}
	 ...
	 */
	do_action( 'profiles_core_process_spammer_status', $user_id, $is_spam );

	// Put things back how we found them.
	add_action( 'make_spam_user', 'profiles_core_mark_user_spam_admin' );
	add_action( 'make_ham_user',  'profiles_core_mark_user_ham_admin'  );
 ...
 */
function profiles_core_mark_user_spam_admin( $user_id ) {
	profiles_core_process_spammer_status( $user_id, 'spam', false );
}
add_action( 'make_spam_user', 'profiles_core_mark_user_spam_admin' );
 ...
 */
function profiles_core_mark_user_ham_admin( $user_id ) {
	profiles_core_process_spammer_status( $user_id, 'ham', false );
}
add_action( 'make_ham_user', 'profiles_core_mark_user_ham_admin' );
 ...
 */
function profiles_is_user_spammer( $user_id = 0 ) {
	 ...
	 */
	return apply_filters( 'profiles_is_user_spammer', (bool) $is_spammer );
}
 ...
 */
function profiles_is_user_deleted( $user_id = 0 ) {
	 ...
	 */
	return apply_filters( 'profiles_is_user_deleted', (bool) $is_deleted );
}
 ...
 */
function profiles_is_user_active( $user_id = 0 ) {
	...
	// Check deleted.
	if ( profiles_is_user_deleted( $user_id ) ) {
		return false;
	}
 ...
 *       profiles_is_user_active().
 *
 * @param int $user_id The user ID to check.
 * @return bool True if inactive, otherwise false.
 */
function profiles_is_user_inactive( $user_id = 0 ) {
	...
	return !profiles_is_user_active( $user_id );
}
 ...
 * @return bool True on success, false on failure.
 */
function profiles_update_user_last_activity( $user_id = 0, $time = '' ) {
	...
	if ( empty( $time ) ) {
		$time = profiles_core_current_time();
	}
	...
	remove_filter( 'update_user_metadata', '_profiles_update_user_meta_last_activity_warning', 10, 4 );
	remove_filter( 'get_user_metadata', '_profiles_get_user_meta_last_activity_warning', 10, 3 );
	profiles_update_user_meta( $user_id, 'last_activity', $time );
	add_filter( 'update_user_metadata', '_profiles_update_user_meta_last_activity_warning', 10, 4 );
	add_filter( 'get_user_metadata', '_profiles_get_user_meta_last_activity_warning', 10, 3 );

	return Profiles_Core_User::update_last_activity( $user_id, $time );
}
 ...
 * @return mixed
 */
function _profiles_get_user_meta_last_activity_warning( $retval, $object_id, $meta_key ) {
	static $warned = false;

	if ( 'last_activity' === $meta_key ) {
		// Don't send the warning more than once per pageload.
		if ( false === $warned ) {
			_doing_it_wrong( 'get_user_meta( $user_id, \'last_activity\' )', __( 'User last_activity data is no longer stored in usermeta. Use profiles_get_user_last_activity() instead.', 'profiles' ), '2.0.0' );
			$warned = true;
		}

		return profiles_get_user_last_activity( $object_id );
	}
 ...
 */
function _profiles_update_user_meta_last_activity_warning( $meta_id, $object_id, $meta_key, $meta_value ) {
	if ( 'last_activity' === $meta_key ) {
		_doing_it_wrong( 'update_user_meta( $user_id, \'last_activity\' )', __( 'User last_activity data is no longer stored in usermeta. Use profiles_update_user_last_activity() instead.', 'profiles' ), '2.0.0' );
		profiles_update_user_last_activity( $object_id, $meta_value );
	}
}
add_filter( 'update_user_metadata', '_profiles_update_user_meta_last_activity_warning', 10, 4 );
 ...
 */
function profiles_get_user_last_activity( $user_id = 0 ) {
	$activity = '';

	$last_activity = Profiles_Core_User::get_last_activity( $user_id );
	 ...
	 */
	return apply_filters( 'profiles_get_user_last_activity', $activity, $user_id );
}
 ...
 * @return bool
 */
function profiles_last_activity_migrate() {
	global $wpdb;
 ...
 */
function profiles_core_get_all_posts_for_user( $user_id = 0 ) {
	global $wpdb;
	...
	return apply_filters( 'profiles_core_get_all_posts_for_user', $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_author = %d AND post_status = 'publish' AND post_type = 'post'", $user_id ) ) );
}
 ...
 * @return bool True on success, false on failure.
 */
function profiles_core_delete_account( $user_id = 0 ) {
	...
	if ( profiles_loggedin_user_id() !== absint( $user_id ) ) {

		// Bail if current user cannot delete any users.
		if ( ! profiles_current_user_can( 'delete_users' ) ) {
			return false;
		}

		// Bail if current user cannot delete this user.
		if ( ! current_user_can_for_blog( profiles_get_root_blog_id(), 'delete_user', $user_id ) ) {
			return false;
		}
	}
	 ...
	 */
	do_action( 'profiles_core_pre_delete_account', $user_id );
	 ...
	 */
	do_action( 'profiles_core_deleted_account', $user_id );
 ...
 * @return bool True on success, false on failure.
 */
function profiles_core_delete_avatar_on_user_delete( $user_id ) {
	return profiles_core_delete_existing_avatar( array(
		'item_id' => $user_id,
		'object'  => 'user',
	) );
}
add_action( 'wpmu_delete_user', 'profiles_core_delete_avatar_on_user_delete' );
add_action( 'delete_user', 'profiles_core_delete_avatar_on_user_delete' );
 ...
 * @return string
 */
function profiles_core_ucfirst( $str ) {
 ...
 */
function profiles_core_boot_spammer( $user ) {
	...
	if ( is_a( $user, 'WP_User' ) && ( ( is_multisite() && (int) $user->spam ) || 1 == $user->user_status ) ) {
		return new WP_Error( 'invalid_username', __( '<strong>ERROR</strong>: Your account has been marked as a spammer.', 'profiles' ) );
	}

	// User is good to go!
	return $user;
}
add_filter( 'authenticate', 'profiles_core_boot_spammer', 30 );
 ...
 */
function profiles_core_remove_data( $user_id ) {

	// Remove last_activity data.
	Profiles_Core_User::delete_last_activity( $user_id );

	// Flush the cache to remove the user from all cached objects.
	wp_cache_flush();
}
add_action( 'wpmu_delete_user',  'profiles_core_remove_data' );
add_action( 'delete_user',       'profiles_core_remove_data' );
add_action( 'profiles_make_spam_user', 'profiles_core_remove_data' );
 ...
 */
function profiles_core_can_edit_settings() {
	if ( profiles_is_my_profile() ) {
		return true;
	}

	if ( is_super_admin( profiles_displayed_user_id() ) && ! is_super_admin() ) {
		return false;
	}

	if ( profiles_current_user_can( 'profiles_moderate' ) || current_user_can( 'edit_users' ) ) {
		return true;
	}
 ...
 * @since 1.2.5
 */
function profiles_core_flush_illegal_names() {
 ...
 */
function profiles_core_get_illegal_names( $value = '', $oldvalue = '' ) {
	...
	$slug_constants = array(
		'Profiles_GROUPS_SLUG',
		'Profiles_MEMBERS_SLUG',
		'Profiles_FORUMS_SLUG',
		'Profiles_BLOGS_SLUG',
		'Profiles_ACTIVITY_SLUG',
		'Profiles_XPROFILE_SLUG',
		'Profiles_FRIENDS_SLUG',
		'Profiles_SEARCH_SLUG',
		'Profiles_SETTINGS_SLUG',
		'Profiles_NOTIFICATIONS_SLUG',
		'Profiles_REGISTER_SLUG',
		'Profiles_ACTIVATION_SLUG',
	);
	 ...
	 */
	$filtered_illegal_names = apply_filters( 'profiles_core_illegal_usernames', array_merge( array( 'www', 'web', 'root', 'admin', 'main', 'invite', 'administrator' ), $profiles_component_slugs ) );
	 ...
	 */
	return apply_filters( 'profiles_core_illegal_names', $illegal_names );
}
add_filter( 'pre_update_site_option_illegal_names', 'profiles_core_get_illegal_names', 10, 2 );
 ...
 *                    of error codes.
 */
function profiles_core_validate_email_address( $user_email ) {
	$errors = array();
 ...
 *
 * Functions like profiles_core_validate_email_address() return a structured array
 * of error codes. profiles_core_add_validation_error_messages() takes this array and
 * parses, adding the appropriate error messages to the WP_Error object.
 *
 * @since 1.7.0
 *
 * @see profiles_core_validate_email_address()
 *
 * @param WP_Error $errors             WP_Error object.
 * @param array    $validation_results The return value of a validation function
 *                                     like profiles_core_validate_email_address().
 */
function profiles_core_add_validation_error_messages( WP_Error $errors, $validation_results ) {
	if ( ! empty( $validation_results['invalid'] ) ) {
		$errors->add( 'user_email', __( 'Please check your email address.', 'profiles' ) );
	}

	if ( ! empty( $validation_results['domain_banned'] ) ) {
		$errors->add( 'user_email',  __( 'Sorry, that email address is not allowed!', 'profiles' ) );
	}
	...
	if ( ! empty( $validation_results['in_use'] ) ) {
		$errors->add( 'user_email', __( 'Sorry, that email address is already used!', 'profiles' ) );
	}
}
 ...
 */
function profiles_core_validate_user_signup( $user_name, $user_email ) {

	// Make sure illegal names include Profiles slugs and values.
	profiles_core_flush_illegal_names();
	...
	// minus the multisite-specific restrictions on user_login.
		...
		if ( empty( $user_name ) ) {
			$errors->add( 'user_name', __( 'Please enter a username', 'profiles' ) );
		}

		// User name can't be on the blacklist.
		$illegal_names = get_site_option( 'illegal_names' );
		if ( in_array( $user_name, (array) $illegal_names ) ) {
			$errors->add( 'user_name', __( 'That username is not allowed', 'profiles' ) );
		}

		// User name must pass WP's validity check.
		if ( ! validate_username( $user_name ) ) {
			$errors->add( 'user_name', __( 'Usernames can contain only letters, numbers, ., -, and @', 'profiles' ) );
		}

		// Minimum of 4 characters.
		if ( strlen( $user_name ) < 4 ) {
			$errors->add( 'user_name',  __( 'Username must be at least 4 characters', 'profiles' ) );
		}

		// No underscores. @todo Why not?
		if ( false !== strpos( ' ' . $user_name, '_' ) ) {
			$errors->add( 'user_name', __( 'Sorry, usernames may not contain the character "_"!', 'profiles' ) );
		}
		...
		if ( $match[0] == $user_name ) {
			$errors->add( 'user_name', __( 'Sorry, usernames must have letters too!', 'profiles' ) );
		}

		// Check into signups.
		$signups = Profiles_Signup::get( array(
		...
		if ( username_exists( $user_name ) || ! empty( $signup ) ) {
			$errors->add( 'user_name', __( 'Sorry, that username already exists!', 'profiles' ) );
		}

		// Validate the email address and process the validation results into
		// error messages.
		$validate_email = profiles_core_validate_email_address( $user_email );
		profiles_core_add_validation_error_messages( $errors, $validate_email );
	 ...
	 */
	return apply_filters( 'profiles_core_validate_user_signup', $result );
}
 ...
 * @return array
 */
function profiles_core_validate_blog_signup( $blog_url, $blog_title ) {
	 ...
	 */
	return apply_filters( 'profiles_core_validate_blog_signup', wpmu_validate_blog_signup( $blog_url, $blog_title ) );
}
 ...
 */
function profiles_core_signup_user( $user_login, $user_password, $user_email, $usermeta ) {
	$profiles = profiles();
	...
	if ( is_multisite() ) {
		 ...
		 * clutter by defining setting the Profiles_SIGNUPS_SKIP_USER_CREATION
		 * to true in your wp-config.php file.
		 */
		if ( ! defined( 'Profiles_SIGNUPS_SKIP_USER_CREATION' ) || ! Profiles_SIGNUPS_SKIP_USER_CREATION ) {
			$user_id = Profiles_Signup::add_backcompat( $user_login, $user_password, $user_email, $usermeta );

			if ( is_wp_error( $user_id ) ) {
				return $user_id;
			}

			profiles_update_user_meta( $user_id, 'activation_key', $activation_key );
		}
		 ...
		 *                               signup data, xprofile data, etc).
		 */
		if ( apply_filters( 'profiles_core_signup_send_activation_key', true, $user_id, $user_email, $activation_key, $usermeta ) ) {
			profiles_core_signup_send_validation_email( $user_id, $user_email, $activation_key, $user_login );
		}
	}
	 ...
	 *                                       signup data, xprofile data, etc).
	 */
	do_action( 'profiles_core_signup_user', $user_id, $user_login, $user_password, $user_email, $usermeta );
 ...
 * @return bool
 */
function profiles_core_signup_blog( $blog_domain, $blog_path, $blog_title, $user_name, $user_email, $usermeta ) {
	 ...
	 * @param void $value
	 */
	return apply_filters( 'profiles_core_signup_blog', wpmu_signup_blog( $blog_domain, $blog_path, $blog_title, $user_name, $user_email, $usermeta ) );
}
 ...
 */
function profiles_core_activate_signup( $key ) {
	global $wpdb;
	...
	if ( is_multisite() ) {
		...
		if ( empty( $signups['signups'] ) ) {
			return new WP_Error( 'invalid_key', __( 'Invalid activation key.', 'profiles' ) );
		}
		...
		if ( $signup->active ) {
			if ( empty( $signup->domain ) ) {
				return new WP_Error( 'already_active', __( 'The user is already active.', 'profiles' ), $signup );
			} else {
				return new WP_Error( 'already_active', __( 'The site is already active.', 'profiles' ), $signup );
			}
		}
		...
		// Create the user. This should only be necessary if Profiles_SIGNUPS_SKIP_USER_CREATION is true.
		if ( ! $user_id ) {
			$user_id = wp_create_user( $signup->user_login, $password, $signup->user_email );

		// Otherwise, update the existing user's status.
		} elseif ( $key === profiles_get_user_meta( $user_id, 'activation_key', true ) || $key === wp_hash( $user_id ) ) {
			...
			profiles_delete_user_meta( $user_id, 'activation_key' );
		...
		if ( ! $user_id ) {
			return new WP_Error( 'create_user', __( 'Could not create user', 'profiles' ), $signup );
		}
		...
		// Activate the signup.
		Profiles_Signup::validate( $key );

		if ( isset( $user_already_exists ) ) {
			return new WP_Error( 'user_already_exists', __( 'That username is already activated.', 'profiles' ), $signup );
		}
		...
		if ( isset( $user_already_created ) ) {
			 ...
			 */
			do_action( 'profiles_core_activated_user', $user_id, $key, $user );
			return $user_id;
		}
	}
 ...
 * @since 2.0.1
 */
function profiles_members_migrate_signups() {
	global $wpdb;
 ...
 * @return bool
 */
function profiles_core_map_user_registration( $user_id ) {
	...
	if ( profiles_is_active( 'xprofile' ) ) {
		$firstname = profiles_get_user_meta( $user_id, 'first_name', true );
		$lastname = ' ' . profiles_get_user_meta( $user_id, 'last_name', true );
 ...
 */
function profiles_core_signup_avatar_upload_dir() {
	$profiles = profiles();
	...
	$path      = profiles_core_avatar_upload_path() . '/' . $directory . '/' . $profiles->signup->avatar_dir;
	$newbdir   = $path;
	$newurl    = profiles_core_avatar_url() . '/' . $directory . '/' . $profiles->signup->avatar_dir;
	$newburl   = $newurl;
	 ...
	 */
	return apply_filters( 'profiles_core_signup_avatar_upload_dir', array(
		'path'    => $path,
		'url'     => $newurl,
		'subdir'  => $newsubdir,
		'basedir' => $newbdir,
 ...
 *
 * @param int|bool $user_id    ID of the new user, false if Profiles_SIGNUPS_SKIP_USER_CREATION is true.
 * @param string   $user_email Email address of the new user.
 * @param string   $key        Activation key.
 * @param string   $user_login Optional. The user login name.
 */
function profiles_core_signup_send_validation_email( $user_id, $user_email, $key, $user_login = '' ) {
	$args = array(
		'tokens' => array(
	...
	profiles_send_email( 'core-user-registration', $to, $args );
}
 ...
 */
function profiles_core_signup_disable_inactive( $user = null, $username = '', $password ='' ) {
	// Login form not used.
	...
	// {@link profiles_core_signup_user()} for more details.
	...
	$signup = Profiles_Signup::get( array( 'user_login' => sanitize_user( $user_login ) ) );
	...
	$resend_url_params = array(
		'action' => 'profiles-resend-activation',
		'id'     => $signup_id,
	);

	$resend_url = wp_nonce_url(
		add_query_arg( $resend_url_params, wp_login_url() ),
		'profiles-resend-activation'
	);

	$resend_string = '<br /><br />' . sprintf( __( 'If you have not received an email yet, <a href="%s">click here to resend it</a>.', 'profiles' ), esc_url( $resend_url ) );

	return new WP_Error( 'profiles_account_not_activated', __( '<strong>ERROR</strong>: Your account has not been activated. Check your email for the activation link.', 'profiles' ) . $resend_string );
}
add_filter( 'authenticate', 'profiles_core_signup_disable_inactive', 30, 3 );
 ...
 * @since 2.0.0
 *
 * @see profiles_core_signup_disable_inactive()
 */
function profiles_members_login_resend_activation_email() {
	global $error;
	...
	// Verify nonce.
	if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'profiles-resend-activation' ) ) {
		die( 'Security check' );
	}
	...
	$resend = Profiles_Signup::resend( array( $signup_id ) );

	// Add feedback message.
	if ( ! empty( $resend['errors'] ) ) {
		$error = __( '<strong>ERROR</strong>: Your account has already been activated.', 'profiles' );
	} else {
		$error = __( 'Activation email resent! Please check your inbox or spam folder.', 'profiles' );
	}
}
add_action( 'login_form_profiles-resend-activation', 'profiles_members_login_resend_activation_email' );
 ...
 * reauthenticate.
 *
 * Note: A spammer cannot log back in - {@see profiles_core_boot_spammer()}.
 *
 * Runs on 'profiles_init' at priority 5 so the members component globals are setup
 ...
 * @since 1.8.0
 */
function profiles_stop_live_spammer() {
	...
	if ( profiles_is_user_spammer( profiles_loggedin_user_id() ) ) {
		// Setup login args.
		$args = array(
			// Custom action used to throw an error message.
			'action' => 'profiles-spam',
		 ...
		 */
		$login_url = apply_filters( 'profiles_live_spammer_redirect', add_query_arg( $args, wp_login_url() ) );
...
add_action( 'profiles_init', 'profiles_stop_live_spammer', 5 );
 ...
 * @since 1.8.0
 */
function profiles_live_spammer_login_error() {
	global $error;

	$error = __( '<strong>ERROR</strong>: Your account has been marked as a spammer.', 'profiles' );

	// Shake shake shake!
	add_action( 'login_head', 'wp_shake_js', 12 );
}
add_action( 'login_form_profiles-spam', 'profiles_live_spammer_login_error' );
 ...
 */
function profiles_get_displayed_user() {
	$profiles = profiles();
	 ...
	 */
	return apply_filters( 'profiles_get_displayed_user', $displayed_user );
}
 ...
 */
function profiles_register_member_type( $member_type, $args = array() ) {
	$profiles = profiles();

	if ( isset( $profiles->members->types[ $member_type ] ) ) {
		return new WP_Error( 'profiles_member_type_exists', __( 'Member type already exists.', 'profiles' ), $member_type );
	}
	 ...
	 */
	$illegal_names = apply_filters( 'profiles_member_type_illegal_names', array( 'any', 'null', '_none' ) );
	if ( in_array( $member_type, $illegal_names, true ) ) {
		return new WP_Error( 'profiles_member_type_illegal_name', __( 'You may not register a member type with this name.', 'profiles' ), $member_type );
	}
	 ...
	 */
	do_action( 'profiles_registered_member_type', $member_type, $type );
 ...
 */
function profiles_get_member_type_object( $member_type ) {
	$types = profiles_get_member_types( array(), 'objects' );
 ...
 * @since 2.2.0
 *
 * @see profiles_register_member_type() for accepted arguments.
 *
 ...
 */
function profiles_get_member_types( $args = array(), $output = 'names', $operator = 'and' ) {
	$types = profiles()->members->types;
	 ...
	 */
	$types = apply_filters( 'profiles_get_member_types', $types, $args, $operator );
 ...
 * @return array $retval See {@see profiles_set_object_terms()}.
 */
function profiles_set_member_type( $user_id, $member_type, $append = false ) {
	// Pass an empty $member_type to remove a user's type.
	if ( ! empty( $member_type ) && ! profiles_get_member_type_object( $member_type ) ) {
		return false;
	}

	$retval = profiles_set_object_terms( $user_id, $member_type, 'profiles_member_type', $append );
	...
	if ( ! is_wp_error( $retval ) ) {
		 ...
		 */
		do_action( 'profiles_set_member_type', $user_id, $member_type, $append );
	}
 ...
 */
function profiles_remove_member_type( $user_id, $member_type ) {
	// Bail if no valid member type was passed.
	if ( empty( $member_type ) || ! profiles_get_member_type_object( $member_type ) ) {
		return false;
	}

	$deleted = profiles_remove_object_terms( $user_id, $member_type, 'profiles_member_type' );
	...
	if ( ! is_wp_error( $deleted ) ) {
		 ...
		 * @param string $member_type Member type.
		 */
		do_action( 'profiles_remove_member_type', $user_id, $member_type );
	}
 ...
 */
function profiles_get_member_type( $user_id, $single = true ) {
	$types = wp_cache_get( $user_id, 'profiles_member_member_type' );

	if ( false === $types ) {
		$types = profiles_get_object_terms( $user_id, 'profiles_member_type' );

		if ( ! is_wp_error( $types ) ) {
			$types = wp_list_pluck( $types, 'name' );
			wp_cache_set( $user_id, $types, 'profiles_member_member_type' );
		}
	}
	 ...
	 */
	return apply_filters( 'profiles_get_member_type', $type, $user_id, $single );
}
 ...
 */
function profiles_has_member_type( $user_id, $member_type ) {
	// Bail if no valid member type was passed.
	if ( empty( $member_type ) || ! profiles_get_member_type_object( $member_type ) ) {
		return false;
	}
	...
	$types = profiles_get_member_type( $user_id, false );
 ...
 *
 * @param int $user_id ID of the user.
 * @return array $value See {@see profiles_set_member_type()}.
 */
function profiles_remove_member_type_on_user_delete( $user_id ) {
	return profiles_set_member_type( $user_id, '' );
}
add_action( 'wpmu_delete_user', 'profiles_remove_member_type_on_user_delete' );
add_action( 'delete_user', 'profiles_remove_member_type_on_user_delete' );
 ...
 * @return string
 */
function profiles_get_current_member_type() {
	 ...
	 */
	return apply_filters( 'profiles_get_current_member_type', profiles()->current_member_type );
}
