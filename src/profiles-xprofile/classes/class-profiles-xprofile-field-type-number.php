 ...
 * @since 2.0.0
 */
class Profiles_XProfile_Field_Type_Number extends Profiles_XProfile_Field_Type {
	...
	public function __construct() {
		...
		$this->name     = _x( 'Number', 'xprofile field type', 'profiles' );

		$this->set_format( '/^\d+|-\d+$/', 'replace' );

		/**
		 * Fires inside __construct() method for Profiles_XProfile_Field_Type_Number class.
		 *
		 * @since 2.0.0
		 *
		 * @param Profiles_XProfile_Field_Type_Number $this Current instance of
		 *                                            the field type number.
		 */
		do_action( 'profiles_xprofile_field_type_number', $this );
	}
	...
	public function admin_field_html( array $raw_properties = array() ) {
		...
		<label for="<?php profiles_the_profile_field_input_name(); ?>" class="screen-reader-text"><?php
			...
			esc_html_e( 'Number field', 'profiles' );
		?></label>
		<input <?php echo $this->get_edit_field_html_elements( $r ); ?>>
	<?php
	}
