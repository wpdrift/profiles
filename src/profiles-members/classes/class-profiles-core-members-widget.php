 ...
 * @since 1.0.3
 */
class Profiles_Core_Members_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$name        = _x( '(Profiles) Members', 'widget name', 'profiles' );
		$description = __( 'A dynamic list of recently active, popular, and newest members', 'profiles' );

		// Call WP_Widget constructor.
		parent::__construct( false, $name, array(
			'description'                 => $description,
			'classname'                   => 'widget_profiles_core_members_widget profiles widget',
			'customize_selective_refresh' => true,
		) );

		if ( is_customize_preview() || is_active_widget( false, false, $this->id_base ) ) {
			add_action( 'profiles_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
		}
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'profiles-widget-members' );
	}
	...
	public function widget( $args, $instance ) {
		...
		$title = $settings['link_title'] ? '<a href="' . profiles_get_members_directory_permalink() . '">' . $title . '</a>' : $title;
		 ...
		 */
		$separator = apply_filters( 'profiles_members_widget_separator', '|' );
		...
		<?php if ( profiles_has_members( $members_args ) ) : ?>

			<div class="item-options" id="members-list-options">
				<a href="<?php profiles_members_directory_permalink(); ?>" id="newest-members" <?php if ( 'newest' === $settings['member_default'] ) : ?>class="selected"<?php endif; ?>><?php esc_html_e( 'Newest', 'profiles' ); ?></a>
				<span class="profiles-separator" role="separator"><?php echo esc_html( $separator ); ?></span>
				<a href="<?php profiles_members_directory_permalink(); ?>" id="recently-active-members" <?php if ( 'active' === $settings['member_default'] ) : ?>class="selected"<?php endif; ?>><?php esc_html_e( 'Active', 'profiles' ); ?></a>
				...
				<?php while ( profiles_members() ) : profiles_the_member(); ?>
					...
					<li class="vcard">
						<div class="item-avatar">
							<a href="<?php profiles_member_permalink() ?>" title="<?php profiles_member_name(); ?>"><?php profiles_member_avatar(); ?></a>
						</div>
							...
							<div class="item-meta">
								<span class="activity"><?php
									if ( 'newest' === $settings['member_default'] ) :
										profiles_member_registered();
									elseif ( 'active' === $settings['member_default'] ) :
										profiles_member_last_active();
									elseif ( 'popular' === $settings['member_default'] ) :
										profiles_member_total_friend_count();
									endif; ?></span>
							</div>
						</div>
					</li>
			...
			<?php wp_nonce_field( 'profiles_core_widget_members', '_wpnonce-members', false ); ?>
		...
		<?php else: ?>
			...
			<div class="widget-error">
				<?php esc_html_e( 'No one has signed up yet!', 'profiles' ); ?>
			</div>
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'link_title' ) ?>">
				...
				<?php esc_html_e( 'Link widget title to Members directory', 'profiles' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'member_default' ) ?>"><?php esc_html_e( 'Default members to show:', 'profiles' ); ?></label>
			<select name="<?php echo $this->get_field_name( 'member_default' ) ?>" id="<?php echo $this->get_field_id( 'member_default' ) ?>">
				<option value="newest"  <?php if ( 'newest'  === $member_default ) : ?>selected="selected"<?php endif; ?>><?php esc_html_e( 'Newest',  'profiles' ); ?></option>
				<option value="active"  <?php if ( 'active'  === $member_default ) : ?>selected="selected"<?php endif; ?>><?php esc_html_e( 'Active',  'profiles' ); ?></option>
				<option value="popular" <?php if ( 'popular' === $member_default ) : ?>selected="selected"<?php endif; ?>><?php esc_html_e( 'Popular', 'profiles' ); ?></option>
			</select>
		</p>
	...
	public function parse_settings( $instance = array() ) {
		return profiles_parse_args( $instance, array(
			'title' 	     => __( 'Members', 'profiles' ),
			'max_members' 	 => 5,
