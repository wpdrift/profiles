 ...
 * @since 2.3.0
 */
abstract class Profiles_Attachment {
	...
	public function __construct( $args = '' ) {
		...
		$params = profiles_parse_args( $args, $this->default_args, $this->action . '_upload_params' );
	 ...
	 *
	 * Used into the $overrides argument of Profiles_Attachment->upload()
	 *
	 * @since 2.3.0
	 *
	...
	public function set_upload_error_strings( $param = array() ) {
		 ...
		 */
		$upload_errors = array(
			0 => __( 'The file was uploaded successfully', 'profiles' ),
			1 => __( 'The uploaded file exceeds the maximum allowed file size for this site', 'profiles' ),
			2 => sprintf( __( 'The uploaded file exceeds the maximum allowed file size of: %s', 'profiles' ), size_format( $this->original_max_filesize ) ),
			3 => __( 'The uploaded file was only partially uploaded.', 'profiles' ),
			4 => __( 'No file was uploaded.', 'profiles' ),
			5 => '',
			6 => __( 'Missing a temporary folder.', 'profiles' ),
			7 => __( 'Failed to write file to disk.', 'profiles' ),
			8 => __( 'File upload stopped by extension.', 'profiles' ),
		);
	...
	public function upload( $file, $upload_dir_filter = '', $time = null ) {
		 ...
		 * Upload action and the file input name are required parameters.
		 *
		 * @see Profiles_Attachment:__construct()
		 */
		 ...
		 */
		$overrides = apply_filters( 'profiles_attachment_upload_overrides', $overrides );
		 ...
		 * @see  Profiles_Attachment->upload_dir_filter()
		 */
	 ...
	 * on the file size.
	 *
	 * @see Profiles_Attachment_Avatar->validate_upload() for an example of use
	 *
	 * @since 2.3.0
	 *
	...
	public function upload_dir_filter( $upload_dir = array() ) {
		 ...
		 * @param array $upload_dir     The original Uploads dir.
		 */
		return apply_filters( 'profiles_attachment_upload_dir', array(
	...
	public function crop( $args = array() ) {
		...
		$r = profiles_parse_args( $args, array(
			...
			'src_abs'       => false,
			'dst_file'      => false,
		), 'profiles_attachment_crop_args' );

		if ( empty( $r['original_file'] ) || ! file_exists( $r['original_file'] ) ) {
			$wp_error->add( 'crop_error', __( 'Cropping the file failed: missing source file.', 'profiles' ) );
			return $wp_error;
		}

		// Check image file pathes.
		$path_error = __( 'Cropping the file failed: the file path is not allowed.', 'profiles' );
		...
		$check_types = array( 'src_file' => array( 'file' => $r['original_file'], 'error' => _x( 'source file', 'Attachment source file', 'profiles' ) ) );
		if ( ! empty( $r['dst_file'] ) ) {
			$check_types['dst_file'] = array( 'file' => $r['dst_file'], 'error' => _x( 'destination file', 'Attachment destination file', 'profiles' ) );
		}
		...
		foreach ( $check_types as $file ) {
			...
			if ( empty( $ext ) || empty( $supported_image_types[ $ext ] ) ) {
				$wp_error->add( 'crop_error', sprintf( __( 'Cropping the file failed: %s is not a supported image file.', 'profiles' ), $file['error'] ) );
				return $wp_error;
			}
		}
	...
	public function script_data() {
		$script_data = array(
			...
			'feedback_messages' => array(
				1 => __( 'Sorry, uploading the file failed.', 'profiles' ),
				2 => __( 'File successfully uploaded.', 'profiles' ),
			),
		);
	...
	public static function get_image_data( $file ) {
		 ...
		 */
		return apply_filters( 'profiles_attachments_get_image_data', $image_data );
	}
