...
function profiles_core_set_uri_globals() {
	...
	if ( defined( 'DOING_AJAX' ) && DOING_AJAX || strpos( $_SERVER['REQUEST_URI'], 'wp-load.php' ) )
		$path = profiles_get_referer_path();
	else
	 ...
	 */
	$path = apply_filters( 'profiles_uri', $path );
	 ...
	 */
	if ( is_multisite() && !is_subdomain_install() && ( profiles_is_multiblog_mode() || 1 != profiles_get_root_blog_id() ) ) {
	...
	// Get site path items.
	$paths = explode( '/', profiles_core_get_site_path() );
	...
	$GLOBALS['profiles_unfiltered_uri'] = &$profiles->unfiltered_uri;
	...
	// URLs with Profiles_ENABLE_ROOT_PROFILES enabled won't be caught above.
	if ( empty( $matches ) && profiles_core_enable_root_profiles() && ! empty( $profiles_uri[0] ) ) {

		// Switch field based on compat.
		$field = profiles_is_username_compatibility_mode() ? 'login' : 'slug';
		 ...
		 */
		$member_slug = apply_filters( 'profiles_core_set_uri_globals_member_slug', $profiles_uri[0] );
	...
	if ( !empty( $profiles_uri[0] ) && ( profiles_get_search_slug() == $profiles_uri[0] ) ) {
		$matches[]   = 1;
		$match       = new stdClass;
		$match->key  = 'search';
		$match->slug = profiles_get_search_slug();
	}
	...
	// Global the unfiltered offset to use in profiles_core_load_template().
	// To avoid PHP warnings in profiles_core_load_template(), it must always be >= 0.
	...
	if ( isset( $match->key ) ) {
		...
		if ( 'members' == $match->key ) {
			...
			if ( $after_member_slug ) {
				...
				/** This filter is documented in profiles-core/profiles-core-catchuri.php */
				$after_member_slug = apply_filters( 'profiles_core_set_uri_globals_member_slug', $after_member_slug );
				...
				} elseif ( profiles_is_username_compatibility_mode() ) {
					$profiles->displayed_user->id = (int) profiles_core_get_userid( urldecode( $after_member_slug ) );

				} else {
					$profiles->displayed_user->id = (int) profiles_core_get_userid_from_nicename( $after_member_slug );
				}
			}

			// Is this a member type directory?
			if ( ! profiles_displayed_user_id() && $after_member_slug === profiles_get_members_member_type_base() && ! empty( $profiles_uri[ $uri_offset + 2 ] ) ) {
				$matched_types = profiles_get_member_types( array(
					'has_directory'  => true,
			...
			if ( ! profiles_displayed_user_id() && ! profiles_get_current_member_type() && $after_member_slug ) {
			...
			if ( profiles_displayed_user_id() && profiles_is_user_spammer( profiles_displayed_user_id() ) ) {
				if ( profiles_current_user_can( 'profiles_moderate' ) ) {
					profiles_core_add_message( __( 'This user has been marked as a spammer. Only site admins can view this profile.', 'profiles' ), 'warning' );
				} else {
					profiles_do_404();
					return;
				}
 ...
 */
function profiles_core_enable_root_profiles() {

	$retval = false;

	if ( defined( 'Profiles_ENABLE_ROOT_PROFILES' ) && ( true == Profiles_ENABLE_ROOT_PROFILES ) )
		$retval = true;
	 ...
	 */
	return apply_filters( 'profiles_core_enable_root_profiles', $retval );
}
 ...
 *
 * Example:
 *   profiles_core_load_template( 'members/index' );
 * Loads:
 ...
 */
function profiles_core_load_template( $templates ) {
	global $wp_query;
	...
	profiles_set_theme_compat_active( false );
	...
	// Only perform template lookup for profiles-default themes.
	if ( ! profiles_use_theme_compat_with_current_theme() ) {
	 ...
	 */
	$located_template = apply_filters( 'profiles_located_template', $template, $filtered_templates );

	/*
	 * If current page is an embed, wipe out profiles-default template.
	 *
	 * Wiping out the profiles-default template allows WordPress to use their special
	...
	if ( !empty( $located_template ) ) {
		 ...
		 */
		do_action( 'profiles_core_pre_load_template', $located_template );
		 ...
		 * @param string $located_template Template found to be loaded.
		 */
		load_template( apply_filters( 'profiles_load_template', $located_template ) );
		 ...
		 */
		do_action( 'profiles_core_post_load_template', $located_template );
	...
	// @todo Some other 404 handling if theme compat doesn't kick in.
		...
		// The rest will be done by profiles_theme_compat_reset_post() later.
		if ( is_profiles() ) {
			status_header( 200 );
			$wp_query->is_page     = true;
			$wp_query->is_singular = true;
		 ...
		 * @since 1.7.0
		 */
		do_action( 'profiles_setup_theme_compat' );
	}
}
 ...
 * @since 1.0.0
 */
function profiles_core_catch_profile_uri() {
	if ( !profiles_is_active( 'xprofile' ) ) {
		 ...
		 */
		profiles_core_load_template( apply_filters( 'profiles_core_template_display_profile', 'members/single/home' ) );
	}
}
 ...
 */
function profiles_core_members_shortlink_redirector( $member_slug ) {
	 ...
	 */
	$me_slug = apply_filters( 'profiles_core_members_shortlink_slug', 'me' );
	...
	if ( false === is_user_logged_in() ) {
		...
		add_action( 'template_redirect', 'profiles_core_no_access', 0 );
	...
	return profiles_core_get_username( $user->ID, $user->user_nicename, $user->user_login );
}
add_filter( 'profiles_core_set_uri_globals_member_slug', 'profiles_core_members_shortlink_redirector' );
 ...
 * @since 1.5.0
 */
function profiles_core_catch_no_access() {
	global $wp_query;

	$profiles = profiles();

	// If coming from profiles_core_redirect() and $profiles_no_status_set is true,
	// we are redirecting to an accessible page so skip this check.
	if ( !empty( $profiles->no_status_set ) )
		return false;

	if ( !isset( $wp_query->queried_object ) && !profiles_is_blog_page() ) {
		profiles_do_404();
	}
}
add_action( 'profiles_template_redirect', 'profiles_core_catch_no_access', 1 );
 ...
 *                            Default: the value of {@link profiles_get_root_domain()}.
 ...
 * }
 */
function profiles_core_no_access( $args = '' ) {
	...
	$defaults = array(
		...
		'root'     => profiles_get_root_domain(), // the landing page you get redirected to when a user doesn't have access.
	 ...
	 */
	$r = apply_filters( 'profiles_core_no_access', $r );
	extract( $r, EXTR_SKIP );

	/*
	 * @ignore Ignore these filters and use 'profiles_core_no_access' above.
	 */
	$mode     = apply_filters( 'profiles_no_access_mode',     $mode,     $root,     $redirect, $message );
	$redirect = apply_filters( 'profiles_no_access_redirect', $redirect, $root,     $message,  $mode    );
	$root     = apply_filters( 'profiles_no_access_root',     $root,     $redirect, $message,  $mode    );
	$message  = apply_filters( 'profiles_no_access_message',  $message,  $root,     $redirect, $mode    );
	$root     = trailingslashit( $root );

	switch ( $mode ) {

		// Option to redirect to wp-login.php.
		// Error message is displayed with profiles_core_no_access_wp_login_error().
		case 2 :
			if ( !empty( $redirect ) ) {
				profiles_core_redirect( add_query_arg( array( 'action' => 'profilesnoaccess' ), wp_login_url( $redirect ) ) );
			} else {
				profiles_core_redirect( $root );
			}

			break;

		// Redirect to root with "redirect_to" parameter.
		// Error message is displayed with profiles_core_add_message().
		case 1 :
		default :
			...
			if ( !empty( $message ) ) {
				profiles_core_add_message( $message, 'error' );
			}

			profiles_core_redirect( $url );
 ...
 *
 * Hooks into the "profilesnoaccess" action defined in profiles_core_no_access().
 *
 * @since 1.5.0
 *
 * @global string $error Error message to pass to wp-login.php.
 */
function profiles_core_no_access_wp_login_error() {
	global $error;
	 ...
	 */
	$error = apply_filters( 'profiles_wp_login_error', __( 'You must log in to access the page you requested.', 'profiles' ), $_REQUEST['redirect_to'] );

	// Shake shake shake!.
	add_action( 'login_head', 'wp_shake_js', 12 );
}
add_action( 'login_form_profilesnoaccess', 'profiles_core_no_access_wp_login_error' );
 ...
 * @since 1.6.0
 *
 * @see Profiles_Members_Component::setup_globals() where
 *      $profiles->canonical_stack['base_url'] and ['component'] may be set.
 * @see profiles_core_new_nav_item() where $profiles->canonical_stack['action'] may be set.
 */
function profiles_redirect_canonical() {
	 ...
	 */
	if ( !profiles_is_blog_page() && apply_filters( 'profiles_do_redirect_canonical', true ) ) {
		...
		$requested_url  = profiles_get_requested_url();
		...
		$canonical_url  = profiles_get_canonical_url();
		...
		if ( $canonical_url !== $req_url_clean ) {
			...
			if ( isset( $profiles->template_message ) ) {
				...
				profiles_core_add_message( $message, $message_type );
			}
			...
			profiles_core_redirect( $canonical_url, 301 );
		}
	}
}
 ...
 * @since 1.6.0
 */
function profiles_rel_canonical() {
	$canonical_url = profiles_get_canonical_url();
...
function profiles_get_canonical_url( $args = array() ) {
	...
	if ( 'page' == get_option( 'show_on_front' ) && $page_on_front = (int) get_option( 'page_on_front' ) ) {
		$front_page_component = array_search( $page_on_front, profiles_core_get_directory_page_ids() );

		// If requesting the front page component directory, canonical
		// URL is the front page. We detect whether we're detecting a
		// component *directory* by checking that profiles_current_action()
		// is empty - ie, this not a single item or a feed.
		if ( false !== $front_page_component && profiles_is_current_component( $front_page_component ) && ! profiles_current_action() ) {
			$profiles->canonical_stack['canonical_url'] = trailingslashit( profiles_get_root_domain() );
		...
		// the members directory to avoid redirect loops.
		} elseif ( profiles_is_register_page() && 'register' == $front_page_component && is_user_logged_in() ) {
			 ...
			 */
			$profiles->canonical_stack['canonical_url'] = apply_filters( 'profiles_loggedin_register_page_redirect_to', profiles_get_members_directory_permalink() );
		}
	}
	 ...
	 */
	return apply_filters( 'profiles_get_canonical_url', $canonical_url, $args );
}
 ...
 */
function profiles_get_requested_url() {
	$profiles = profiles();
	 ...
	 */
	return apply_filters( 'profiles_get_requested_url', $profiles->canonical_stack['requested_url'] );
}
 ...
 * @since 1.6.0
 */
function _profiles_maybe_remove_redirect_canonical() {
	if ( ! profiles_is_blog_page() )
		remove_action( 'template_redirect', 'redirect_canonical' );
}
add_action( 'profiles_init', '_profiles_maybe_remove_redirect_canonical' );
 ...
 * creates a race condition with profiles_template_redirect(), our piggyback hook.
 * Due to a legacy bug in Profiles, internal BP content (such as members and
 * groups) is marked 404 in $wp_query until profiles_core_load_template(), when BP
 ...
 * the race condition. If profiles_core_load_template() runs, it dies before reaching
 * maybe_redirect_404(). If profiles_core_load_template() does not run, it means that
 ...
 */
function _profiles_rehook_maybe_redirect_404() {
	if ( defined( 'NOBLOGREDIRECT' ) ) {
		remove_action( 'template_redirect', 'maybe_redirect_404' );
		add_action( 'template_redirect', 'maybe_redirect_404', 100 );
	}
}
add_action( 'template_redirect', '_profiles_rehook_maybe_redirect_404', 1 );
 ...
 * @since 1.6.0
 */
function _profiles_maybe_remove_rel_canonical() {
	if ( ! profiles_is_blog_page() && ! is_404() ) {
		remove_action( 'wp_head', 'rel_canonical' );
		add_action( 'profiles_head', 'profiles_rel_canonical' );
	}
}
add_action( 'wp_head', '_profiles_maybe_remove_rel_canonical', 8 );
