<?php
/**
 * Profiles XProfile Admin.
 *
 * @package Profiles
 * @suprofilesackage XProfileAdmin
 * @since 1.0.0
 */
...
if ( ! profiles()->do_autoload ) {
	require dirname( __FILE__ ) . '/classes/class-profiles-xprofile-user-admin.php';
}
...
function xprofile_admin_screen( $message = '', $type = 'error' ) {
	...
	$groups = profiles_xprofile_get_groups( array(
		'fetch_fields' => true
	) ); ?>
	...
	<div class="wrap">
		...
		<h1>
			<?php _ex( 'Profile Fields', 'Settings page header', 'profiles'); ?>
			<a id="add_group" class="add-new-h2" href="users.php?page=profiles-profile-setup&amp;mode=add_group"><?php _e( 'Add New Field Group', 'profiles' ); ?></a>
		</h1>
		...
		<form action="" id="profile-field-form" method="post">
			...
			wp_nonce_field( 'profiles_reorder_fields', '_wpnonce_reorder_fields'        );
			wp_nonce_field( 'profiles_reorder_groups', '_wpnonce_reorder_groups', false );
			...
			<div id="tabs">
					...
					<?php if ( !empty( $groups ) ) : foreach ( $groups as $group ) : ?>
						...
						<li id="group_<?php echo esc_attr( $group->id ); ?>">
							<a href="#tabs-<?php echo esc_attr( $group->id ); ?>" class="ui-tab">
								<?php
								/** This filter is documented in profiles-xprofile/profiles-xprofile-template.php */
								echo esc_html( apply_filters( 'profiles_get_the_profile_group_name', $group->name ) );
								?>

								<?php if ( !$group->can_delete ) : ?>
									<?php _e( '(Primary)', 'profiles'); ?>
								<?php endif; ?>
					...
					<div id="tabs-<?php echo esc_attr( $group->id ); ?>" class="tab-wrapper">
						<div class="tab-toolbar">
							<div class="tab-toolbar-left">
								<a class="button-primary" href="users.php?page=profiles-profile-setup&amp;group_id=<?php echo esc_attr( $group->id ); ?>&amp;mode=add_field"><?php _e( 'Add New Field', 'profiles' ); ?></a>
								<a class="button edit" href="users.php?page=profiles-profile-setup&amp;mode=edit_group&amp;group_id=<?php echo esc_attr( $group->id ); ?>"><?php _e( 'Edit Group', 'profiles' ); ?></a>

								<?php if ( $group->can_delete ) : ?>

									<div class="delete-button">
										<a class="confirm submitdelete deletion ajax-option-delete" href="users.php?page=profiles-profile-setup&amp;mode=delete_group&amp;group_id=<?php echo esc_attr( $group->id ); ?>"><?php _e( 'Delete Group', 'profiles' ); ?></a>
									</div>
						...
						<?php if ( ! empty( $group->description ) ) : ?>
							...
							<p><?php
							/** This filter is documented in profiles-xprofile/profiles-xprofile-template.php */
							echo esc_html( apply_filters( 'profiles_get_the_profile_group_description', $group->description ) );
							?></p>
						...
						<fieldset id="<?php echo esc_attr( $group->id ); ?>" class="connectedSortable field-group">
							...
							printf( esc_html__( 'Fields for "%s" Group', 'profiles' ), apply_filters( 'profiles_get_the_profile_group_name', $group->name ) );
							?></legend>
							...
							if ( !empty( $group->fields ) ) :
								...
								<p class="nodrag nofields"><?php _e( 'There are no fields in this group.', 'profiles' ); ?></p>
						...
						<?php if ( empty( $group->can_delete ) ) : ?>
							...
							<p><?php esc_html_e( '* Fields in this group appear on the signup page.', 'profiles' ); ?></p>
				...
				<?php endforeach; else : ?>
					...
					<div id="message" class="error"><p><?php _e( 'You have no groups.', 'profiles' ); ?></p></div>
					<p><a href="users.php?page=profiles-profile-setup&amp;mode=add_group"><?php _e( 'Add New Group', 'profiles' ); ?></a></p>
...
function xprofile_admin_manage_group( $group_id = null ) {
	...
	if ( isset( $_POST['save_group'] ) ) {
		...
		if ( Profiles_XProfile_Group::admin_validate() ) {
			...
			if ( false === $group->save() ) {
				$message = __( 'There was an error saving the group. Please try again.', 'profiles' );
				$type    = 'error';

			// Save successful.
			} else {
				$message = __( 'The group was saved successfully.', 'profiles' );
				$type    = 'success';

				// @todo remove these old options
				if ( 1 == $group_id ) {
					profiles_update_option( 'profiles-xprofile-base-group-name', $group->name );
				}
				 ...
				 * @since 1.0.0
				 *
				 * @param Profiles_XProfile_Group $group Current Profiles_XProfile_Group object.
				 */
...
function xprofile_admin_delete_group( $group_id ) {
	...
	if ( !$group->delete() ) {
		$message = __( 'There was an error deleting the group. Please try again.', 'profiles' );
		$type    = 'error';
	} else {
		$message = __( 'The group was deleted successfully.', 'profiles' );
		$type    = 'success';
...
function xprofile_admin_manage_field( $group_id, $field_id = null ) {
	...
	if ( isset( $_POST['saveField'] ) ) {
		if ( Profiles_XProfile_Field::admin_validate() ) {
			...
			if ( empty( $field_id ) ) {
				$message = __( 'There was an error saving the field. Please try again.', 'profiles' );
				$type    = 'error';
			} else {
				$message = __( 'The field was saved successfully.', 'profiles' );
				$type    = 'success';

				// @todo remove these old options
				if ( 1 == $field_id ) {
					profiles_update_option( 'profiles-xprofile-fullname-field-name', $field->name );
				}
				...
				if ( ! empty( $_POST['default-visibility'] ) && in_array( $_POST['default-visibility'], wp_list_pluck( profiles_xprofile_get_visibility_levels(), 'id' ) ) ) {
					profiles_xprofile_update_field_meta( $field_id, 'default_visibility', $_POST['default-visibility'] );
				}
				...
				if ( ! empty( $_POST['allow-custom-visibility'] ) && in_array( $_POST['allow-custom-visibility'], array( 'allowed', 'disabled' ) ) ) {
					profiles_xprofile_update_field_meta( $field_id, 'allow_custom_visibility', $_POST['allow-custom-visibility'] );
				}

				// Validate signup.
				if ( ! empty( $_POST['signup-position'] ) ) {
					profiles_xprofile_update_field_meta( $field_id, 'signup_position', (int) $_POST['signup-position'] );
				} else {
					profiles_xprofile_delete_meta( $field_id, 'field', 'signup_position' );
				}

				// Save autolink settings.
				if ( isset( $_POST['do_autolink'] ) && 'on' === wp_unslash( $_POST['do_autolink'] ) ) {
					profiles_xprofile_update_field_meta( $field_id, 'do_autolink', 'on' );
				} else {
					profiles_xprofile_update_field_meta( $field_id, 'do_autolink', 'off' );
				}
				 ...
				 * @since 1.0.0
				 *
				 * @param Profiles_XProfile_Field $field Current Profiles_XProfile_Field object.
				 */
				do_action( 'xprofile_fields_saved_field', $field );

				$groups = profiles_xprofile_get_groups();
			}
...
function xprofile_admin_delete_field( $field_id, $field_type = 'field', $delete_data = false ) {
	...
	if ( !$field->delete( (bool) $delete_data ) ) {
		...
		$message = sprintf( __( 'The %s was deleted successfully!', 'profiles' ), $field_type );
		$type    = 'success';
	...
	<fieldset id="draggable_field_<?php echo esc_attr( $field->id ); ?>"class="sortable<?php echo ' ' . $field->type; if ( !empty( $class ) ) echo ' ' . $class; ?>">
		<legend>
			<span>
				...
				<?php if ( empty( $field->can_delete )                                    ) : ?><?php esc_html_e( '(Primary)',  'profiles' ); endif; ?>
				<?php profiles_the_profile_field_required_label(); ?>
				<?php if ( profiles_xprofile_get_meta( $field->id, 'field', 'signup_position' ) ) : ?><?php esc_html_e( '(Sign-up)',  'profiles' ); endif; ?>
				<?php if ( profiles_get_member_types() ) : echo $field->get_member_type_label(); endif; ?>
		...
		<div class="field-wrapper">
			...
			<div class="actions">
				<a class="button edit" href="users.php?page=profiles-profile-setup&amp;group_id=<?php echo esc_attr( $admin_group->id ); ?>&amp;field_id=<?php echo esc_attr( $field->id ); ?>&amp;mode=edit_field"><?php _e( 'Edit', 'profiles' ); ?></a>

				<?php if ( $field->can_delete ) : ?>

					<div class="delete-button">
						<a class="confirm submit-delete deletion" href="users.php?page=profiles-profile-setup&amp;field_id=<?php echo esc_attr( $field->id ); ?>&amp;mode=delete_field"><?php _e( 'Delete', 'profiles' ); ?></a>
					</div>
...
function profiles_xprofile_admin_form_field_types( $select_field_type ) {
	...
	foreach ( profiles_xprofile_get_field_types() as $field_name => $field_class ) {
		...
		if ( ! $the_category ) {
			$the_category = _x( 'Other', 'xprofile field type category', 'profiles' );
		}
...
add_action( 'profiles_init', array( 'Profiles_XProfile_User_Admin', 'register_xprofile_user_admin' ), 11 );
