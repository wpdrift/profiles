 ...
 *
 * @package Profiles
 * @suprofilesackage XProfileFunctions
 * @since 1.5.0
 */
 ...
 *
 * Procedural wrapper for Profiles_XProfile_Group::get() method.
 *
 * @since 2.1.0
 *
 * @param array $args See {@link Profiles_XProfile_Group::get()} for description of arguments.
 * @return array $groups
 */
function profiles_xprofile_get_groups( $args = array() ) {

	$groups = Profiles_XProfile_Group::get( $args );
	 ...
	 */
	return apply_filters( 'profiles_xprofile_get_groups', $groups, $args );
}
...
function xprofile_insert_field_group( $args = '' ) {
	...
	$field_group              = new Profiles_XProfile_Group( $r['field_group_id'] );
...
function profiles_xprofile_get_field_types() {
	$fields = array(
		...
		'multiselectbox' => 'Profiles_XProfile_Field_Type_Multiselectbox',
		'number'         => 'Profiles_XProfile_Field_Type_Number',
		'url'            => 'Profiles_XProfile_Field_Type_URL',
		'radio'          => 'Profiles_XProfile_Field_Type_Radiobutton',
		'selectbox'      => 'Profiles_XProfile_Field_Type_Selectbox',
		'textarea'       => 'Profiles_XProfile_Field_Type_Textarea',
		'textbox'        => 'Profiles_XProfile_Field_Type_Textbox',
	);
...
function profiles_xprofile_create_field_type( $type ) {
	...
	if ( $class && class_exists( $class ) ) {
		...
		return new Profiles_XProfile_Field_Type_Placeholder;
	}
}
...
function xprofile_get_field( $field ) {
	if ( $field instanceof Profiles_XProfile_Field ) {
		...
		$_field = new Profiles_XProfile_Field();
		$_field->fill_data( $field );
	} else {
		$_field = Profiles_XProfile_Field::get_instance( $field );
	}
...
function xprofile_get_field_data( $field, $user_id = 0, $multi_format = 'array' ) {
	...
	$values = maybe_unserialize( Profiles_XProfile_ProfileData::get_value_byid( $field_id, $user_id ) );
...
function xprofile_set_field_data( $field, $user_id, $value, $is_required = false ) {
	...
	$field           = new Profiles_XProfile_ProfileData();
...
function xprofile_set_field_visibility_level( $field_id = 0, $user_id = 0, $visibility_level = '' ) {
	...
	$allowed_values = profiles_xprofile_get_visibility_levels();
	if ( !array_key_exists( $visibility_level, $allowed_values ) ) {
		return false;
	}

	// Stored in an array in usermeta.
	$current_visibility_levels = profiles_get_user_meta( $user_id, 'profiles_xprofile_visibility_levels', true );
	...
	return profiles_update_user_meta( $user_id, 'profiles_xprofile_visibility_levels', $current_visibility_levels );
}
...
function xprofile_get_field_visibility_level( $field_id = 0, $user_id = 0 ) {
	...
	$current_levels = profiles_get_user_meta( $user_id, 'profiles_xprofile_visibility_levels', true );
...
function xprofile_delete_field_data( $field = '', $user_id = 0 ) {
	...
	$field = new Profiles_XProfile_ProfileData( $field_id, $user_id );
...
function xprofile_format_profile_field( $field_type, $field_value ) {
	...
	$field_value = profiles_unserialize_profile_field( $field_value );
 ...
 *
 * Runs at profiles_setup_globals:100 to ensure that all components have loaded their
...
function xprofile_override_user_fullnames() {
	...
	if ( profiles_loggedin_user_id() ) {
		profiles()->loggedin_user->fullname = profiles_core_get_user_displayname( profiles_loggedin_user_id() );
	}
...
function xprofile_avatar_upload_dir( $directory = 'avatars', $user_id = 0 ) {
	...
	$path      = profiles_core_avatar_upload_path() . '/' . $directory. '/' . $user_id;
	$newbdir   = $path;
	$newurl    = profiles_core_avatar_url() . '/' . $directory. '/' . $user_id;
	$newburl   = $newurl;
...
/**
 * When search_terms are passed to Profiles_User_Query, search against xprofile fields.
 *
 * @since 2.0.0
 *
 * @param array         $sql   Clauses in the user_id SQL query.
 * @param Profiles_User_Query $query User query object.
 * @return array
 */
function profiles_xprofile_profiles_user_query_search( $sql, Profiles_User_Query $query ) {
	global $wpdb;
	...
	$search_terms_clean = profiles_esc_like( wp_kses_normalize_entities( $query->query_vars['search_terms'] ) );
...
add_action( 'profiles_user_query_uid_clauses', 'profiles_xprofile_profiles_user_query_search', 10, 2 );
...
function xprofile_sync_wp_profile( $user_id = 0 ) {
	...
	profiles_update_user_meta( $user_id, 'nickname',   $fullname  );
	profiles_update_user_meta( $user_id, 'first_name', $firstname );
	profiles_update_user_meta( $user_id, 'last_name',  $lastname  );
 ...
 */
function xprofile_sync_profiles_profile( &$errors, $update, &$user ) {

	// Bail if profile syncing is disabled.
	if ( profiles_disable_profile_sync() || ! $update || $errors->get_error_codes() ) {
		return;
	}
 ...
 * @return bool True on success, false on failure.
 */
function profiles_xprofile_delete_meta( $object_id, $object_type, $meta_key = false, $meta_value = false, $delete_all = false ) {
	global $wpdb;
	...
	add_filter( 'query', 'profiles_filter_metaid_column_name' );
	add_filter( 'query', 'profiles_xprofile_filter_meta_query' );
	...
	remove_filter( 'query', 'profiles_xprofile_filter_meta_query' );
	remove_filter( 'query', 'profiles_filter_metaid_column_name' );
 ...
 */
function profiles_xprofile_get_meta( $object_id, $object_type, $meta_key = '', $single = true ) {
	// Sanitize object type.
	if ( ! in_array( $object_type, array( 'group', 'field', 'data' ) ) ) {
		return false;
	}
 ...
 */
function profiles_xprofile_update_meta( $object_id, $object_type, $meta_key, $meta_value, $prev_value = '' ) {
	add_filter( 'query', 'profiles_filter_metaid_column_name' );
	add_filter( 'query', 'profiles_xprofile_filter_meta_query' );
 ...
 */
function profiles_xprofile_add_meta( $object_id, $object_type, $meta_key, $meta_value, $unique = false ) {
	add_filter( 'query', 'profiles_filter_metaid_column_name' );
	add_filter( 'query', 'profiles_xprofile_filter_meta_query' );
 ...
 * @return bool|int
 */
function profiles_xprofile_update_fieldgroup_meta( $field_group_id, $meta_key, $meta_value ) {
	return profiles_xprofile_update_meta( $field_group_id, 'group', $meta_key, $meta_value );
}
 ...
 * @return bool|int
 */
function profiles_xprofile_update_field_meta( $field_id, $meta_key, $meta_value ) {
	return profiles_xprofile_update_meta( $field_id, 'field', $meta_key, $meta_value );
}
 ...
 * @return bool|int
 */
function profiles_xprofile_update_fielddata_meta( $field_data_id, $meta_key, $meta_value ) {
	return profiles_xprofile_update_meta( $field_data_id, 'data', $meta_key, $meta_value );
}
 ...
 * @return int Field ID.
 */
function profiles_xprofile_fullname_field_id() {
	$id = wp_cache_get( 'fullname_field_id', 'profiles_xprofile' );

	if ( false === $id ) {
		global $wpdb;

		$profiles = profiles();
		$id = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$profiles->profile->table_name_fields} WHERE name = %s", profiles_xprofile_fullname_field_name() ) );

		wp_cache_set( 'fullname_field_id', $id, 'profiles_xprofile' );
	}
 ...
 */
function profiles_xprofile_fullname_field_name() {
	 ...
	 * @since 1.5.0
	 *
	 * @param string $value Profiles_XPROFILE_FULLNAME_FIELD_NAME Full name field constant.
	 */
	return apply_filters( 'profiles_xprofile_fullname_field_name', Profiles_XPROFILE_FULLNAME_FIELD_NAME );
}
 ...
 * @return bool
 */
function profiles_xprofile_is_richtext_enabled_for_field( $field_id = null ) {
	if ( ! $field_id ) {
	 ...
	 */
	return apply_filters( 'profiles_xprofile_is_richtext_enabled_for_field', $enabled, $field_id );
}
 ...
 * @return array
 */
function profiles_xprofile_get_visibility_levels() {
	 ...
	 */
	return apply_filters( 'profiles_xprofile_get_visibility_levels', profiles()->profile->visibility_levels );
}
 ...
 * @since 1.6.0
 *
 * @see Profiles_XProfile_Group::get()
 ...
 */
function profiles_xprofile_get_hidden_fields_for_user( $displayed_user_id = 0, $current_user_id = 0 ) {
	if ( !$displayed_user_id ) {
		$displayed_user_id = profiles_displayed_user_id();
	}
	...
	if ( !$current_user_id ) {
		$current_user_id = profiles_loggedin_user_id();
	}
	 ...
	 */
	return apply_filters( 'profiles_xprofile_get_hidden_fields_for_user', $hidden_fields, $displayed_user_id, $current_user_id );
}
 ...
 * current field. (See profiles_xprofile_get_hidden_fields_for_user().) This
 ...
 * @since 1.8.2
 *
 * @see profiles_xprofile_get_hidden_fields_for_user()
 *
 * @param int $displayed_user_id The id of the user the profile fields belong to.
 * @param int $current_user_id   The id of the user viewing the profile.
 * @return array An array of visibility levels hidden to the current user.
 */
function profiles_xprofile_get_hidden_field_types_for_user( $displayed_user_id = 0, $current_user_id = 0 ) {
	...
	if ( ! empty( $current_user_id ) ) {
		...
		if ( $displayed_user_id == $current_user_id || profiles_current_user_can( 'profiles_moderate' ) ) {
			$hidden_levels = array();
	 ...
	 * @param int   $current_user_id   ID of the current user.
	 */
	return apply_filters( 'profiles_xprofile_get_hidden_field_types_for_user', $hidden_levels, $displayed_user_id, $current_user_id );
}
...
function profiles_xprofile_get_fields_by_visibility_levels( $user_id, $levels = array() ) {
	...
	$user_visibility_levels = profiles_get_user_meta( $user_id, 'profiles_xprofile_visibility_levels', true );

	// Parse the user-provided visibility levels with the default levels, which may take
	// precedence.
	$default_visibility_levels = Profiles_XProfile_Group::fetch_default_visibility_levels();
