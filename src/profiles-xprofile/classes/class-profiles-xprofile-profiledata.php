 ...
 * @since 1.6.0
 */
class Profiles_XProfile_ProfileData {
	...
	/**
	 * Profiles_XProfile_ProfileData constructor.
	 *
	 * @since 1.5.0
	 *
	...
	public function populate( $field_id, $user_id ) {
		...
		$profiledata = wp_cache_get( $cache_key, 'profiles_xprofile_data' );
		...
		if ( false === $profiledata ) {
			...
			if ( $profiledata ) {
				wp_cache_set( $cache_key, $profiledata, 'profiles_xprofile_data' );
			}
		}
	...
	public function exists() {
		...
		$cached    = wp_cache_get( $cache_key, 'profiles_xprofile_data' );
		 ...
		 * @param Profiles_XProfile_ProfileData $this   Instance of the current Profiles_XProfile_ProfileData class.
		 */
	...
	public function save() {
		...
		$this->last_updated = apply_filters( 'xprofile_data_last_updated_before_save', profiles_core_current_time(), $this->id );
		 ...
		 * @since 1.0.0
		 *
		 * @param Profiles_XProfile_ProfileData $this Current instance of the profile data being saved.
		 */
	...
	public function delete() {
		 ...
		 * @since 1.9.0
		 *
		 * @param Profiles_XProfile_ProfileData $this Current instance of the profile data being deleted.
		 */
	...
	public static function get_data_for_user( $user_id, $field_ids ) {
		...
		$uncached_field_ids = profiles_xprofile_get_non_cached_field_ids( $user_id, $field_ids );
		...
		if ( ! empty( $uncached_field_ids ) ) {
			...
			foreach ( $uncached_field_ids as $field_id ) {
				...
				if ( isset( $queried_data[ $field_id ] ) ) {
					wp_cache_set( $cache_key, $queried_data[ $field_id ], 'profiles_xprofile_data' );
				...
				// to avoid future cache misses.
					...
					wp_cache_set( $cache_key, $d, 'profiles_xprofile_data' );
				}
			}
		}
		...
		foreach ( $field_ids as $field_id ) {
			$cache_key = "{$user_id}:{$field_id}";
			$data[]    = wp_cache_get( $cache_key, 'profiles_xprofile_data' );
		}
	...
	public static function get_fielddataid_byid( $field_id, $user_id ) {
		...
		if ( empty( $field_id ) || empty( $user_id ) ) {
			...
			// Check cache first.
			$cache_key = "{$user_id}:{$field_id}";
			$fielddata = wp_cache_get( $cache_key, 'profiles_xprofile_data' );
	...
	public static function get_value_byid( $field_id, $user_ids = null ) {
		...
		if ( empty( $user_ids ) ) {
			$user_ids = profiles_displayed_user_id();
		}
