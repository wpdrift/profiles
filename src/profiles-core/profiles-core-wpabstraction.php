 ...
 * running and will alter the environment so Profiles can run regardless.
 *
 * The code below mostly contains function mappings. This file is subject to
 * change at any time.
 *
 * @package Profiles
 * @suprofilesackage WPAbstraction
 * @since 1.2.0
 */
 ...
 */
function profiles_get_major_wp_version() {
	global $wp_version;
...
if ( !is_multisite() ) {
	...
	$wpdb->blogid      = Profiles_ROOT_BLOG;
	...
	if ( !function_exists( 'switch_to_blog' ) ) {
		...
		function switch_to_blog( $new_blog, $deprecated = null ) {
			return profiles_get_root_blog_id();
		}
	}
 ...
 */
function profiles_core_get_status_sql( $prefix = false ) {
	if ( !is_multisite() )
