...
class Profiles_XProfile_Group {
	...
	public function save() {
		 ...
		 * @since 1.0.0
		 *
		 * @param Profiles_XProfile_Group $this Current instance of the group being saved. Passed by reference.
		 */
	...
	public function delete() {
		 ...
		 * @since 2.0.0
		 *
		 * @param Profiles_XProfile_Group $this Current instance of the group being deleted. Passed by reference.
		 */
		...
		if ( Profiles_XProfile_Field::delete_for_group( $this->id ) ) {

			// Remove profile data for the groups fields.
			for ( $i = 0, $count = count( $this->fields ); $i < $count; ++$i ) {
				Profiles_XProfile_ProfileData::delete_for_field( $this->fields[$i]->id );
			}
		}
	...
	/**
	 * Populates the Profiles_XProfile_Group object with profile field groups, fields,
	 * and field data.
	 *
	 * @since 1.2.0
	 *
	...
	public static function get( $args = array() ) {
		...
		// exclude_fields parameter. See profiles_xprofile_get_hidden_fields_for_user().
		$hidden_user_fields = profiles_xprofile_get_hidden_fields_for_user( $r['user_id'] );
		...
		if ( profiles_get_member_types() ) {
			if ( $r['user_id'] || false !== $r['member_type'] ) {
				...
				if ( $r['user_id'] ) {
					$member_types = profiles_get_member_type( $r['user_id'], false );
					if ( empty( $member_types ) ) {
						$member_types = array( 'null' );
					}
				}

				$member_types_fields = Profiles_XProfile_Field::get_fields_for_member_type( $member_types );
		...
		$uncached_field_ids = profiles_get_non_cached_ids( $field_ids, 'profiles_xprofile_fields' );
		if ( ! empty( $uncached_field_ids ) ) {
			...
			foreach ( $uncached_fields as $uncached_field ) {
				...
				wp_cache_set( $fid, $uncached_field, 'profiles_xprofile_fields' );
			}
		}
		...
		if ( ! empty( $r['fetch_field_data'] ) ) {
			...
			if ( ! empty( $field_ids ) && ! empty( $r['user_id'] ) ) {
				$field_data = Profiles_XProfile_ProfileData::get_data_for_user( $r['user_id'], $field_ids );
			}
		...
		if ( ! empty( $r['update_meta_cache'] ) ) {
			profiles_xprofile_update_meta_cache( $object_ids );
		}
	...
	protected static function get_group_data( $group_ids ) {
		...
		foreach ( $group_ids as $group_id ) {
			...
			$group_data = wp_cache_get( $group_id, 'profiles_xprofile_groups' );
			if ( false !== $group_data ) {
		...
		if ( ! empty( $uncached_gids ) ) {
			...
			if ( ! empty( $queried_gdata ) && ! is_wp_error( $queried_gdata ) ) {
				...
				foreach ( (array) $queried_gdata as $gdata ) {
					...
					wp_cache_set( $gdata->id, $gdata, 'profiles_xprofile_groups' );
				}
			}
		}
	...
	public static function fetch_visibility_level( $user_id = 0, $fields = array() ) {
		...
		$visibility_levels = profiles_get_user_meta( $user_id, 'profiles_xprofile_visibility_levels', true );

		foreach( (array) $fields as $key => $field ) {

			// Does the admin allow this field to be customized?
			$visibility   = profiles_xprofile_get_meta( $field->id, 'field', 'allow_custom_visibility' );
			...
			// If no admin-set default is saved, fall back on a global default.
				...
				$fallback_visibility = profiles_xprofile_get_meta( $field->id, 'field', 'default_visibility' );
				...
				$field_visibility = ! empty( $fallback_visibility )
					? $fallback_visibility
					: apply_filters( 'profiles_xprofile_default_visibility_level', 'public' );
			}
	...
	public static function fetch_default_visibility_levels() {
		...
		$default_visibility_levels = wp_cache_get( 'default_visibility_levels', 'profiles_xprofile' );
		...
		if ( false === $default_visibility_levels ) {
			...
			wp_cache_set( 'default_visibility_levels', $default_visibility_levels, 'profiles_xprofile' );
		}
	...
	public function render_admin_form() {
		...
		if ( empty( $this->id ) ) {
			...
			$action	= add_query_arg( array( 'page' => 'profiles-profile-setup', 'mode' => 'add_group' ), 'users.php' );
			$button	= __( 'Save', 'profiles' );

		// Existing field group.
		} else {
			$title  = __( 'Edit Field Group', 'profiles' );
			$action	= add_query_arg( array( 'page' => 'profiles-profile-setup', 'mode' => 'edit_group', 'group_id' => $this->id ), 'users.php' );
		...
		<div class="wrap">
			...
			<form id="profiles-xprofile-add-field-group" action="<?php echo esc_url( $action ); ?>" method="post">
				<div id="poststuff">
					<div id="post-body" class="metabox-holder columns-<?php echo ( 1 == get_current_screen()->get_columns() ) ? '1' : '2'; ?>">
						<div id="post-body-content">
							 ...
							 * @since 2.6.0
							 *
							 * @param Profiles_XProfile_Group $this Current XProfile group.
							 */
									...
									<div id="submitcomment" class="submitbox">
										<div id="major-publishing-actions">
											...
											<div id="delete-action">
												<a href="users.php?page=profiles-profile-setup" class="deletion"><?php _e( 'Cancel', 'profiles' ); ?></a>
											</div>
											<div class="clear"></div>
										</div>
									</div>
