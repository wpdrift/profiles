 ...
 *
 * @package    Profiles
 * @suprofilesackage XProfileSettings
 * @since 2.0.0
 */
 ...
 * @return array
 */
function profiles_xprofile_get_settings_fields( $args = '' ) {
 ...
 *
 */
function profiles_xprofile_settings_add_feedback_message() {
	...
	if ( ! profiles_is_my_profile() && profiles_core_can_edit_settings() ) {
		$message = __( "This member's profile settings have been saved.", 'profiles' );
	}

	// Add the message.
	profiles_core_add_message( $message, $type );
}
add_action( 'profiles_xprofile_settings_after_save', 'profiles_xprofile_settings_add_feedback_message' );
