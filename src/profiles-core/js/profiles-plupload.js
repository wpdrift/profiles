/* global profiles, plupload, Profiles_Uploader, _, JSON, Backbone */
...
( function( exports, $ ) {
	...
	/**
	 * Extend the profiles global with what we need from the wp one.
	 * and make sure previously defined Profiles attributes
	 * are not removed (eg: profiles.mentions)
	 */
	_.extend( profiles, _.pick( wp, 'Backbone', 'ajax', 'template' ) );
	 ...
	 */
	profiles.Uploader.uploader = function() {
		var self = this,
			isIE = navigator.userAgent.indexOf('Trident/') !== -1 || navigator.userAgent.indexOf('MSIE ') !== -1;

		this.params  = Profiles_Uploader.settings;
		this.strings = Profiles_Uploader.strings;
		...
		this.uploader.bind( 'Init', function( uploader ) {
			 ...
			 * difficulties, we're adding a ui_available_width argument to the profiles_params
			 ...
			 */
			if ( 'profiles_avatar_upload' ===  uploader.settings.multipart_params.action ) {
				 uploader.settings.multipart_params.profiles_params.ui_available_width = container.width();
			}
		...
		this.feedback = function( message, data, file ) {
			...
			profiles.Uploader.filesError.unshift( {
				message: message,
				data:    data,
				file:    file
			} );
		...
		this.uploader.bind( 'FilesAdded', function( uploader, files ) {
			...
			if ( ! uploader.settings.multi_selection && files.length > 1 ) {
				...
				$( self ).trigger( 'profiles-uploader-warning', self.strings.unique_file_warning );
				return;
			}
			...
			_.each( files, function( file ) {
				...
				if ( max > hundredmb && file.size > hundredmb && uploader.runtime !== 'html5' ) {
					...
					file.item = new profiles.Models.File( attributes );
					profiles.Uploader.filesQueue.add( file.item );
				}
		...
		this.uploader.bind( 'FileUploaded', function( uploader, file, response ) {
			...
			profiles.Uploader.filesUploaded.add( file.item );
		...
		this.uploader.bind( 'BeforeUpload', function( uploader, files ) {
			$( self ).trigger( 'profiles-uploader-new-upload', uploader, files );
		} );
		...
		this.uploader.bind( 'UploadComplete', function( uploader, files ) {
			$( self ).trigger( 'profiles-uploader-upload-complete', uploader, files );
			profiles.Uploader.filesQueue.reset();
		} );
		...
		this.uploader.bind( 'Error', function( uploader, pluploadError ) {
			...
			$( self ).trigger( 'profiles-uploader-warning', message );
			uploader.refresh();
		} );
	};

	// Create a very generic Model for files
	profiles.Models.File = Backbone.Model.extend( {
		file: {}
	} );

	// Add Collections to store queue, uploaded files and errors
	$.extend( profiles.Uploader, {
	...
	profiles.View = profiles.Backbone.View.extend( {
	...
	profiles.Views.Uploader = profiles.View.extend( {
		className: 'profiles-uploader-window',
		template: profiles.template( 'upload-window' ),

		defaults: _.pick( Profiles_Uploader.settings.defaults, 'container', 'drop_element', 'browse_button' ),
		...
		initUploader: function() {
			this.uploader = new profiles.Uploader.uploader();
			$( this.uploader ).on( 'profiles-uploader-warning', _.bind( this.setWarning, this ) );
			$( this.uploader ).on( 'profiles-uploader-new-upload', _.bind( this.resetWarning, this ) );
		},
		...
		setWarning: function( event, message ) {
			...
			var warning = new profiles.Views.uploaderWarning( {
				value: message
			} ).render();
	...
	profiles.Views.uploaderWarning = profiles.View.extend( {
		tagName: 'p',
		className: 'warning',
	...
	profiles.Views.uploaderStatus = profiles.View.extend( {
		className: 'files',

		initialize: function() {
			_.each( this.collection.models, this.addFile, this );
			this.collection.on( 'change:percent', this.progress, this );
			profiles.Uploader.filesError.on( 'add', this.feedback, this );
		},

		addFile: function( file ) {
			this.views.add( new profiles.Views.uploaderProgress( { model: file } ) );
		},

		progress:function( model ) {
			if ( ! _.isUndefined( model.get( 'percent' ) ) ) {
				$( '#' + model.get('id') + ' .profiles-progress .profiles-bar' ).css( 'width', model.get('percent') + '%' );
			}
		},
	...
	profiles.Views.uploaderProgress = profiles.View.extend( {
		className: 'profiles-uploader-progress',
		template: profiles.template( 'progress-window' )
	} );
