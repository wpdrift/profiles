 ...
 * @since 2.0.0
 */
function profiles_core_admin_tools() {
	?>
	<div class="wrap">
		...
		<form class="settings" method="post" action="">
					...
					<tr valign="top">
						...
						<td>
							<fieldset>
								...
								<?php foreach ( profiles_admin_repair_list() as $item ) : ?>
			...
			<fieldset class="submit">
				<input class="button-primary" type="submit" name="profiles-tools-submit" value="<?php esc_attr_e( 'Repair Items', 'profiles' ); ?>" />
				<?php wp_nonce_field( 'profiles-do-counts' ); ?>
			</fieldset>
		</form>
	</div>
	<?php
 ...
 * @since 2.0.0
 */
function profiles_admin_repair_handler() {
	if ( ! profiles_is_post_request() || empty( $_POST['profiles-tools-submit'] ) ) {
		return;
	}

	check_admin_referer( 'profiles-do-counts' );

	// Bail if user cannot moderate.
	$capability = profiles_core_do_network_admin() ? 'manage_network_options' : 'manage_options';
	if ( ! profiles_current_user_can( $capability ) ) {
		return;
	}
	...
	foreach ( (array) profiles_admin_repair_list() as $item ) {
	...
	if ( count( $messages ) ) {
		foreach ( $messages as $message ) {
			profiles_admin_tools_feedback( $message[1] );
		}
	}
}
add_action( profiles_core_admin_hook(), 'profiles_admin_repair_handler' );
 ...
 * @return array
 */
function profiles_admin_repair_list() {
	$repair_list = array();
	...
	$repair_list[20] = array(
		'profiles-total-member-count',
		__( 'Repair total members count.', 'profiles' ),
		'profiles_admin_repair_count_members',
	);

	$repair_list[25] = array(
		'profiles-last-activity',
		__( 'Repair member "last activity" data.', 'profiles' ),
		'profiles_admin_repair_last_activity',
	);
	...
	if ( profiles_is_active( 'friends' ) ) {
		$repair_list[0] = array(
			'profiles-user-friends',
			__( 'Repair total friends count for each member.', 'profiles' ),
			'profiles_admin_repair_friend_count',
		);
	}
	...
	if ( profiles_is_active( 'groups' ) ) {
		$repair_list[10] = array(
			...
			'profiles_admin_repair_group_count',
		);
	}

	// Blogs:
	// - user blog count.
	if ( profiles_is_active( 'blogs' ) ) {
		$repair_list[90] = array(
			'profiles-blog-records',
			__( 'Repopulate site tracking records.', 'profiles' ),
			'profiles_admin_repair_blog_records',
		);
	}
	 ...
	 */
	return (array) apply_filters( 'profiles_repair_list', $repair_list );
}
 ...
 * @return array
 */
function profiles_admin_repair_friend_count() {
	global $wpdb;
		...
		while ( $offset < $total_users ) {
			...
			foreach ( $friendships as $friendship ) {
				if ( ! isset( $updated[ $friendship->initiator_user_id ] ) ) {
					Profiles_Friends_Friendship::total_friend_count( $friendship->initiator_user_id );
					$updated[ $friendship->initiator_user_id ] = 1;
				}

				if ( ! isset( $updated[ $friendship->friend_user_id ] ) ) {
					Profiles_Friends_Friendship::total_friend_count( $friendship->friend_user_id );
 ...
 * @return array
 */
function profiles_admin_repair_group_count() {
	global $wpdb;
	...
	if ( $total_users > 0 ) {
		...
		while ( $offset < $total_users ) {
			...
			foreach ( $users as $user ) {
				Profiles_Groups_Member::refresh_total_group_count_for_user( $user );
			}
 ...
 * @return array
 */
function profiles_admin_repair_blog_records() {
 ...
 * @since 2.0.0
 */
function profiles_admin_repair_count_members() {
	$statement = __( 'Counting the number of active members on the site&hellip; %s', 'profiles' );
	delete_transient( 'profiles_active_member_count' );
	profiles_core_get_active_member_count();
	return array( 0, sprintf( $statement, __( 'Complete!', 'profiles' ) ) );
}
 ...
 * @since 2.0.0
 */
function profiles_admin_repair_last_activity() {
 ...
 * @return bool
 */
function profiles_admin_tools_feedback( $message, $class = false ) {
	...
	add_action( profiles_core_do_network_admin() ? 'network_admin_notices' : 'admin_notices', $lambda );
 ...
 * @since 2.0.0
 */
function profiles_core_admin_available_tools_page() {
	?>
	<div class="wrap">
		 ...
		 * @since 2.0.0
		 */
		do_action( 'profiles_network_tool_box' ); ?>
 ...
 * @since 2.0.0
 */
function profiles_core_admin_available_tools_intro() {
	$query_arg = array(
		'page' => 'profiles-tools'
	);

	$page = profiles_core_do_network_admin() ? 'admin.php' : 'tools.php' ;
	$url  = add_query_arg( $query_arg, profiles_get_admin_url( $page ) );
	?>
 ...
 * @return array
 */
function profiles_admin_reinstall_emails() {
	$switched = false;
	...
	$email_types = get_terms( profiles_get_email_tax_type(), array(
		'fields'                 => 'ids',
		'hide_empty'             => false,
	...
	if ( $email_types ) {
		foreach ( $email_types as $term_id ) {
			wp_delete_term( (int) $term_id, profiles_get_email_tax_type() );
		}
	}
 ...
 * @since 2.6.0
 */
function profiles_core_admin_notice_repopulate_blogs_resume() {
	$screen = get_current_screen();
	if ( 'tools_page_profiles-tools-network' !== $screen->id ) {
		return;
	}

	if ( '' === profiles_get_option( '_profiles_record_blogs_offset' ) ) {
		return;
	}
...
add_action( 'network_admin_notices', 'profiles_core_admin_notice_repopulate_blogs_resume' );
